...
const html_plugin = require("./itds-html");
const vue_style_plugin = require("./itds-vue-style");
...
const argv = yargs
	...
	.option("skip_itds", {
		type: "boolean",
		description: "Skip building itds assets",
	})
	.option("files", {
		type: "string",
	...
	.option("using-cached", {
		type: "boolean",
		description:
			"Skips build and uses cached build artifacts to update assets.json (used by Shashi)",
	})
	.example("node esbuild --apps itds,okayblue", "Run build only for itds and okayblue")
	.example(
		"node esbuild --files itds/website.bundle.js,itds/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_itds && app == "itds")
);
...
function get_files_to_build(files) {
	// files: ['itds/website.bundle.js', 'okayblue/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
...
async function update_assets_json_in_cache() {
	...
	if (process.env.ITDS_DOCKER_BUILD) {
		return;
	}
...
async function notify_redis({ error, success, changed_files }) {
	...
	if (error) {
		...
		let stack = error.stack.replace(new RegExp(shashi_path, "g"), "");
		payload = {
			error,
			formatted,
			stack,
