...
if (process.env.ITDS_SHASHI_ROOT) {
	...
	const itds_path = path.resolve(__dirname, "..");
	shashi_path = path.resolve(itds_path, "..", "..");
}
...
function get_cloned_apps() {
	/**
	 * Returns itds apps in the shashi/apps folder
	 */
	const apps = [];
	for (const app of fs.readdirSync(apps_path)) {
		const app_path = path.resolve(apps_path, app);
		if (is_itds_app(app, app_path)) apps.push(app);
	}

	return apps;
}

function is_itds_app(app_name, app_path) {
	/**
	 * Same as the is_itds_app check in itds/shashi
	 */
