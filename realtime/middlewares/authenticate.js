...
function authenticate_with_itds(socket, next) {
	...
	socket.itds_request = (path, args = {}, opts = {}) => {
	...
	socket
		.itds_request("/api/method/itds.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-itds-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-itds-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_itds;
