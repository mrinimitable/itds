...
import itds
from itds.defaults import _clear_cache
from itds.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		itds.init(site)
		itds.connect()
		enable_scheduler = cint(itds.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		itds.db.close()
...
def _new_site(
	...
	"""Install a new Itds site"""

	from itds.utils import scheduler
	...
	if not db_name:
		db_name = f"_{itds.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(itds.get_site_path()))

	with filelock("shashi_new_site", timeout=1):
		install_db(
		...
		apps_to_install = ["itds"] + (itds.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if itds.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from itds.database import bootstrap_database, drop_user_and_database, setup_database
	...
	itds.flags.in_install_db = True

	if root_login:
		itds.flags.root_login = root_login

	if root_password:
		itds.flags.root_password = root_password
	...
	itds.conf.admin_password = itds.conf.admin_password or admin_password

	remove_missing_apps()

	itds.db.create_auth_table()
	itds.db.create_global_search_table()
	itds.db.create_user_settings_table()

	itds.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/mrinimitable/shashi/blob/develop/shashi/utils/__init__.py#L390
	...
	from itds.exceptions import InvalidRemoteException

	for org in ["itds", "okayblue"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/mrinimitable/shashi/blob/develop/shashi/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/mrinimitable/shashi/blob/develop/shashi/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from itds.model.sync import sync_for
	from itds.modules.utils import sync_customizations
	from itds.utils.fixtures import sync_fixtures

	itds.flags.in_install = name
	itds.flags.ignore_in_install = False

	itds.clear_cache()
	app_hooks = itds.get_hooks(app_name=name)
	...
	if name not in itds.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = itds.get_attr(before_install)()
		if out is False:
			return

	for fn in itds.get_hooks("before_app_install"):
		itds.get_attr(fn)(name)
	...
	itds.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		itds.get_attr(after_install)()

	for fn in itds.get_hooks("after_app_install"):
		itds.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		itds.get_attr(after_sync)()  #

	itds.client_cache.erase_persistent_caches()
	itds.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		itds.db.set_global("installed_apps", json.dumps(installed_apps))
		itds.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = itds.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in itds.get_installed_apps():
	...
	for app in itds.get_installed_apps():
		if app != app_name:
			hooks = itds.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from itds.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	itds.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		itds.get_attr(before_uninstall)()

	for fn in itds.get_hooks("before_app_uninstall"):
		itds.get_attr(fn)(app_name)

	modules = itds.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		itds.get_attr(after_uninstall)()

	for fn in itds.get_hooks("after_app_uninstall"):
		itds.get_attr(fn)(app_name)

	itds.client_cache.erase_persistent_caches()

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	itds.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in itds.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					itds.delete_doc(doctype.name, doctype.name, ignore_on_trash=True, force=True)
					itds.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			itds.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in itds.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				itds.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = itds.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if itds.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			itds.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			itds.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from itds.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		itds.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	itds.db.commit()


def init_singles():
	singles = itds.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if itds.db.get_singles_dict(single):
			continue

		try:
			doc = itds.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, itds.DoesNotExistError):
...
def make_conf(
	...
	sites_path = itds.local.sites_path
	itds.destroy()
	itds.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	itds.create_folder(os.path.join(itds.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from itds.config import clear_site_config_cache
	from itds.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(itds.local, "conf"):
		itds.local.conf[key] = value


def get_site_config_path():
	return os.path.join(itds.local.site_path, "site_config.json")
...
def clear_site_locks():
	...
	path = Path(itds.get_site_path("locks"))
...
def make_site_dirs():
	for dir_path in [
		...
		path = itds.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = itds.get_module_list(app)
	for module in modules:
		d = itds.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("itds_subscription", "shopping_cart")
	installed_apps = json.loads(itds.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from itds.utils import get_shashi_relative_path

	file_path = get_shashi_relative_path(file_path)

	# Need to do itds.init to maintain the site locals
	itds.init(site_name)
	abs_site_path = os.path.abspath(itds.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	"""Check if input db backup will get downgraded on current shashi
	...
	current_version = Version(itds.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Itds {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the itds version used to create the specified database dump.

	This methods supports older versions of Itds wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Itds (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin itds metadata" in header[0]:
		for line in header[1:]:
			if "end itds metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["itds"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if itds.conf.db_type == "mariadb":
		from itds.database.mariadb.setup_db import import_db_from_sql
	elif itds.conf.db_type == "postgres":
		import warnings

		from itds.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			itds.throw(
				...
				exc=itds.ExecutableNotFound,
			)
		try:
			itds.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise itds.InvalidDatabaseFile
