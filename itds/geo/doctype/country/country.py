...
import itds
from itds import _
from itds.model.document import Document, bulk_insert
...
def get_countries_and_currencies():
	from itds.geo.country_info import get_all as get_geo_data
		...
		if country.currency and country.currency not in added_currencies:
			...
			currencies.append(
				itds.get_doc(
					...
					number_format=itds.db.escape(country.number_format)[1:-1],
				)
			)
