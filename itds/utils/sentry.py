...
import itds
import itds.monitor
from itds.database.database import Database, EmptyQueryValues


class ItdsIntegration(Integration):
	identifier = "itds"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = itds._dict({"scheduled": False, "wait": 0})
		...
		if itds.form_dict.cmd:
			path = f"/api/method/{itds.form_dict.cmd}"
		else:
			path = itds.request.path
	...
	scope.set_user({"id": itds.local.site})
	user = getattr(itds.session, "user", "Unidentified")
	scope.set_tag("itds_user", user)
	# Extract `X-Itds-Request-ID` to store as a separate field if its present
	if trace_id := itds.monitor.get_trace_id():
		scope.set_tag("itds_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if itds.request:
				evt_processor = _make_wsgi_event_processor(itds.request.environ, False)
				scope.add_event_processor(evt_processor)
				if itds.request.is_json:
					scope.set_context("JSON Body", itds.request.json)
				elif itds.request.form:
					scope.set_context("Form Data", itds.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], itds.ValidationError | itds.PermissionError):
					return
		...
		itds.logger().error("Failed to capture exception", exc_info=True)
