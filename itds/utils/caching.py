...
def request_cache(func: Callable) -> Callable:
	...
	Cache is stored in `itds.local.request_cache`.
	...
	def wrapper(*args, **kwargs):
		_cache = getattr(itds.local, "request_cache", None)
		if _cache is None:
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	The cache is stored in `itds.utils.caching._SITE_CACHE`.
	...
	workers, use redis (itds.cache API) instead.
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			site = getattr(itds.local, "site", None)
			if not site:
				return func(*args, **kwargs)
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			itds.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			cached_val = itds.cache.get_value(func_call_key, user=user, shared=shared)
			...
			if itds.cache.exists(func_call_key, user=user, shared=shared):
				return None

			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			itds.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
	...
	*,
	...
	def outer(func: Callable) -> Callable:
		...
		def inner(*args, **kwargs):
			...
			if itds.request and itds.request.method == "GET" and qualified_name in itds.request.path:
				itds.local.response_headers.set("Cache-Control", cache_headers)
			return ret
...
def deprecated_local_cache(namespace, key, generator, regenerate_if_none=False):
	if namespace not in itds.local.cache:
		itds.local.cache[namespace] = {}

	if key not in itds.local.cache[namespace]:
		itds.local.cache[namespace][key] = generator()

	elif itds.local.cache[namespace][key] is None and regenerate_if_none:
		# if key exists but the previous result was None
		itds.local.cache[namespace][key] = generator()

	return itds.local.cache[namespace][key]
