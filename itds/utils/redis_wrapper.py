...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support itds.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		if user:
			if user is True:
				user = itds.local.session.get("user")

			key = f"user:{user}:{key}"

		return f"{itds.local.conf.get('db_name')}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False, *, use_local_cache=True):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in itds.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		local_cache = itds.local.cache
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(itds.local.cache) if regex.match(cstr(k))]
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		itds.local.cache.setdefault(_name, {})[key] = value
	...
	def hdel(
		...
		:param shared: shared itds key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in itds.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in itds.local.cache[_name]:
				del itds.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in itds.local.cache[_name]:
					del itds.local.cache[_name][key]
...
def setup_cache() -> RedisWrapper:
	if itds.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in itds.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=itds.conf.get("redis_cache_sentinel_username"),
			sentinel_password=itds.conf.get("redis_cache_sentinel_password"),
			master_username=itds.conf.get("redis_cache_master_username"),
			master_password=itds.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			itds.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(itds.conf.get("redis_cache"))
	...
	def _enable_client_tracking(self, conn):
		try:
			...
			if "client ID" in str(e) and "does not exist" in str(e):
				...
				itds.client_cache.healthy = False
...
class ClientCache:
	...
	This feature is internal to Itds Framework and is subjected to change without any notice.
	...
	Usage/Notes:
		...
		- Same keys can be accessed with `itds.cache` too, but that won't implement invalidation.
		...
		- Never use `itds.cache`'s request local cache along with client-side cache. Two
	...
	def get_doc(self, doctype: str, name: str | None = None):
		...
		key = itds.get_document_cache_key(doctype, name)
		return self.get_value(key, generator=lambda: itds.get_doc(doctype, name))
	...
	def erase_persistent_caches(self, *, doctype=None):
		...
		try:
			self.redis.publish(
				...
				json.dumps({"doctype": doctype, "site": itds.local.site}),
			)
		except redis.exceptions.ConnectionError:
			# Assume shashi isn't running
			pass
	...
	def _handle_persistent_cache_invalidation(self, message):
		...
		payload = itds._dict(json.loads(message["data"]))
