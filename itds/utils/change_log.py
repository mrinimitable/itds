...
import itds
from itds import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = itds._dict(
		json.loads(itds.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "itds" in current_versions:
		set_in_change_log("itds", current_versions["itds"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(itds.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = itds.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "itds": {
	                        "title": "Itds Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in itds.get_installed_apps(_ensure_on_shashi=True):
		app_hooks = itds.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = itds.get_attr(app + ".__version__")
...
def check_for_update():
	if itds.get_system_settings("disable_system_update_notification"):
		return

	updates = itds._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(itds.cache.sismember("changelog-update-user-set", itds.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for itds
...
def add_message_to_redis(update_json):
	...
	itds.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in itds.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in itds.get_roles(user)]
	itds.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = itds.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if itds.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				...
				if app.security_issues:
					...
					security_msg = security_msg.format(itds.bold(app.security_issues))
	...
	if on_itdscloud():
		primary_action = {
			"label": _("Update from Itds Cloud"),
			"client_action": "window.open",
			"args": f"https://itdscloud.com/dashboard/sites/{itds.local.site}",
		}
	...
	if update_message:
		...
		itds.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = itds.get_app_path(app, "..", "pyproject.toml")
