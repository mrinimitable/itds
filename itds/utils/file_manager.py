...
import itds
from itds import _, conf
from itds.query_builder.utils import DocType
from itds.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = itds.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = itds.form_dict.doctype
	dn = itds.form_dict.docname
	file_url = itds.form_dict.file_url
	filename = itds.form_dict.filename
	itds.form_dict.is_private = cint(itds.form_dict.is_private)

	if not filename and not file_url:
		itds.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = itds.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	itds.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = itds.form_dict.file_size
	...
	try:
		...
		return itds.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using itds.form
	if "filedata" in itds.form_dict:
		if "," in itds.form_dict.filedata:
			itds.form_dict.filedata = itds.form_dict.filedata.rsplit(",", 1)[1]
		itds.uploaded_content = safe_b64decode(itds.form_dict.filedata)
		itds.uploaded_filename = itds.form_dict.filename
		return itds.uploaded_filename, itds.uploaded_content
	else:
		itds.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in itds.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = itds.get_doc("File", name)
		return {k: b.get(k) for k in itds.get_hooks()["write_file_keys"]}
	return False
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in itds.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				itds.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = itds.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = itds.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = itds.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = itds.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = itds.utils.get_site_path("public", "files", parts[-1])

		else:
			path = itds.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = itds.get_all("File", {"file_name": fname}, pluck="name")
