...
def get_modules_from_all_apps_for_user(user: str | None = None) -> list[dict]:
	...
	global_blocked_modules = itds.get_doc("User", "Administrator").get_blocked_modules()
	user_blocked_modules = itds.get_doc("User", user).get_blocked_modules()
...
def get_modules_from_app(app):
	return itds.get_all("Module Def", filters={"app_name": app}, fields=["module_name", "app_name as app"])


def get_all_empty_tables_by_module():
	table_rows = itds.qb.Field("table_rows")
	table_name = itds.qb.Field("table_name")
	information_schema = itds.qb.Schema("information_schema")

	empty_tables = (
		itds.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = itds.get_all("DocType", fields=["name", "module"])
