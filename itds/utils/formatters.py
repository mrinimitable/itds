...
import itds
from itds.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if not df:
		...
		default_currency = itds.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return itds.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return itds.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = itds.get_meta(df.options)
		...
		if isinstance(value, str):
			return itds._(value, context=df.parent or "")
