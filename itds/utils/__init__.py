...
from itds.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(itds.local, "fullnames"):
		itds.local.fullnames = {}

	if not itds.local.fullnames.get(user):
		p = itds.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			itds.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			itds.local.fullnames[user] = user

	return itds.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return itds.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			itds.throw(
				error_message.format(itds.bold(phone_number), itds.bold(fieldname)),
				title=error_title,
				exc=itds.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		itds.throw(
			itds._("{0} is not a valid Phone Number").format(phone_number), itds.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in Itds doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		itds.throw(itds._("{0} is not a valid Name").format(name), itds.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = itds.utils.escape_html(e)
				itds.throw(
					itds._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(
	...
	if not is_valid and throw:
		itds.throw(itds._("'{0}' is not a valid URL").format(itds.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if itds.flags.in_import or itds.flags.in_install or itds.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(itds.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", itds._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	itds.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return itds.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return itds.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = itds.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return itds.local.site_path
...
def get_shashi_path():
	return os.environ.get("ITDS_SHASHI_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(itds.__file__), "..", "..", "..")
	)


def get_shashi_id():
	return itds.get_conf().get("shashi_id", get_shashi_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or itds.local.site}@{get_shashi_id()}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the Itds WSGI."""
	from itds.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = itds.get_hooks().get(hook_name)
	if method:
		method = itds.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in itds.get_hooks(hook):
		out = out or itds.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(itds.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in itds.get_installed_apps():
				include_path = itds.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(itds.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else itds.utils.md_to_html(text)
...
def get_site_info():
	from itds.email.queue import get_emails_sent_this_month
	from itds.utils.user import get_system_managers

	# only get system users
	users = itds.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", itds.STANDARD_USERS)},
	...
	system_settings = itds.db.get_singles_dict("System Settings")
	space_usage = itds._dict((itds.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": itds.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in itds.get_hooks("get_site_info"):
		site_info.update(itds.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return orjson.loads(itds.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                shashi --site okayblue.local execute itds.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = itds.db.count(doctype)

	return orjson.loads(itds.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Example:
	        via terminal:
	                shashi --site okayblue.local execute itds.utils.call --args '''["itds.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return orjson.loads(itds.as_json(itds.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = itds.as_unicode(filters)
...
def set_request(**kwargs):
	...
	itds.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return itds.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(itds.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return itds.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = itds.parse_json(itds.read_file("assets/assets.json"))

		if assets_rtl := itds.read_file("assets/assets-rtl.json"):
			assets.update(itds.parse_json(assets_rtl))
...
def get_shashi_relative_path(file_path):
	"""Fix paths relative to the shashi root directory if exists and return the absolute path.
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, itds._dict()).update(
...
class CallbackManager:
	...
	Example usage: itds.db.after_commit
	"""
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from itds.utils.safe_exec import safe_eval
...
def create_folder(path, with_init=False):
	...
	from itds.utils import touch_file
