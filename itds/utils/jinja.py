...
def get_jenv():
	...
	if jenv := getattr(itds.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	from itds.utils.safe_exec import UNSAFE_ATTRIBUTES

	UNSAFE_ATTRIBUTES = UNSAFE_ATTRIBUTES - {"format", "format_map"}

	class ItdsSandboxedEnvironment(SandboxedEnvironment):
	...
	# itds will be loaded last, so app templates will get precedence
	jenv = ItdsSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def validate_template(html):
	...
	try:
		...
		itds.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from itds import _, get_traceback, throw
...
def get_jloader():
	...
	itds.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	apps = itds.get_hooks("template_apps")
	if not apps:
		apps = list(reversed(itds.get_installed_apps(_ensure_on_shashi=True)))
...
def get_jinja_hooks():
	...
	if not getattr(itds.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = itds.get_module(obj_path)
			except ModuleNotFoundError:
				obj = itds.get_attr(obj_path)
