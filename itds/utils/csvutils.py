...
from itds.core.doctype.file.file import FILE_ENCODING_OPTIONS
from itds.utils import cint, comma_or, cstr, flt
...
def read_csv_content_from_attached_file(doc):
	...
	try:
		_file = itds.get_doc("File", fileid)
...
def read_csv_content(fcontent, use_sniffer: bool = False):
	...
	content = [itds.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	if use_sniffer:
		...
		try:
			...
			dialect = sniffer.sniff(
				sample="\n".join(sample_content), delimiters=itds.flags.delimiter_options
			)
		except csv.Error:
	...
	try:
		...
		itds.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	itds.response["result"] = cstr(to_csv(args.data))
	itds.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	itds.response["result"] = cstr(to_csv(data))
	itds.response["doctype"] = filename
	itds.response["type"] = "csv"
...
def check_record(d):
	...
	from itds.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				itds.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and itds.db.exists(doctype, d["name"]):
		if overwrite:
			doc = itds.get_doc(doctype, d["name"])
