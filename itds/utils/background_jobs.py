...
import itds.monitor
from itds import _
from itds.utils import CallbackManager, cint, get_shashi_id, get_sites
from itds.utils.caching import site_cache
from itds.utils.commands import log
from itds.utils.data import sbool
from itds.utils.redis_queue import RedisQueue
...
MAX_QUEUED_JOBS = 500  # itds.enqueue will start failing when these many jobs exist in queue.
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = itds.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	duplicate calls
	:param now: if now=True, the method is executed via itds.call()
	...
	if deduplicate:
		if not job_id:
			itds.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status(refresh=False) in (JobStatus.QUEUED, JobStatus.STARTED):
			itds.logger().error(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not itds.in_test:
		from itds.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not itds.in_test)
	if call_directly:
		return itds.call(method, **kwargs)
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=itds.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=itds.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		itds.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	"""
	return enqueue(
		"itds.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(itds.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in itds.get_hooks("before_job"):
		itds.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (itds.db.InternalError, itds.RetryBackgroundJobError) as e:
		itds.db.rollback(chain=True)

		if retry < 5 and (
			isinstance(e, itds.RetryBackgroundJobError)
			or (itds.db.is_deadlocked(e) or itds.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			# 1205 = lock wait timeout
			# or RetryBackgroundJobError is explicitly raised
			itds.job.after_job.reset()
			itds.destroy()
			time.sleep(retry + 1)
			...
			itds.log_error(title=method_name)
			raise

	except Exception as e:
		itds.db.rollback(chain=True)
		itds.log_error(title=method_name)
		itds.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		...
		if not hasattr(itds.local, "site"):
			itds.init(site, force=True)
			itds.connect()
		for after_job_task in itds.get_hooks("after_job"):
			itds.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		itds.local.job.after_job.run()
...
class ItdsWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_itds_scheduler()
	...
	def start_itds_scheduler(self):
		from itds.utils.scheduler import start_scheduler

		Thread(target=start_scheduler, daemon=True).start()


class ItdsWorkerNoFork(ItdsWorker):
...
def start_worker_pool(
	...
	import itds.database.query  # sqlparse and indirect imports
	import itds.query_builder  # pypika
	import itds.utils  # common utils
	import itds.utils.safe_exec
	import itds.utils.scheduler
	...
	no_fork = sbool(os.environ.get("ITDS_BACKGROUND_WORKERS_NOFORK", False))

	worker_klass = ItdsWorkerNoFork if no_fork else ItdsWorker
	pool = WorkerPool(
		queues=queues,
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		itds.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(itds.local, "conf"):
		raise Exception("You need to call itds.init")
	...
	try:
		...
		log(
			f"Please make sure that Redis Queue runs @ {itds.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def enqueue_test_job():
	enqueue("itds.utils.background_jobs.test_job", s=100)
...
def create_job_id(job_id: str | None = None) -> str:
	...
	return f"{itds.local.site}||{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from itds.utils import create_batch

	conf = itds.conf if itds.conf else itds.get_conf(site=job.kwargs.get("site"))
...
def _check_queue_size(q: Queue):
	max_jobs = cint(itds.conf.max_queued_jobs) or MAX_QUEUED_JOBS
	...
	if cint(q.count) >= max_jobs:
		...
		itds.throw(
			...
			exc=itds.QueueOverloaded,
			primary_action=primary_action if itds.has_permission("System Health Report") else None,
		)
...
def _start_sentry():
	sentry_dsn = os.getenv("ITDS_SENTRY_DSN")
	if not sentry_dsn:
		return
