...
import itds
from itds.utils.data import cint, cstr
...
def get_print(
	...
	from itds.website.serve import get_response_without_exception_handling
	...
	if "pdf_generator" not in local.form_dict:
		...
		if pdf_generator is None:
			pdf_generator = (
				itds.get_cached_value("Print Format", print_format, "pdf_generator") or "wkhtmltopdf"
			)
	...
	if local.form_dict.pdf_generator != "wkhtmltopdf":
		hook_func = itds.get_hooks("pdf_generator")
...
def attach_print(
	...
	from itds.utils import scrub_urls
	from itds.utils.pdf import get_pdf
	...
	itds.local.flags.ignore_print_permissions = True

	with print_language(lang or itds.local.lang):
		content = ""
	...
	itds.local.flags.ignore_print_permissions = False
