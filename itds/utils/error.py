...
EXCLUDE_EXCEPTIONS = (
	...
	itds.CSRFTokenError,  # CSRF covers OAuth too
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from itds.utils.sentry import capture_exception
	...
	traceback = itds.as_unicode(traceback or itds.get_traceback(with_context=True))
	...
	if itds.flags.read_only or defer_insert:
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				itds.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For itds.throws the exception source is possibly present after skipping itds.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "itds":
