...
from itds.boot import get_allowed_reports
from itds.core.doctype.domain_settings.domain_settings import get_active_modules
from itds.permissions import AUTOMATIC_ROLES, get_roles, get_valid_perms
from itds.query_builder import DocType, Order
...
class UserPermissions:
	"""
	A user permission object can be accessed as `itds.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or itds.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = itds.get_doc("User", self.name).as_dict()
			except itds.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not itds.db.is_table_missing(e):
					raise

			return user

		if not itds.flags.in_install_db and not itds.in_test:
			user_doc = itds.cache.hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = itds.get_doc(user_doc)
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in itds.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = itds.share.get_shared_doctypes()
		no_list_view_link = []
		...
		if "System Manager" in self.get_roles():
			self.can_import += itds.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += itds.get_all(
				"Property Setter",
				pluck="doc_type",
	...
	def get_defaults(self):
		...
		self.defaults = itds.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		if d.get("default_workspace"):
			workspace = itds.get_cached_doc("Workspace", d.default_workspace)
			d.default_workspace = {
				"name": workspace.name,
		...
		d.name = self.name
		d.onboarding_status = itds.parse_json(d.onboarding_status)
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = itds.get_cached_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(itds.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	itds.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return itds.get_cached_value("User", username or itds.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	...
	return itds.db.get_value(
		"User",
		{
			"email": username or itds.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
		cache=True,
