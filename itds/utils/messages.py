...
from itds.utils.data import safe_decode
...
def msgprint(
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			if inspect.isclass(raise_exception) and issubclass(raise_exception, Exception):
				...
				exc = itds.ValidationError(msg)
			if out.__itds_exc_id:
				exc.__itds_exc_id = out.__itds_exc_id
			raise exc

	if itds.flags.mute_messages:
		_raise_exception()
		return
	...
	if itds.flags.print_messages and out.message:
	...
	if raise_exception:
		...
		out.__itds_exc_id = itds.generate_hash()
	...
	if realtime:
		itds.publish_realtime(event="msgprint", message=out)
	else:
		itds.message_log.append(out)
	_raise_exception()
...
def get_message_log() -> list[dict]:
	return [msg_out for msg_out in itds.local.message_log]


def clear_last_message():
	if len(itds.local.message_log) > 0:
		itds.local.message_log = itds.local.message_log[:-1]


def throw(
	msg: str,
	exc: type[Exception] | Exception = itds.ValidationError,
	title: str | None = None,
	is_minimizable: bool = False,
	wide: bool = False,
	as_list: bool = False,
	...
	:param msg: Message.
	:param exc: Exception class. Default `itds.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def throw_permission_error():
	throw(_("Not permitted"), itds.PermissionError)
