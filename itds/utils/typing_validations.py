...
import itds
from itds.exceptions import ItdsTypeError
...
ItdsPydanticConfig = ConfigDict(arbitrary_types_allowed=True)
...
def TypeAdapter(type_):
	try:
		return PydanticTypeAdapter(type_, config=ItdsPydanticConfig)
...
def transform_parameter_types(func: Callable, args: tuple, kwargs: dict):
	...
	func_params = itds._get_cached_signature_params(func)[0]
