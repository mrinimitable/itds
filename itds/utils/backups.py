...
from itds.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		site = itds.local.site or itds.generate_hash(length=8)
	...
	def _set_existing_tables(self):
		...
		if not hasattr(self, "_existing_tables"):
			self._existing_tables = itds.db.get_tables()
	...
	def set_backup_tables_from_config(self):
		...
		backup_conf = itds.conf.get("backup", {})
		self._set_existing_tables()
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file and site_config_backup_path):
			...
			if itds.get_system_settings("encrypt_backup"):
	...
	def set_backup_file_name(self):
		...
		enc = "-enc" if itds.get_system_settings("encrypt_backup") else ""
		...
		for path in paths:
			if os.path.exists(path):
				...
				try:
					...
					itds.utils.execute_in_shell(command)
	...
	def print_summary(self):
		...
		print(f"Backup Summary for {itds.local.site} at {now()}")
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = itds.get_site_path(folder, "files")
			...
			try:
				itds.utils.execute_in_shell(
					cmd_string.format(backup_path, files_path),
					verbose=self.verbose,
					low_priority=True,
					check_exit_code=True,
				)
			except itds.CommandFailedError as e:
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(itds.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		if self.db_type == "sqlite":
			...
			db_path = Path(itds.get_site_path()) / "db" / f"{self.db_name}.db"
			command = f"gzip -k {db_path} -c > {self.backup_path_db}"

			itds.utils.execute_in_shell(command, low_priority=True, check_exit_code=True)
		...
		if not gzip_exc:
			itds.throw(
				_("gzip not found in PATH! This is required to take a backup."), exc=itds.ExecutableNotFound
			)

		if self.old_backup_metadata:
			database_header_content = [
				f"Backup generated by Itds {itds.__version__} on branch {get_app_branch('itds') or 'N/A'}",
				"",
			]
		else:
			database_header_content = [
				"begin itds metadata",
				"[itds]",
				f"version = {itds.__version__}",
				f"branch = {get_app_branch('itds') or 'N/A'}",
				"end itds metadata",
				"",
			]
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of Itds Site {itds.local.site}",
	...
	def send_email(self):
		"""
		Sends the link to backup file located at okayblue/backups
		"""
		from itds.email import get_system_managers
...
download only after 24 hours."""
		...
		itds.sendmail(recipients=recipient_list, message=msg, subject=subject)
...
def _get_tables(doctypes: list[str], existing_tables: list[str]) -> list[str]:
	...
	for doctype in doctypes:
		if not doctype:
			continue
		table = itds.utils.get_table_name(doctype)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(itds.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	return itds.utils.get_site_path(conf.get("backup_path", "private/backups"))
...
def get_or_generate_backup_encryption_key():
	...
	key = itds.conf.get(BACKUP_ENCRYPTION_CONFIG_KEY)
	if key:
		return key
