...
from itds.utils import cint, get_test_client, get_url

try:
	_site = itds.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("itds.app.get_site_name", return_value=self.site):
				header_patch = patch("itds.get_request_header", new=patch_request_header)
...
class ItdsAPITestCase(IntegrationTestCase):
...
class TestResourceAPI(ItdsAPITestCase):
	DOCTYPE = "ToDo"
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = itds.get_doc({"doctype": "ToDo", "description": itds.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		...
		for name in cls.GENERATED_DOCUMENTS:
			itds.delete_doc_if_exists(cls.DOCTYPE, name)
		itds.db.commit()
	...
	def test_create_document(self):
		data = {"description": itds.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = itds.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "itds.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(ItdsAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/itds.realtime.get_user_info
		response = self.get(self.method("itds.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("itds.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "itds.tests.test_api.test"

		def get_message(resp, msg_type):
			return itds.parse_json(itds.parse_json(itds.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "itds.tests.test_api.test_array"
...
class TestReadOnlyMode(ItdsAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": itds.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(ItdsAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["itds.tests.test_api.before_request"],
				"after_request": ["itds.tests.test_api.after_request"],
			}
		):
...
class TestResponse(ItdsAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/itds.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/itds.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from itds.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = itds.generate_hash()
		file = itds.get_doc(
			{
				"doctype": "File",
