import itds
from itds.boot import get_user_pages_or_reports
from itds.desk.doctype.note.note import _get_unseen_notes, get_unseen_notes, mark_as_seen
...
class TestPermissionQueries(IntegrationTestCase):
	...
	def test_get_user_pages_or_reports_with_permission_query(self):
		...
		itds.get_doc(
			...
			script="""conditions = f"(`tabReport`.is_standard = 'Yes' or `tabReport`.owner = '{itds.session.user}')"
				""",
		).insert()
		...
		allowed_reports = itds.cache.get_value("has_role:Report", user=itds.session.user)
