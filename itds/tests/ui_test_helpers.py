...
UI_TEST_USER = "itds@example.com"


def whitelist_for_tests(fn):
	if itds.request and not itds.in_test and not itds._dev_server:
		itds.throw("Cannot run UI tests. Use a development server with `shashi start`")

	return itds.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = itds.db.exists(doc.doctype, filters)
		if not name:
			d = itds.get_doc(doc)
...
def prepare_webform_test():
	for note in itds.get_all("Note", pluck="name"):
		itds.delete_doc("Note", note, force=True)

	itds.delete_doc_if_exists("Web Form", "note")
	...
	doc = itds.get_doc(
		{
			...
			"communication_date": itds.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from itds.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if itds.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = itds.new_doc("Contact")
...
def create_contact_records():
	if itds.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if itds.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	itds.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = itds.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if itds.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return
...
def create_web_page(title, route, single_thread):
	web_page = itds.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = itds.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": itds.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not itds.db.exists("Web Form", "update-profile-duplicate"):
		doc = itds.get_doc("Web Form", "edit-profile")
		_doc = itds.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if itds.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		itds.get_doc(doc).insert(ignore_if_duplicate=True)


@whitelist_for_tests
def create_blog_post():
	blog_category = itds.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = itds.get_doc(
		{
...
def create_test_user(username=None):
	...
	if itds.db.exists("User", name):
		return

	user = itds.new_doc("User")
	user.email = name
	user.first_name = "Itds"
	user.new_password = itds.local.conf.admin_password
	...
	all_roles = set(itds.get_all("Role", pluck="name"))
	...
	itds.db.set_single_value("Workspace Settings", "workspace_setup_completed", 1)


@whitelist_for_tests
def setup_tree_doctype():
	itds.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not itds.db.exists("Custom Tree", "All Trees"):
		itds.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	itds.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
@whitelist_for_tests
def setup_inbox():
	itds.db.delete("User Email")
	doc = itds.new_doc("Email Account")
...
def setup_default_view(view, force_reroute=None):
	itds.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	itds.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not itds.db.exists("Custom Field", "Note-kanban"):
		itds.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not itds.db.exists("Kanban Board", "_Note _Kanban"):
		itds.get_doc(
			{
...
def create_todo(description):
	return itds.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not itds.db.exists("Kanban Board", "Admin Kanban"):
		itds.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	itds.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		itds.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
