...
from itds.utils import global_search, now_datetime
...
class TestGlobalSearch(IntegrationTestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in itds.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		itds.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = itds.get_doc("Event", itds.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = itds.get_all("Event")[0].name
		event = itds.get_doc("Event", event_name)
		...
		itds.delete_doc("Event", event_name)
		global_search.sync_global_search()
		itds.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = itds.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
