...
import itds
from itds import get_hooks
from itds.tests import IntegrationTestCase
from itds.utils import set_request
from itds.website.page_renderers.static_page import StaticPage
from itds.website.serve import get_response, get_response_content
from itds.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(IntegrationTestCase):
	...
	def test_home_page(self):
		...
		role = itds.get_doc(doctype="Role", role_name="home-page-test", desk_access=0).insert(
		...
		itds.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		itds.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		itds.set_user("Administrator")
		itds.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		itds.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		itds.set_user("test-user-for-home-page@example.com")
		itds.cache.hdel("home_page", itds.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		itds.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		itds.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		itds.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["itds.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(itds, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(itds, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			itds, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_page_load(self):
		...
		html = itds.safe_decode(response.get_data())
	...
	def test_redirect(self):
		...
		delattr(itds.hooks, "website_redirects")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["itds.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(itds, "get_hooks", patched_get_hooks):
	...
	def test_json_sidebar_data(self):
		itds.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		itds.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		itds.flags.force_website_cache = True
		...
		itds.flags.force_website_cache = False
	...
	def test_never_render(self):
		...
		WWW = Path(itds.get_app_path("itds")) / "www"
	...
	def test_resolve_class(self):
		from itds.utils.jinja_globals import resolve_class

		context = itds._dict(primary=True)
		...
		self.assertEqual(
			itds.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(itds, "get_hooks", patched_get_hooks):
			itds.set_user("Administrator")
			itds.hooks.app_include_js.append("test_app_include.js")
			itds.hooks.app_include_css.append("test_app_include.css")
			itds.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			itds.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
