...
from itds.utils.caching import redis_cache, request_cache, site_cache
...
@site_cache
def ping() -> str:
	register_with_external_service(itds.local.site)
	return itds.local.site
...
class TestSiteCache(ItdsAPITestCase):
...
class TestRedisCache(ItdsAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if itds.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestDocumentCache(ItdsAPITestCase):
	TEST_DOCTYPE = "User"
	TEST_DOCNAME = "Administrator"
	TEST_FIELD = "middle_name"

	def setUp(self) -> None:
		self.test_value = itds.generate_hash()

	def test_caching(self):
		itds.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		with self.assertQueryCount(0):
			doc = itds.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		doc.db_set(self.TEST_FIELD, self.test_value)
		new_doc = itds.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		self.assertIsNot(doc, new_doc)  # Shouldn't be same object from itds.local
		self.assertEqual(new_doc.get(self.TEST_FIELD), self.test_value)  # Cache invalidated and fetched
		itds.db.rollback()

		doc_after_rollback = itds.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
...
class TestRedisWrapper(ItdsAPITestCase):
	def test_delete_keys(self):
		prefix = "test_del_"

		for i in range(5):
			itds.cache.set_value(f"{prefix}{i}", 1)

		self.assertEqual(len(itds.cache.get_keys(prefix)), 5)
		itds.cache.delete_keys(prefix)
		self.assertEqual(len(itds.cache.get_keys(prefix)), 0)

	def test_hash(self):
		key = "test_hash"

		# Confirm that there's no data initially
		exists = itds.cache.exists(key)
		self.assertFalse(exists)

		# Insert 5 key-value pairs
		for i in range(5):
			itds.cache.hset(key, f"key_{i}", f"value_{i}")

		# Check that we have 5 values
		values = itds.cache.hgetall(key)
		self.assertEqual(len(values), 5)

		# Check that each value matches
		for i in range(5):
			value = itds.cache.hget(key, f"key_{i}")
			self.assertEqual(value, f"value_{i}")

		# Check the keys themselves
		keys = itds.cache.hkeys(key)
		for i in range(5):
			self.assertIn(f"key_{i}".encode(), keys)

		# Delete a single key and check that we still have the remaining 4
		itds.cache.hdel(key, "key_1")
		values = itds.cache.hgetall(key)
		self.assertEqual(len(values), 4)

		# Delete 2 keys and check that we still have the remaining 2
		itds.cache.hdel(key, ["key_2", "key_3"])
		values = itds.cache.hgetall(key)
	...
	def test_user_cache_clear(self):
		from itds.cache_manager import user_cache_keys

		# Set some keys that a user's cache would usually have
		user1 = itds.utils.random_string(10)
		user2 = itds.utils.random_string(10)
		for key in user_cache_keys:
			itds.cache.hset(key, user1, key)
			itds.cache.hset(key, user2, key)

		itds.clear_cache(user=user1)

		# Check that the keys for user1 are gone
		for key in set(user_cache_keys) - {"home_page"}:
			self.assertFalse(itds.cache.hexists(key, user1))
			self.assertTrue(itds.cache.hexists(key, user2))

	def test_doctype_cache_clear(self):
		from itds.cache_manager import doctype_cache_keys

		# Set some keys that a user's cache would usually have
		doctype1 = new_doctype(itds.utils.random_string(10))
		doctype2 = new_doctype(itds.utils.random_string(10))
		for key in doctype_cache_keys:
			itds.cache.hset(key, doctype1.name, key)
			itds.cache.hset(key, doctype2.name, key)

		itds.clear_cache(doctype=doctype1.name)

		# Check that the keys for doctype1 are gone
		for key in doctype_cache_keys:
			self.assertFalse(itds.cache.hexists(key, doctype1.name))
			self.assertTrue(itds.cache.hexists(key, doctype2.name))

	def test_backward_compat_cache(self):
		self.assertEqual(itds.cache, itds.cache())


class TestHttpCache(ItdsAPITestCase):
	def test_http_headers(self):
		resp = self.get(
			self.method("itds.client.is_document_amended"),
