...
import itds
from itds.integrations.oauth2 import encode_params
from itds.tests import IntegrationTestCase
from itds.tests.test_api import get_test_client, make_request, suppress_stdout
from itds.tests.utils import make_test_records

if TYPE_CHECKING:
	from itds.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class ItdsRequestTestCase(IntegrationTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			itds.local.login_manager.login_as("test@example.com")
			self._sid = itds.session.sid
...
class TestOAuth20(ItdsRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Itds server URL reqired for id_token generation
		itds_login_key: SocialLoginKey = itds.new_doc("Social Login Key")
		itds_login_key.get_social_login_provider("Itds", initialize=True)
		itds_login_key.base_url = itds.utils.get_url()
		itds_login_key.enable_social_login = 0
		itds_login_key.insert(ignore_if_duplicate=True)
		itds.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = itds.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/itds.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/itds.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = itds.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/itds.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = itds.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/itds.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "ItdsRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/itds.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(itds.utils.get_url(), endpoint)
