...
import itds
from itds.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from itds.utils import cint, set_request


class TestTwoFactor(IntegrationTestCase):
	def setUp(self):
		self.http_requests = create_http_request()
		self.login_manager = itds.local.login_manager
	...
	def tearDown(self):
		itds.local.response["verification"] = None
		itds.local.response["tmp_id"] = None
		disable_2fa()
		itds.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in itds.local."""
		authenticate_for_2factor(self.user)
		verification_obj = itds.local.response["verification"]
		tmp_id = itds.local.response["tmp_id"]
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(itds.cache.get(f"{tmp_id}{k}"), f"{k} not available")
	...
	def test_two_factor_is_enabled(self):
		...
		# Scenario 3
		enable_2fa()
		user = itds.get_doc("User", self.user)
		user.restrict_ip = itds.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		itds.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = itds.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(itds.AuthenticationError):
	...
	def test_render_string_template(self):
		...
		args = {"issuer_name": "Itds Technologies"}
		_str = "Verification Code from {{issuer_name}}"
		_str = itds.render_template(_str, args)
		self.assertEqual(_str, "Verification Code from Itds Technologies")
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(itds.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	itds.form_dict["usr"] = "test@example.com"
	itds.form_dict["pwd"] = "Eastern_43A1W"
	itds.local.form_dict["cmd"] = "login"
	return HTTPRequest()


def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	"""Enable Two factor in system settings."""
	system_settings = itds.get_doc("System Settings")
...
def toggle_2fa_all_role(state=None):
	...
	all_role = itds.get_doc("Role", "All")
