...
import itds
from itds.app import make_form_dict
...
from itds.model.document import LazyChildTable
from itds.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from itds.tests import IntegrationTestCase
from itds.utils import cint, now_datetime, set_request
...
class TestDocument(IntegrationTestCase):
	...
	def test_load_single(self):
		d = itds.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(itds.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		...
		doc = itds.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(itds.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(itds.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(itds.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		from itds.desk.doctype.event.event import Event
		...
		self.assertRaises(itds.ValidationError, d3.discard)
	...
	def test_mandatory(self):
		...
		itds.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(itds.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(itds.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			itds.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except itds.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = itds.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(itds.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = itds.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = itds.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		itds.set_user("Guest")
		self.assertRaises(itds.PermissionError, self.test_insert)
		itds.set_user("Administrator")

	def test_permission_single(self):
		itds.set_user("Guest")
		d = itds.get_doc("Website Settings", "Website Settings")
		self.assertRaises(itds.PermissionError, d.save)
		itds.set_user("Administrator")

	def test_link_validation(self):
		itds.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(itds.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(itds.ValidationError, d.validate)
		self.assertRaises(itds.ValidationError, d.run_method, "validate")
		self.assertRaises(itds.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = itds.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(itds.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(itds.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = itds.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(itds.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		itds.delete_doc_if_exists("Currency", "Itds Coin", 1)

		d = itds.get_doc(
			{"doctype": "Currency", "currency_name": "Itds Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(itds.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(itds.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		itds.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = itds.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_virtual_fields(self):
		...
		itds.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		...
		def patch_note(class_=None):
			return patch("itds.controllers", new={itds.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"itds.utils.now_datetime() - itds.utils.get_datetime(doc.creation)"
				if with_options
				else ""
			)
	...
	def test_doc_events(self):
		...
		for doctype, doc_hooks in itds.get_doc_hooks().items():
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = itds.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise itds.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(itds.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			itds.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)

	def test_db_set_singles(self):
		c = itds.get_doc("Contact Us Settings")
		key, val = "email_id", "admin1@example.com"
		c.db_set(key, val)
		changed_val = itds.db.get_single_value(c.doctype, key)
...
class TestDocumentWebView(IntegrationTestCase):
	def get(self, path, user="Guest"):
		...
		make_form_dict(itds.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = itds.get_doc("Document Share Key", {"key": document_key})
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from itds.modules.utils import get_doc_path, scrub

		itds.flags.allow_doctype_export = True

		itds.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
	...
	def test_bulk_inserts(self):
		from itds.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = itds.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = itds.generate_hash()
		...
		all_docs = set(itds.get_all(doctype, pluck="name"))
		all_child_docs = set(
			itds.get_all(
...
class TestLazyDocument(IntegrationTestCase):
	def test_lazy_documents(self):
		# Warmup meta etc
		_ = itds.get_lazy_doc("User", "Guest")
		eager_guest: User = itds.get_doc("User", "Guest")

		# Only one query for parent document
		with self.assertQueryCount(1):
			guest: User = itds.get_lazy_doc("User", "Guest")
		...
		self.assertTrue(itds.get_lazy_doc("User", "Guest").get("roles"))

	def test_lazy_doc_efficient_saves(self):
		# Only touched tables and self should be updated
		guest = itds.get_lazy_doc("User", "Guest")
		with self.assertQueryCount(1):
			guest.db_update_all()
	...
	def test_for_update(self):
		guest = itds.get_lazy_doc("User", "Guest", for_update=True)
