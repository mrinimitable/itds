...
from itds.utils import get_shashi_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	itds.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(itds.cache.hget(USER_TRANSLATION_KEY, itds.local.lang))
		self.assertIsNotNone(itds.cache.hget(MERGED_TRANSLATION_KEY, itds.local.lang))

		clear_cache()

		self.assertIsNone(itds.cache.hget(USER_TRANSLATION_KEY, itds.local.lang))
		self.assertIsNone(itds.cache.hget(MERGED_TRANSLATION_KEY, itds.local.lang))

	def test_extract_message_from_file(self):
		data = itds.translate.get_messages_from_file(translation_string_file)
		shashi_path = get_shashi_path()
		file_path = itds.get_app_path("itds", "tests", "translation_test_file.txt")
	...
	def test_translation_with_context(self):
		t1 = itds.new_doc("Translation")
		t1.language = "fr"
		t1.source_text = "Change"
		t1.translated_text = "Changement"
		t1.save()

		t2 = itds.new_doc("Translation")
		t2.language = "fr"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for itds.translate.get_language

		Case 1: itds.form_dict._lang is set
		"""

		itds.form_dict._lang = first_lang

		with patch.object(itds.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: itds.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(itds.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: itds.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: itds.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: itds.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			itds._("attr")
			_("name")
			itds._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(itds.get_app_path(app)) / "translations"
