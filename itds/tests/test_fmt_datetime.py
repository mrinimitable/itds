...
class TestFmtDatetime(IntegrationTestCase):
	...
	def setUp(self):
		# create test domain
		self.pre_test_date_format = itds.db.get_default("date_format")
		self.pre_test_time_format = itds.db.get_default("time_format")

	def tearDown(self):
		itds.db.set_default("date_format", self.pre_test_date_format)
		itds.db.set_default("time_format", self.pre_test_time_format)
		itds.local.user_date_format = None
		itds.local.user_time_format = None
		itds.db.rollback()

	# Test utility functions

	def test_set_default_date_format(self):
		itds.db.set_default("date_format", "ZYX321")
		self.assertEqual(itds.db.get_default("date_format"), "ZYX321")

	def test_set_default_time_format(self):
		itds.db.set_default("time_format", "XYZ123")
		self.assertEqual(itds.db.get_default("time_format"), "XYZ123")
	...
	def test_format_date(self):
		...
		for fmt, valid_fmt in test_date_formats.items():
			itds.db.set_default("date_format", fmt)
			itds.local.user_date_format = None
	...
	def test_format_time(self):
		...
		for fmt, valid_fmt in test_time_formats.items():
			itds.db.set_default("time_format", fmt)
			itds.local.user_time_format = None
	...
	def test_format_datetime(self):
		...
		for date_fmt, valid_date in test_date_formats.items():
			itds.db.set_default("date_format", date_fmt)
			itds.local.user_date_format = None
			for time_fmt, valid_time in test_time_formats.items():
				itds.db.set_default("time_format", time_fmt)
				itds.local.user_time_format = None
