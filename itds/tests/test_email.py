...
from itds.email.doctype.email_account.test_email_account import TestEmailAccount
from itds.email.doctype.email_queue.email_queue import QueueBuilder
...
class TestEmail(IntegrationTestCase):
	...
	def test_send_after(self):
		...
		from itds.email.queue import flush

		flush()
		email_queue = itds.db.sql("""select name from `tabEmail Queue` where status='Sent'""", as_dict=1)
	...
	def test_flush(self):
		...
		self.assertTrue("test1@example.com" in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue("Unsubscribe" in itds.safe_decode(itds.flags.sent_mail))
	...
	def test_cc_footer(self):
		...
		self.assertTrue(
			...
			in itds.safe_decode(itds.flags.sent_mail)
		)

	def test_expose(self):
		from itds.utils import set_request
		from itds.utils.verified_command import verify_request
		...
		email_obj = email.message_from_string(itds.safe_decode(itds.flags.sent_mail))
		for part in email_obj.walk():
			...
			if content:
				...
				query_string = re.search(
					r"(?<=/api/method/itds.email.queue.unsubscribe\?).*(?=" + eol + ")", content.decode()
				).group(0)
	...
	def test_sender(self):
		def _patched_assertion(email_account, assertion):
			with patch.object(QueueBuilder, "get_outgoing_email_account", return_value=email_account):
				...
				email_queue_sender = itds.db.get_value("Email Queue", {"status": "Sent"}, "sender")
	...
	def test_unsubscribe(self):
		from itds.email.queue import unsubscribe
		...
		email_queue = itds.db.get_value("Email Queue", {"status": "Not Sent"})
		queue_recipients = [
			r.recipient
			for r in itds.db.sql(
				"""select recipient from `tabEmail Queue Recipient`
		...
		itds.get_doc("Email Queue", email_queue).send()
		self.assertTrue("Unsubscribe" in itds.safe_decode(itds.flags.sent_mail))
	...
	def test_image_parsing(self):
		...
		itds.db.delete("Communication", {"sender": "sukh@yyy.com"})

		with open(itds.get_app_path("itds", "tests", "data", "email_with_image.txt")) as raw:
			messages = {
...
class TestEmailIntegrationTest(IntegrationTestCase):
	...
	def setUp(self) -> None:
		# Itds code is configured to not attempting sending emails during test.
		itds.flags.testing_email = True
		requests.delete(f"{self.SMTP4DEV_WEB}/api/Messages/*")
		return super().setUp()

	def tearDown(self) -> None:
		itds.flags.testing_email = False
	...
	def test_store_attachments(self):
		...
		communication = itds.get_doc("Communication", name)

		attachments = get_attachments(communication.doctype, communication.name)
		self.assertEqual(len(attachments), 1)

		file = itds.get_doc("File", attachments[0].name)
