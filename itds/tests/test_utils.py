...
import itds
from itds.installer import parse_app_name
from itds.model.document import Document
from itds.tests import IntegrationTestCase, MockedRequestTestCase
...
from itds.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from itds.utils.data import (
	add_to_date,
	add_trackers_to_url,
	add_years,
	cast,
...
from itds.utils.dateutils import get_dates_from_timegrain
from itds.utils.diff import _get_value_from_version, get_version_diff, version_query
from itds.utils.identicon import Identicon
from itds.utils.image import optimize_image, strip_exif_data
from itds.utils.make_random import can_make, get_random, how_many
from itds.utils.response import json_handler
from itds.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/itds/test.jpg">
			</div>
			<div style="background-image: url('/assets/itds/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/itds/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/itds/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from itds.utils.html_utils import clean_email_html
...
class TestValidationUtils(IntegrationTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://itds.io", throw=True))
		...
		self.assertRaises(itds.ValidationError, validate_url, "itds", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://itds.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://itds.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@itds.com"))
		self.assertTrue(validate_email_address("someone@itds.com, anyone@itds.io"))
		self.assertTrue(validate_email_address("test%201@itds.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@itds.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(
			itds.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)

		self.assertEqual(validate_email_address("Some%20One@itds.com"), "Some%20One@itds.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@itds.com"),
			"erp+Job%20Applicant=JA00004@itds.com",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(itds.InvalidNameError, validate_name, name, True)


class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(itds.get_app_path("itds", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			itds.get_app_path("itds", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
	...
	def test_optimize_image(self):
		image_file_path = itds.get_app_path("itds", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(itds.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = itds.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(itds.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				itds.utils.get_first_day_of_week("2020-12-25"),
				itds.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				itds.utils.get_first_day_of_week("2020-12-20"),
				itds.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			itds.utils.get_first_day_of_week("2020-12-25"),
			itds.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			itds.utils.get_first_day_of_week("2020-12-21"),
			itds.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			itds.utils.get_last_day_of_week("2020-12-24"),
			itds.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			itds.utils.get_last_day_of_week("2020-12-28"),
			itds.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(itds.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(itds.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from itds.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from itds.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = itds.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/itds/healthcare",
			status=200,
			json={},
		)
		shashi_path = get_shashi_path()
		itds_app = os.path.join(shashi_path, "apps", "itds")
		self.assertEqual("itds", parse_app_name(itds_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/mrinimitable/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:itds/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("itds/healthcare@develop"))
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(itds.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(itds.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(itds.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(itds.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(IntegrationTestCase):
	def test_lazy_import_module(self):
		from itds.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("itds.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `itds.tests.data.load_sleep` loaded"], output)


class TestIdenticon(IntegrationTestCase):
	def test_get_gravatar(self):
		# developers@itds.io has a gravatar linked so str URL will be returned
		toggle_test_mode(False)
		gravatar_url = get_gravatar("developers@itds.io")
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@itds.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(itds.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("itds", installed_apps)
...
class TestTypingValidations(IntegrationTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(itds.ItdsTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = itds.get_last_doc("Report")
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = itds._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(itds.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(IntegrationTestCase):
	def test_validate_argument_types(self):
		...
		from itds.core.doctype.doctype.doctype import DocType
		from itds.utils.typing_validations import (
			ItdsTypeError,
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(ItdsTypeError):
		...
		doctype = itds.get_last_doc("DocType")
...
class TestChangeLog(IntegrationTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("itds"), str)

	def test_parse_github_url(self):
		# using okayblue as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/mrinimitable/okayblue.git")
		self.assertEqual(owner, "itds")
		self.assertEqual(repo, "okayblue")

		owner, repo = parse_github_url("https://github.com/mrinimitable/okayblue")
		self.assertEqual(owner, "itds")
		self.assertEqual(repo, "okayblue")

		owner, repo = parse_github_url("git@github.com:itds/okayblue.git")
		self.assertEqual(owner, "itds")
		self.assertEqual(repo, "okayblue")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("itds.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": itds.get_doc("UTM Source", "test_source"),
			"utm_medium": itds.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": itds.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
