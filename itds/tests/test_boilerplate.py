...
import itds
from itds.modules.patch_handler import get_all_patches, parse_as_configfile
from itds.utils.boilerplate import (
	PatchCreator,
...
class TestBoilerPlate(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		cls.shashi_path = itds.utils.get_shashi_path()
		cls.apps_dir = os.path.join(cls.shashi_path, "apps")
	...
	def delete_test_app(cls, app_name):
		test_app_dir = os.path.join(cls.shashi_path, "apps", app_name)
	...
	def test_create_app(self):
		...
		new_app_dir = os.path.join(self.shashi_path, self.apps_dir, app_name)
	...
	def test_new_patch_util(self):
		...
		patches_txt = pathlib.Path(pathlib.Path(itds.get_app_path("itds", "patches.txt")))
		...
		expected_patch = "itds.core.doctype.user.patches.delete_all_users"
