...
class TestTestUtils(IntegrationTestCase):
	...
	def test_document_assertions(self):
		currency = itds.new_doc("Currency")
	...
	def test_thread_locals(self):
		itds.flags.temp_flag_to_be_discarded = True

	def test_temp_setting_changes(self):
		current_setting = itds.get_system_settings("logout_on_password_reset")

		with IntegrationTestCase.change_settings(
			"System Settings", {"logout_on_password_reset": int(not current_setting)}
		):
			updated_settings = itds.get_single_value("System Settings", "logout_on_password_reset")
			self.assertNotEqual(current_setting, updated_settings)

		restored_settings = itds.get_single_value("System Settings", "logout_on_password_reset")
...
def tearDownModule():
	...
	assert "temp_flag_to_be_discarded" not in itds.flags
	assert not itds.db.exists("Currency", "STONKS")
