...
if TYPE_CHECKING:
	from itds.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from itds.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	with patch.object(itds, "get_hooks", patched_hooks):
		yield
...
def enable_safe_exec() -> None:
	...
	conf = os.path.join(itds.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from itds.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly itds.tests.* namespace
