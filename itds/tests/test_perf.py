...
class TestPerformance(IntegrationTestCase):
	def reset_request_specific_caches(self):
		...
		itds.destroy()  # releases everything on itds.local
		itds.init(self.TEST_SITE)
		itds.connect()
		itds.clear_cache()

	def setUp(self) -> None:
		self.HOST = itds.utils.get_site_url(itds.local.site)
	...
	def test_permitted_fieldnames(self):
		...
		doc = itds.new_doc("Prepared Report")
		# load permitted fieldnames once
		doc.permitted_fieldnames

		with patch("itds.model.base_document.get_permitted_fields") as mocked:
			doc.as_dict()
			# get_permitted_fields should not be called again
			mocked.assert_not_called()

	def test_set_value_query_count(self):
		itds.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			itds.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			itds.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_get_value_limits(self):
		...
		# Warm up code
		itds.db.get_values("User", filters=filters[0], limit=1)
		for filter in filters:
			with self.assertRowsRead(1):
				self.assertEqual(1, len(itds.db.get_values("User", filters=filter, limit=1)))
			with self.assertRowsRead(2):
				self.assertEqual(2, len(itds.db.get_values("User", filters=filter, limit=2)))

			self.assertEqual(
				len(itds.db.get_values("User", filters=filter)), itds.db.count("User", filter)
			)

			with self.assertRowsRead(1):
				itds.db.get_value("User", filters=filter)

			with self.assertRowsRead(1):
				itds.db.exists("User", filter)
	...
	def test_req_per_seconds_basic(self):
		...
		req_count = 1000
		client = ItdsClient(self.HOST, "Administrator", self.ADMIN_PASSWORD)
	...
	def test_consistent_build_version(self):
		from itds.utils import get_build_version
	...
	def test_get_list_single_query(self):
		...
		user = itds.get_doc("User", TEST_USER)
	...
	def test_no_ifnull_checks(self):
		query = itds.get_all("DocType", {"autoname": ("is", "set")}, run=0).lower()
		self.assertNotIn("coalesce", query)
		self.assertNotIn("ifnull", query)

	def test_no_stale_ref_sql(self):
		"""itds.db.sql should not hold any internal references to result set.
	...
	def test_idle_cpu_utilization_redis_pubsub(self):
		pid = itds.client_cache.invalidator_thread.native_id
		process = psutil.Process(pid)
		self.assertLess(process.cpu_percent(interval=1.0), 2)

	def test_cpu_allocation(self):
		from itds._optimizations import assign_core
...
@run_only_if(db_type_is.MARIADB)
class TestOverheadCalls(ItdsAPITestCase):
	...
	def test_get_doc_overheads(self):
		sid = self.sid
		tables = len(itds.get_meta("User").get_table_fields())
