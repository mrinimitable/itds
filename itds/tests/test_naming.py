...
from itds.utils import now_datetime, nowdate, nowtime
...
class TestNaming(IntegrationTestCase):
	...
	def test_append_number_if_name_exists(self):
		...
		note = itds.get_doc({"doctype": DOCTYPE, "title": TITLE}).insert()
	...
	def test_field_autoname_name_sync(self):
		country = itds.get_last_doc("Country")
	...
	def test_child_table_naming(self):
		...
		name = itds.generate_hash(length=10)

		doc = itds.new_doc(dt_with_child_autoname.name)
	...
	def test_format_autoname_for_consecutive_week_number(self):
		...
		todo_doctype = itds.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{WW}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = itds.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		itds.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		current_index = itds.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]

		self.assertEqual(current_index.get("current"), 0)
		itds.db.delete("Series", {"name": series})

		series = f"TEST-{year}-"
		key = "TEST-.YYYY.-.#####"
		name = f"TEST-{year}-00002"
		itds.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		current_index = itds.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		itds.db.delete("Series", {"name": series})
		itds.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		current_index = itds.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = itds.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = itds.new_doc("Submittable Doctype")
		doc.save()
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = itds.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(itds.NameError, tag.insert)

		# set by passing set_name as ToDo
		self.assertRaises(itds.NameError, make_invalid_todo)

		# name (via title field) cannot be the same as the doctype
		note = itds.get_doc({"doctype": "Currency", "currency_name": "Currency"})
		self.assertRaises(itds.NameError, note.insert)

		# case 2: set name with "New ---"
		tag = itds.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(itds.NameError, tag.insert)
		...
		tag = itds.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(itds.ValidationError, tag.insert)

	def test_autoincremented_naming(self):
		from itds.core.doctype.doctype.test_doctype import new_doctype

		doctype = "autoinc_doctype" + itds.generate_hash(length=5)
		dt = new_doctype(doctype, autoname="autoincrement").insert(ignore_permissions=True)

		for i in range(1, 20):
			self.assertEqual(itds.new_doc(doctype).save(ignore_permissions=True).name, i)
	...
	def test_naming_series_validation(self):
		dns = itds.get_doc("Document Naming Settings")
	...
	def test_hash_collision(self):
		...
		name = itds.generate_hash()
		for _ in range(10):
			itds.flags.in_import = True
			itds.new_doc(doctype).update({"name": name}).insert()
		itds.flags.pop("in_import", None)
		...
		with self.patch_hooks(
			{
				"naming_series_variables": {
					"PM": ["itds.tests.test_naming.parse_naming_series_variable"],
				},
			},
		):
	...
	def test_uuid_naming(self):
		...
		self.assertEqual("uuid", itds.db.get_column_type(uuid_doctype, "name"))

		# Auto set names
		document = itds.new_doc(uuid_doctype).insert()
		...
		for uid in (uuid_utils.uuid4(), uuid_utils.uuid7()):
			doc = itds.new_doc(uuid_doctype, name=uid).insert()
			self.assertEqual(doc.name, str(uid))

		# Can specify valid UUID strings too
		for uid in (uuid_utils.uuid4(), uuid_utils.uuid7()):
			doc = itds.new_doc(uuid_doctype, name=str(uid)).insert()
			self.assertEqual(doc.name, str(uid))

		self.assertRaises(InvalidUUIDValue, itds.new_doc(uuid_doctype, name="XYZ").insert)
...
def make_invalid_todo():
	itds.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")
