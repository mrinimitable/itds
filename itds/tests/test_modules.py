...
from itds.modules import export_customizations, export_module_json, get_module_path
from itds.modules.utils import export_doc, sync_customizations
...
class TestUtils(IntegrationTestCase):
	def setUp(self):
		self._dev_mode = itds.local.conf.developer_mode
		itds.local.conf.developer_mode = True

	def tearDown(self):
		itds.db.rollback()
		itds.local.conf.developer_mode = self._dev_mode
		itds.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		itds.local.flags.in_import = True
		doc = itds.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		doc = itds.get_last_doc("DocType", {"issingle": 0, "custom": 0})
		...
		with open(export_doc_path) as f:
			export_doc_before = itds.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=itds.as_json(export_doc_before))

		itds.flags.in_import = False
		itds.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			itds.parse_json(f.read())  # export_doc_after
	...
	def test_reload_doc(self):
		itds.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(itds.db.get_value("DocType", "Note", "migration_hash"))
		itds.db.set_value(
			"DocField",
		...
		self.assertEqual(
			itds.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		itds.reload_doctype("Note")
		self.assertEqual(
			itds.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
		self.assertTrue(itds.db.get_value("DocType", "Note", "migration_hash"))
	...
	def test_export_doc(self):
		...
		note.title = itds.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			itds.get_app_path("itds", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = itds.scrub(doctype.name)
			path = itds.get_app_path("itds", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		itds.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(itds.get_module_path("Desk", "Note"))
