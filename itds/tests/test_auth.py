...
import itds
from itds.auth import LoginAttemptTracker
from itds.itdsclient import AuthError, ItdsClient
from itds.sessions import Session, get_expired_sessions, get_expiry_in_seconds
from itds.tests import IntegrationTestCase, UnitTestCase
from itds.tests.test_api import ItdsAPITestCase
from itds.utils import get_datetime, get_site_url, now
from itds.utils.data import add_to_date
from itds.www.login import _generate_temporary_login_link
...
class TestAuth(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = itds.get_site_config().host_name or get_site_url(itds.local.site)
	...
	def tearDownClass(cls):
		itds.db.rollback()
		itds.delete_doc("User", cls.test_user_email, force=True)
		itds.local.request_ip = None
		itds.form_dict.email = None
		itds.local.response["http_status_code"] = None
		itds.db.commit()

	def set_system_settings(self, k, v):
		itds.db.set_single_value("System Settings", k, v)
		itds.clear_cache()
		itds.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		ItdsClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		ItdsClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			ItdsClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = ItdsClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = ItdsClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = ItdsClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
	...
	def test_disable_user_pass_login(self):
		ItdsClient(self.HOST_NAME, self.test_user_email, self.test_user_password).get_list("ToDo")
	...
	def test_correct_cookie_expiry_set(self):
		client = ItdsClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
...
class TestAllowedReferrer(UnitTestCase):
	def test_is_allowed_referrer(self):
		...
		itds.cache.set_value("allowed_referrers", ["https://example.com"])
		itds.local.request = create_request({"Referer": "https://example.com/some/path"})
		http_request = itds.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid referrer
		itds.local.request = create_request({"Referer": "https://malicious.com"})
		http_request = itds.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test with valid origin
		itds.local.request = create_request({"Origin": "https://example.com"})
		http_request = itds.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid origin
		itds.local.request = create_request({"Origin": "https://malicious.com"})
		http_request = itds.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Clean up
		itds.cache.delete_value("allowed_referrers")
...
class TestSessionExpiry(ItdsAPITestCase):
	def test_session_expires(self):
		sid = self.sid  # triggers login for test case login
		s: Session = itds.local.session_obj
