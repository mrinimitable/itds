...
import itds
from itds.modules import patch_handler
...
execute:itds.db.updatedb("Item")
execute:itds.function(arg="1")
...
class TestPatches(IntegrationTestCase):
	def test_patch_module_names(self):
		itds.flags.final_patches = []
		itds.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(itds.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		itds.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("itds", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("itds", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("itds")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("itds")
		finished_patches = itds.db.count("Patch Log")
...
class TestPatchReader(IntegrationTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("itds"),
			patch_handler.get_patches_from_app("itds", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("itds", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(itds.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = itds.get_module(module)
...
def _get_dotted_path(file: Path, app) -> str:
	app_path = Path(itds.get_app_path(app))
