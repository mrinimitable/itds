...
from itds.utils import add_days, get_datetime
from itds.utils.doctor import purge_pending_jobs
...
class TestScheduler(IntegrationTestCase):
	def setUp(self):
		...
		purge_pending_jobs()
		if not itds.get_all("Scheduled Job Type", limit=1):
			sync_jobs()
	...
	def test_enqueue_jobs(self):
		itds.db.sql("update `tabScheduled Job Type` set last_execution = '2010-01-01 00:00:00'")

		enqueued_jobs = enqueue_events()

		self.assertIn("itds.desk.notifications.clear_notifications", enqueued_jobs)
		self.assertIn("itds.utils.change_log.check_for_update", enqueued_jobs)
		self.assertIn(
	...
	@patch.object(itds.utils.itdscloud, "on_itdscloud", return_value=True)
	@patch.dict(itds.conf, {"developer_mode": 0})
	...
	def test_once_a_day_for_dormant(self, _mocks):
		...
		itds.db.truncate("Scheduled Job Log")
		...
		job = get_test_job(method="itds.tests.test_scheduler.test_method", frequency="Daily")
		job.execute()
		job_log = itds.get_doc("Scheduled Job Log", dict(scheduled_job_type=job.name))
...
def get_test_job(method="itds.tests.test_scheduler.test_timeout_10", frequency="All") -> ScheduledJobType:
	if not itds.db.exists("Scheduled Job Type", dict(method=method)):
		job = itds.get_doc(
		...
		job = itds.get_doc("Scheduled Job Type", dict(method=method))
