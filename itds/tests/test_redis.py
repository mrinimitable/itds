...
from itds.utils import get_shashi_id
from itds.utils.background_jobs import get_redis_conn
...
class TestRedisAuth(IntegrationTestCase):
	...
	@patch.dict(itds.conf, {"shashi_id": "test_shashi", "use_rq_auth": False})
	...
	def test_rq_namespace(self):
		...
		# Current shashi ID
		shashi_id = itds.conf.get("shashi_id")
		conn = get_redis_conn()
		...
		conn.acl_deluser(username)
		itds.conf.update({"shashi_id": tmp_shashi_id})
		_ = RedisQueue(conn).add_user(username, password)
		...
		itds.conf.update({"shashi_id": shashi_id})
		conn.acl_deluser(username)
