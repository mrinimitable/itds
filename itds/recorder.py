...
import itds
from itds import _
from itds.database.utils import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			itds.throw("You must record one of jobs or requests")

	def store(self):
		itds.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return itds.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		itds.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = itds.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(itds.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if itds.local._recorder.config.capture_stack:
	...
	itds.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from itds.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	"""
	itds.db.rollback()
	itds.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(itds.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = itds.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		itds.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	flag_value = itds.client_cache.get_value(RECORDER_INTERCEPT_FLAG)
	if flag_value or force:
		itds.local._recorder = Recorder(force=force)
		return itds.local._recorder
	elif flag_value is None:
		# Explicitly set it once so next requests can use client-side cache
		itds.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)


def dump():
	if hasattr(itds.local, "_recorder"):
		itds.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and itds.request
			and self.config.request_filter in itds.request.path
		):
			self.path = itds.request.path
			self.cmd = itds.local.form_dict.cmd or ""
			self.method = itds.request.method
			self.headers = dict(itds.local.request.headers)
			self.form_dict = itds.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and itds.job and self.config.jobs_filter in itds.job.method:
			self.event_type = "Background Job"
			self.path = itds.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = itds.generate_hash(length=10)
		self.time = now_datetime()

		self._patch_sql(itds.db)
	...
	def dump(self):
		...
		itds.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = profiler_output
		itds.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	def _patch_sql(self, db: "Database"):
		...
		itds.db._sql = itds.db.sql
		itds.db.sql = record_sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(itds.local, "_recorder"):
			itds.local._recorder.cleanup()
			del itds.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if itds.session.user != "Administrator":
			itds.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def status(*args, **kwargs):
	return bool(itds.cache.get_value(RECORDER_INTERCEPT_FLAG))
...
def start(
	...
	**kwargs,
	...
	itds.client_cache.set_value(RECORDER_INTERCEPT_FLAG, True)
	itds.cache.expire_key(RECORDER_INTERCEPT_FLAG, RECORDER_AUTO_DISABLE)
...
def stop(*args, **kwargs):
	itds.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
	itds.enqueue(post_process, now=itds.in_test)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = itds.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(itds.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def export_data(*args, **kwargs):
	return list(itds.cache.hgetall(RECORDER_REQUEST_HASH).values())
...
def delete(*args, **kwargs):
	itds.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	itds.cache.delete_value(RECORDER_REQUEST_HASH)
...
def import_data(file: str) -> None:
	file_doc = itds.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		itds.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		itds.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
