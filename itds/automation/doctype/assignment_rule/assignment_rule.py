...
import itds
from itds import _
from itds.cache_manager import clear_doctype_map, get_doctype_map
from itds.desk.form import assign_to
from itds.model import log_types
from itds.model.document import Document
from itds.utils.data import comma_and
...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from itds.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from itds.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			itds.throw(
				_("Assignment Rule is not allowed on document type {0}").format(itds.bold(_("ToDo")))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=itds.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if itds.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return itds.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			itds.msgprint(itds._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = itds.flags.assignment_day or itds.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = itds.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		itds.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = itds.get_cached_doc("Assignment Rule", rule.get("name"))
...
def get_assignment_rules() -> list[str]:
	return itds.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
