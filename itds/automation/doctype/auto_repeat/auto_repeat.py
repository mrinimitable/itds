...
import itds
from itds import _
from itds.automation.doctype.assignment_rule.assignment_rule import get_repeated
from itds.contacts.doctype.contact.contact import (
	get_contacts_linked_from,
	get_contacts_linking_to,
)
from itds.core.doctype.communication.email import make
...
from itds.utils.background_jobs import get_jobs
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from itds.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
	...
	def on_update(self):
		itds.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		itds.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		itds.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def set_dates(self):
		if self.disabled:
			...
			if self.end_date and getdate(self.end_date) < getdate(self.next_schedule_date):
				itds.throw(_("The Next Scheduled Date cannot be later than the End Date."))
	...
	def validate_reference_doctype(self):
		if itds.in_test or itds.flags.in_patch:
			return
		if not itds.get_meta(self.reference_doctype).allow_auto_repeat:
			itds.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not itds.get_meta(self.reference_doctype).is_submittable:
			itds.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(
					itds.bold(_("Submit on Creation"))
				)
			)

	def validate_dates(self):
		if itds.flags.in_patch:
			return
		...
		if self.end_date == self.start_date:
			itds.throw(
				_("{0} should not be same as {1}").format(
					itds.bold(_("End Date")), itds.bold(_("Start Date"))
				)
			)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						itds.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				itds.throw(_("'Recipients' not specified"))
		...
		if len(set(auto_repeat_days)) != len(auto_repeat_days):
			...
			itds.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, itds.bold(", ".join(repeated_days))
				)
			)

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = itds.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not itds.flags.in_patch:
			itds.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			itds.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not itds.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = itds.get_doc(self.reference_doctype, self.reference_document)
		new_doc = itds.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = itds.get_doc("Auto Repeat", self.name)
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = itds.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(itds.bold(_("Note")), itds.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = itds.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				itds.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		itds.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = itds.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = itds.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "itds.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[itds.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		itds.enqueue(enqueued_method, data=data, queue="long")


def create_repeated_entries(data):
	for d in data:
		doc = itds.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				itds.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def get_auto_repeat_entries(date=None):
	...
	auto_repeat = itds.qb.DocType("Auto Repeat")
	query = itds.qb.from_(auto_repeat)
...
def update_reference(docname: str, reference: str):
	doc = itds.get_doc("Auto Repeat", str(docname))
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	itds.has_permission("Auto Repeat", "write", throw=True)
	doc = itds.get_doc(reference_dt, reference_doc)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = itds.render_template(message, {"doc": doc})
	if subject:
		subject_preview = itds.render_template(subject, {"doc": doc})
