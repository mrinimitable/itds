...
itds.provide("itds.auto_repeat");

itds.ui.form.on("Auto Repeat", {
	setup: function (frm) {
		frm.fields_dict["reference_doctype"].get_query = function () {
			return {
				query: "itds.automation.doctype.auto_repeat.auto_repeat.get_auto_repeat_doctypes",
			};
		};
	...
	refresh: function (frm) {
		...
		if (!frm.is_dirty()) {
			...
			frm.add_custom_button(label, () =>
				itds.set_route("List", frm.doc.reference_doctype, { auto_repeat: frm.doc.name })
			);
		}

		// auto repeat schedule
		itds.auto_repeat.render_schedule(frm);
	...
	toggle_submit_on_creation: function (frm) {
		...
		if (frm.doc.reference_doctype) {
			itds.model.with_doctype(frm.doc.reference_doctype, () => {
				let meta = itds.get_meta(frm.doc.reference_doctype);
	...
	template: function (frm) {
		if (frm.doc.template) {
			itds.model.with_doc("Email Template", frm.doc.template, () => {
				let email_template = itds.get_doc("Email Template", frm.doc.template);
	...
	preview_message: function (frm) {
		if (frm.doc.message) {
			itds.call({
				method: "itds.automation.doctype.auto_repeat.auto_repeat.generate_message_preview",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						itds.msgprint(r.message.message, r.message.subject);
					}
				},
			});
		} else {
			itds.msgprint(__("Please setup a message first"), __("Message not setup"));
		}
	},
});

itds.auto_repeat.render_schedule = function (frm) {
	if (!frm.is_dirty() && frm.doc.status !== "Disabled") {
		frm.call("get_auto_repeat_schedule").then((r) => {
			frm.dashboard.reset();
			frm.dashboard.add_section(
				itds.render_template("auto_repeat_schedule", {
