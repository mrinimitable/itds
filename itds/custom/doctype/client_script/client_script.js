...
itds.ui.form.on("Client Script", {
	...
	refresh(frm) {
		if (frm.doc.dt && frm.doc.script) {
			frm.add_custom_button(__("Go to {0}", [frm.doc.dt]), () =>
				itds.set_route("List", frm.doc.dt, "List")
			);
		}

		if (frm.doc.view == "Form") {
			frm.add_custom_button(__("Add script for Child Table"), () => {
				itds.model.with_doctype(frm.doc.dt, () => {
					const child_tables = itds.meta
			...
			if (!frm.is_new()) {
				frm.add_custom_button(__("Compare Versions"), () => {
					new itds.ui.DiffView("Client Script", "script", frm.doc.name);
				});
			}
		}
	...
	dt(frm) {
		frm.toggle_display("view", !itds.boot.single_types.includes(frm.doc.dt));
	...
	view(frm) {
		let has_form_boilerplate = frm.doc.script.includes("itds.ui.form.on");
...
itds.ui.form.on('Task',  'validate',  function(frm) {
