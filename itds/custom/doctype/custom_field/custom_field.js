...
itds.ui.form.on("Custom Field", {
	...
	dt: function (frm) {
		...
		return itds.call({
			method: "itds.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && itds.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				itds.prompt(
					...
					function (data) {
						itds
							.xcall(
								"itds.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
itds.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
