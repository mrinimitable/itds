...
itds.provide("itds.customize_form");

itds.ui.form.on("Customize Form", {
	setup: function (frm) {
		...
		frm.set_query("doc_type", function () {
			return {
				filters: [
					...
					["DocType", "restrict_to_domain", "in", itds.boot.active_domains],
				],
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.doc_type) {
			itds.model.with_doctype(frm.doc.doc_type).then(() => {
				frm.page.set_title(__("Customize Form - {0}", [__(frm.doc.doc_type)]));
				itds.customize_form.set_primary_action(frm);

				frm.add_custom_button(
					__("Go to {0} List", [__(frm.doc.doc_type)]),
					function () {
						itds.set_route("List", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					__("Set Permissions"),
					function () {
						itds.set_route("permission-manager", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					...
					function () {
						itds.customize_form.confirm(__("Remove all customizations?"), frm);
					},
					__("Actions")
				);
				...
				frm.toggle_display(
					["queue_in_background"],
					itds.get_meta(frm.doc.doc_type).is_submittable || 0
				);
	...
	set_default_doc_type(frm) {
		let doc_type;
		if (itds.route_options && itds.route_options.doc_type) {
			doc_type = itds.route_options.doc_type;
				...
				return frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							itds.customize_form.clear_locals_and_refresh(frm);
						}
					},
				});
			}
	...
	async trim_table(frm) {
		let dropped_columns = await itds.xcall(
			"itds.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			itds.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(
			...
			frm.add_custom_button(
				...
				function () {
					itds.prompt(
						...
						function (data) {
							itds.call({
								method: "itds.modules.utils.export_customizations",
								args: {
									doctype: frm.doc.doc_type,
									module: data.module,
	...
	setup_sort_order(frm) {
		...
		if (frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields, function (df) {
				return itds.model.is_value_type(df.fieldtype) ? df.fieldname : null;
			});
	...
	setup_default_views(frm) {
		itds.model.set_default_views_for_doctype(frm.doc.doc_type, frm);
	},
});

// can't delete standard fields
itds.ui.form.on("Customize Form Field", {
	before_fields_remove: function (frm, doctype, name) {
		const row = itds.get_doc(doctype, name);
	...
	fields_add: function (frm, cdt, cdn) {
		var f = itds.model.get_doc(cdt, cdn);
...
itds.ui.form.on("DocType Link", {
	before_links_remove: function (frm, doctype, name) {
		let row = itds.get_doc(doctype, name);
		...
		if (!(row.custom || row.__islocal)) {
			itds.msgprint(__("Cannot delete standard link. You can hide it if you want"));
			throw "cannot delete standard link";
		}
	},
	links_add: function (frm, cdt, cdn) {
		let f = itds.model.get_doc(cdt, cdn);
		f.custom = 1;
	},
...
itds.ui.form.on("DocType Action", {
	before_actions_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			itds.msgprint(__("Cannot delete standard action. You can hide it if you want"));
...
itds.ui.form.on("DocType State", {
	before_states_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			itds.msgprint(__("Cannot delete standard document state."));
...
itds.customize_form.save_customization = function (frm) {
	if (frm.doc.doc_type) {
		return frm.call({
			doc: frm.doc,
			freeze: true,
...
itds.customize_form.update_fields_from_form_builder = function (frm) {
...
itds.customize_form.set_primary_action = function (frm) {
...
itds.customize_form.confirm = function (msg, frm) {
	...
	itds.customize_form.confirm.dialog = d;
	d.show();
};

itds.customize_form.clear_locals_and_refresh = function (frm) {
	delete frm.doc.__unsaved;
	// clear doctype from locals
	itds.model.clear_doc("DocType", frm.doc.doc_type);
	delete itds.meta.docfield_copy[frm.doc.doc_type];
	frm.refresh();
};

function render_form_builder(frm) {
	if (itds.form_builder && itds.form_builder.doctype === frm.doc.doc_type) {
	...
	if (itds.form_builder) {
		...
		itds.form_builder.doctype = frm.doc.doc_type;
		itds.form_builder.customize = true;
