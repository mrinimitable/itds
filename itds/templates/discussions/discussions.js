itds.ready(() => {
	...
	itds.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	itds.realtime.init(window.socketio_port || "9000");

	itds.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	itds.realtime.on("update_message", (data) => {
		update_message(data);
	});

	itds.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == itds.session.user) {
	...
	} else if (topic.owner == itds.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		itds.call({
			method: "itds.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = itds.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	itds.call({
		method: "itds.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	itds.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new itds.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".itds-control").removeClass("hide-control");
