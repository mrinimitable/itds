...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Itds framework. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Itds version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
# see also itds.__init__ enabling them when a dev_server
class ItdsDeprecationError(Warning):
...
class ItdsDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingItdsDeprecationWarning(ItdsDeprecationWarning):
...
warnings.simplefilter("error", ItdsDeprecationError)
warnings.simplefilter("ignore", PendingItdsDeprecationWarning)


class V15ItdsDeprecationWarning(ItdsDeprecationError):
	pass


class V16ItdsDeprecationWarning(ItdsDeprecationWarning):
	pass


class V17ItdsDeprecationWarning(PendingItdsDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}ItdsDeprecationWarning"
		current_module = sys.modules[__name__]
	try:
		return getattr(current_module, class_name)
	except AttributeError:
		return PendingItdsDeprecationWarning
...
for warning_filter in pythonwarnings.split(","):
	...
	if len(parts) >= 3 and (
		parts[2] in ("ItdsDeprecationError", "ItdsDeprecationWarning", "PendingItdsDeprecationWarning")
		or parts[2].endswith("ItdsDeprecationWarning")
	):
		try:
...
try:
	...
	def _deprecated(message: str, category=ItdsDeprecationWarning, stacklevel=1) -> Callable[[T], T]:
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: itds.utils.make_esc  (fully qualified)
	...
	def decorator(func):
		...
		if os.path.basename(caller_filepath) != "deprecation_dumpster.py":
			raise RuntimeError(
				...
				+ colorize("itds/deprecation_dumpster.py\n", Color.CYAN)
				+ colorize("Move the entire function there and import it back via adding\n ", Color.YELLOW)
				+ colorize(f"from itds.deprecation_dumpster import {func.__name__}\n", Color.CYAN)
				+ colorize("to file\n ", Color.YELLOW)
...
def _old_deprecated(func):
	return deprecated(
		"itds.deprecations.deprecated",
		"2024-09-13",
		"v17",
		"Make use of the itds/deprecation_dumpster.py file, instead. 🎉🗑️",
	)(_deprecated("")(func))


def _old_deprecation_warning(msg):
	@deprecated(
		"itds.deprecations.deprecation_warning",
		"2024-09-13",
		"v17",
		"Use itds.deprecation_dumpster.deprecation_warning, instead. 🎉🗑️",
	)
...
@deprecated("itds.utils.make_esc", "unknown", "v17", "Not used anymore.")
...
@deprecated(
	"itds.db.is_column_missing",
	"unknown",
	"v17",
	"Renamed to itds.db.is_missing_column.",
)
def is_column_missing(e):
	import itds

	return itds.db.is_missing_column(e)


@deprecated(
	"itds.desk.doctype.bulk_update.bulk_update",
	"unknown",
	"v17",
	"Unknown.",
)
...
def get_js(items):
	...
	for src in items:
		...
		if ".." in src or src[0] != "assets":
			itds.throw(_("Invalid file path: {0}").format("/".join(src)))

		contentpath = os.path.join(itds.local.sites_path, *src)
		with open(contentpath) as srcfile:
			code = itds.utils.cstr(srcfile.read())
...
@deprecated(
	"itds.utils.print_format.read_multi_pdf",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated("itds.gzip_compress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated("itds.gzip_decompress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated(
	"itds.email.doctype.email_queue.email_queue.send_mail",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated(
	"itds.geo.country_info.get_translated_dict",
	"unknown",
	"v17",
	"Use itds.geo.country_info.get_translated_countries, instead.",
)
def get_translated_dict():
	from itds.geo.country_info import get_translated_countries
...
@deprecated("itds.tests_runner.get_modules", "2024-20-08", "v17", "use itds.tests.utils.get_modules")
def test_runner_get_modules(doctype):
	from itds.tests.utils import get_modules

	return get_modules(doctype)


@deprecated(
	"itds.tests_runner.make_test_records", "2024-20-08", "v17", "use itds.tests.utils.make_test_records"
)
...
@deprecated(
	"itds.tests_runner.make_test_objects", "2024-20-08", "v17", "use itds.tests.utils.make_test_objects"
)
...
@deprecated(
	"itds.tests_runner.make_test_records_for_doctype",
	"2024-20-08",
	"v17",
	"use itds.tests.utils.make_test_records_for_doctype",
)
...
@deprecated(
	"itds.tests_runner.print_mandatory_fields",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_print_mandatory_fields(*args, **kwargs):
	from itds.tests.utils.generators import print_mandatory_fields

	return print_mandatory_fields(*args, **kwargs)


@deprecated(
	"itds.tests_runner.get_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_get_test_record_log(doctype):
	from itds.tests.utils.generators import TestRecordManager

	return TestRecordManager().get(doctype)


@deprecated(
	"itds.tests_runner.add_to_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"itds.tests_runner.main",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_main(*args, **kwargs):
	from itds.commands.testing import main

	return main(*args, **kwargs)


@deprecated(
	"itds.tests_runner.xmlrunner_wrapper",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"itds.tests.upate_system_settings",
	"2024-20-08",
	"v17",
...
@deprecated(
	"itds.tests.get_system_setting",
	"2024-20-08",
	"v17",
	"use `itds.db.get_single_value('System Settings', key)`",
)
def tests_get_system_setting(key):
	import itds

	return itds.db.get_single_value("System Settings", key)


@deprecated(
	"itds.tests.utils.change_settings",
	"2024-20-08",
	"v17",
	"use `itds.tests.change_settings` or the cls.change_settings",
)
...
@deprecated(
	"itds.tests.utils.patch_hooks",
	"2024-20-08",
	"v17",
	"use `itds.tests.patch_hooks` or the cls.patch_hooks",
)
def tests_patch_hooks(*args, **kwargs):
	from itds.tests.classes.context_managers import patch_hooks

	return patch_hooks(*args, **kwargs)


@deprecated(
	"itds.tests.utils.debug_on",
	"2024-20-08",
	"v17",
	"use `itds.tests.debug_on` or the cls.debug_on",
)
def tests_debug_on(*args, **kwargs):
	from itds.tests.classes.context_managers import debug_on

	return debug_on(*args, **kwargs)


@deprecated(
	"itds.tests.utils.timeout",
	"2024-20-08",
	"v17",
	"use `itds.tests.timeout` or the cls.timeout",
)
def tests_timeout(*args, **kwargs):
	from itds.tests.classes.context_managers import timeout

	return timeout(*args, **kwargs)


def get_tests_CompatItdsTestCase():
	...
	class ItdsTestCase(unittest.TestCase):
		"""Base test class for Itds tests.
		...
		@deprecated(
			"itds.tests.utils.ItdsTestCase",
			"2024-20-08",
			"v17",
			"Import `itds.tests.UnitTestCase` or `itds.tests.IntegrationTestCase` respectively instead of `itds.tests.utils.ItdsTestCase` - also see wiki for more info: https://github.com/mrinimitable/itds/wiki#testing-guide",
		)
		...
		def setUpClass(cls) -> None:
			...
			cls.addClassCleanup(_restore_thread_locals, copy.deepcopy(itds.local.flags))
			cls.addClassCleanup(_rollback_db)

			return super().setUpClass()

		def _apply_debug_decorator(self, exceptions=()):
			from itds.tests.utils import debug_on
		...
		# --- Itds Framework specific assertions
		...
		def assertRedisCallCounts(self, count):
			...
			try:
				orig_execute = itds.cache.execute_command
				itds.cache.execute_command = execute_command_and_count
				yield
				self.assertLessEqual(
					len(commands), count, msg="commands executed: \n" + "\n".join(str(c) for c in commands)
				)
			finally:
				itds.cache.execute_command = orig_execute
		...
		def enable_safe_exec(cls) -> None:
			...
			from itds.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

			cls._common_conf = os.path.join(itds.local.sites_path, "common_site_config.json")
		...
		def switch_site(self, site: str):
			...
			try:
				old_site = itds.local.site
				itds.init(site, force=True)
				itds.connect()
				yield
			finally:
				itds.init(old_site, force=True)
				itds.connect()
		...
		def freeze_time(self, time_to_freeze, is_utc=False, *args, **kwargs):
			...
			from itds.utils.data import convert_utc_to_timezone, get_datetime, get_system_timezone
...
# remove alongside get_tests_CompatItdsTestCase
def get_compat_itds_test_case_preparation(cfg):
	import unittest

	import itds
	from itds.testing.environment import IntegrationTestPreparation

	class ItdsTestCasePreparation(IntegrationTestPreparation):
		def __call__(self, suite: unittest.TestSuite, app: str, category: str) -> None:
			super().__call__(suite, app, category)
			candidates = []
			app_path = itds.get_app_path(app)
			for path, folders, files in os.walk(itds.get_app_path(app)):
				...
				for filename in files:
					if filename.startswith("test_") and filename.endswith(".py"):
						...
						module = itds.get_module(module_name)
						candidates.append((module, path, filename))
			compat_preload_test_records_upfront(candidates)

	return ItdsTestCasePreparation(cfg)


@deprecated(
	"itds.model.trace.traced_field_context",
	"2024-20-08",
	"v17",
	"use `cls.trace_fields`",
)
def model_trace_traced_field_context(*args, **kwargs):
	from itds.tests.classes.context_managers import trace_fields

	return trace_fields(*args, **kwargs)


@deprecated(
	"itds.tests.utils.get_dependencies",
	"2024-20-09",
	"v17",
	"refactor to use itds.tests.utils.get_missing_records_doctypes",
)
def tests_utils_get_dependencies(doctype):
	"""Get the dependencies for the specified doctype"""
	import itds
	from itds.tests.utils.generators import get_modules
	...
	for df in meta.get_table_fields():
		link_fields.extend(itds.get_meta(df.options).get_link_fields())
...
@deprecated(
	"itds.tests_runner.get_dependencies",
	"2024-20-08",
	"v17",
	"refactor to use itds.tests.utils.get_missing_record_doctypes",
)
...
@deprecated(
	"itds.get_test_records",
	"2024-20-09",
	"v17",
	"""Please access the global test records pool via cls.globalTestRecords['Some Doc'] -> list.
If not an IntegrationTestCase, use itds.tests.utils.load_test_records_for (check return type).
""",
)
def itds_get_test_records(doctype):
	import itds
	from itds.tests.utils.generators import load_test_records_for
...
@deprecated(
	"itds.utils.data.get_number_format_info",
	"unknown",
	"v16",
	"Use `NumberFormat.from_string()` from `itds.utils.number_format` instead",
)
def get_number_format_info(format: str) -> tuple[str, str, int]:
	"""DEPRECATED: use `NumberFormat.from_string()` from `itds.utils.number_format` instead.
...
@deprecated(
	...
	"2024-11-12",
	"yet unknown",
	"""It has been added for compatibility in addition to the new .itds sentinel file inside the module. This is for your info: you don't have to do anything.
""",
)
def boilerplate_modules_txt(dest, app_name, app_title):
	...
	with open(os.path.join(dest, app_name, app_name, "modules.txt"), "w") as f:
		f.write(itds.as_unicode(app_title))
