itds.pages["print-format-builder"].on_page_load = function (wrapper) {
	itds.print_format_builder = new itds.PrintFormatBuilder(wrapper);
	itds.breadcrumbs.add("Setup", "Print Format");
};

itds.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = itds.get_route();
	if (route.length > 1) {
		itds.model.with_doc("Print Format", route[1], function () {
			itds.print_format_builder.print_format = itds.get_doc("Print Format", route[1]);
			itds.print_format_builder.refresh();
		});
	} else if (itds.route_options) {
		if (itds.route_options.make_new) {
			let { doctype, name, based_on, beta } = itds.route_options;
			itds.route_options = null;
			itds.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			itds.print_format_builder.print_format = itds.route_options.doc;
			itds.route_options = null;
			itds.print_format_builder.refresh();
		}
	}
...
itds.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = itds.ui.make_app_page({
			parent: this.parent,
	...
	show_start() {
		this.page.main.html(itds.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = itds.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			itds.model.with_doc("Print Format", name, function (doc) {
				itds.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = itds.ui.form.make_control({
		...
		this.name_input = itds.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				itds.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		itds.call({
			method: "itds.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		itds.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = itds.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				itds.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(itds.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			itds.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						itds.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!itds.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(itds.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = itds.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = itds.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				itds.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = itds.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				itds.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		itds.call({
			...
			callback: function (r) {
				...
				itds.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
