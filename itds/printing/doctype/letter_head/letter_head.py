...
from itds.utils import flt, is_image
...
class LetterHead(Document):
	...
	def validate_disabled_and_default(self):
		if self.disabled and self.is_default:
			itds.throw(_("Letter Head cannot be both disabled and default"))

		if not self.is_default and not self.disabled:
			if not itds.db.exists("Letter Head", dict(is_default=1)):
				self.is_default = 1
	...
	def set_image_as_html(
		...
		if not self.get(field) or not is_image(self.get(field)):
			itds.msgprint(failure_msg, alert=True, indicator="orange")
			return
...
{dimension}="{dimension_value}" style="{dimension}: {dimension_value}px;">
		...
		itds.msgprint(success_msg, alert=True)
	...
	def set_as_default(self):
		...
		if self.is_default:
			itds.db.set_value("Letter Head", {"name": ["!=", self.name]}, "is_default", 0)

			set_default("letter_head", self.name)

			# update control panel - so it loads new letter directly
			itds.db.set_default("default_letter_head_content", self.content)
		else:
			itds.defaults.clear_default("letter_head", self.name)
			itds.defaults.clear_default("default_letter_head_content", self.content)
