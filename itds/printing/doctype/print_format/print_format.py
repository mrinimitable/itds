...
from itds.utils.weasyprint import download_pdf, get_html
...
class PrintFormat(Document):
	...
	def validate(self):
		if (
			...
			and not itds.flags.in_install
			and not itds.in_test
		):
			itds.throw(itds._("Standard Print Format cannot be updated"))

		# old_doc_type is required for clearing item cache
		self.old_doc_type = itds.db.get_value("Print Format", self.name, "doc_type")
		...
		if self.custom_format and self.raw_printing and not self.raw_commands:
			itds.throw(_("{0} are required").format(itds.bold(_("Raw Commands"))), itds.MandatoryError)

		if self.custom_format and not self.html and not self.raw_printing:
			itds.throw(_("{0} is required").format(itds.bold(_("HTML"))), itds.MandatoryError)

		if self.print_format_for == "Report" and not self.report:
			itds.throw(_("{0} is required").format(itds.bold(_("Report"))), itds.MandatoryError)
	...
	def on_update(self):
		if hasattr(self, "old_doc_type") and self.old_doc_type:
			itds.clear_cache(doctype=self.old_doc_type)
		if self.doc_type:
...
def make_default(name: str):
	...
	print_format = itds.get_doc("Print Format", name)
	print_format.check_permission("write")

	doctype = itds.get_doc("DocType", print_format.doc_type)
	if doctype.custom:
	...
	itds.msgprint(
		itds._("{0} is now default print format for {1} doctype").format(
			itds.bold(name), itds.bold(print_format.doc_type)
		)
	)
