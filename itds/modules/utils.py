...
import itds
from itds import _, get_module_path, scrub
...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
	...
	`/home/gavin/itds-shashi/apps/itds/itds/core/print_format/_test_print_format_1/_test_print_format_1`
	"""
	if not itds.flags.in_import and is_standard and itds.conf.developer_mode:
		...
		return os.path.join(
			itds.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module: str, doctype: str, name: str) -> "ModuleType":
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=itds.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return itds.get_module(module_name)
...
def export_customizations(
	...
	This will be synced with shashi migrate"""

	sync_on_migrate = cint(sync_on_migrate)
	with_permissions = cint(with_permissions)

	if not itds.conf.developer_mode:
		itds.throw(_("Only allowed to export customizations in developer mode"))

	custom = {
		"custom_fields": itds.get_all("Custom Field", fields="*", filters={"dt": doctype}, order_by="name"),
		"property_setters": itds.get_all(
			"Property Setter", fields="*", filters={"doc_type": doctype}, order_by="name"
		),
		"custom_perms": [],
		"links": itds.get_all("DocType Link", fields="*", filters={"parent": doctype}, order_by="name"),
		"doctype": doctype,
		"sync_on_migrate": sync_on_migrate,
	}

	if with_permissions:
		custom["custom_perms"] = itds.get_all(
	...
	for d in itds.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(itds.as_json(custom))

		itds.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
		return path
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in itds.local.app_modules.get(app_name) or []:
			folder = itds.get_app_path(app_name, module_name, "custom")
			if os.path.exists(folder):
				for fname in os.listdir(folder):
					if fname.endswith(".json"):
						...
						elif itds.flags.in_install and app:
							sync_customizations_for_doctype(data, folder, fname)
...
def sync_customizations_for_doctype(data: dict, folder: str, filename: str = ""):
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = itds.get_doc(data)
					doc.db_insert()

			match custom_doctype:
				case "Custom Field":
					for d in data[key]:
						field = itds.db.get_value(
						...
						if not field:
							...
							custom_field = itds.get_doc("Custom Field", field)
				...
				case "Custom DocPerm":
					...
					itds.db.delete("Custom DocPerm", {"parent": doc_type})
	...
	if update_schema and not itds.db.get_value("DocType", doctype, "issingle"):
...
def reload_doc(
	...
	from itds.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from itds.modules.export_file import write_document_file

	print(f"Exporting Document {doctype} {name}")
	module = module or itds.db.get_value("DocType", name, "module")
	write_document_file(itds.get_doc(doctype, name), module)


def get_doctype_module(doctype: str) -> str:
	"""Return **Module Def** name of given doctype."""
	doctype_module_map = itds.cache.get_value(
		"doctype_modules",
		generator=lambda: dict(itds.qb.from_("DocType").select("name", "module").run()),
	)

	if module_name := doctype_module_map.get(doctype):
		return module_name
	else:
		itds.throw(_("DocType {} not found").format(doctype), exc=itds.DoesNotExistError)
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	if key not in doctype_python_modules:
		try:
			doctype_python_modules[key] = itds.get_module(module_name)
...
def get_module_app(module: str) -> str:
	app = itds.local.module_app.get(scrub(module))
	if app is None:
		itds.throw(_("Module {} not found").format(module), exc=itds.DoesNotExistError)
	return app
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		itds.throw(_("App not found for module: {0}").format(module))
	return itds.get_hooks(hook="app_publisher", app_name=app)[0]


def make_boilerplate(
	template: str, doc: Union["Document", "itds._dict"], opts: Union[dict, "itds._dict"] = None
):
	...
	doc = doc or itds._dict()
	opts = opts or itds._dict()
	app_publisher = get_app_publisher(doc.module)
	base_class = "Document"
	base_class_import = "from itds.model.document import Document"
	controller_body = "pass"

	if doc.get("is_tree"):
		base_class = "NestedSet"
		base_class_import = "from itds.utils.nestedset import NestedSet"
	...
	with open(target_file_path, "w") as target, open(template_file_path) as source:
		...
		target.write(itds.as_unicode(controller_file_content))
