...
Check out some examples [here](https://itdsframework.com/docs/v14/user/en/api/query-builder)
...
```python
itds.db.sql("select `name` from `tabUser`")
```
...
itds.qb.from_("User").select(Field("name"))
...
```python
itds.db.get_values("User", fieldname="name", filters={})
```

This module is used to support the 3rd way of query generation in itds.
...
This module supports almost all the features present in db_query which powers `itds.get_all` and `itds.get_list`
...
1. Dict Query
   ...
   ```python
       itds.db.get_values("ToDo", fieldname="name", filters={"description": "Something Random"})
   ```

   and many other possible caveats to the dict representation such as

   ```python
      itds.db.get_values("User", fieldname="name",
      filters={"name": ("like", "admin%")})

      itds.db.get_values("ToDo", fieldname="name", filters={"description": ("in", ["somso%", "someome"])})
   ```
...
2. Misc Query
   ...
   - To support this
     ...
     ```python
     itds.db.get_values("ToDo", fieldname="name", filters=["description", "=", "someone"])
     ```
...
3. Criterion Query
   ...
   itds.db.get_values("User", fieldname="name", filters=Field("name") == "Administrator")
...
### 2. Implementing the missing features which are present in `itds.get_list` and `itds.get_all` (do we even need so much magic?)
