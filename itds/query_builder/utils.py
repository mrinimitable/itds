...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(itds.conf.db_type)
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from itds.database.query import Engine
...
def DocType(*args, **kwargs):
	return itds.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return itds.qb.Table(*args, **kwargs)


def execute_query(query, *args, **kwargs):
	child_queries = query._child_queries
	query, params = prepare_query(query)
	result = itds.local.db.sql(query, params, *args, **kwargs)  # nosemgrep
...
def prepare_query(query):
	...
	if itds.local.flags.get("in_safe_exec", False):
		from itds.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, check_safe_sql_query
		...
		if not check_safe_sql_query(query, throw=False):
			...
			if len(callstack) >= 3 and SERVER_SCRIPT_FILE_PREFIX in callstack[2].filename:
				raise itds.PermissionError("Only SELECT SQL allowed in scripting")
...
def patch_query_execute():
	...
	This excludes the use of `itds.db.sql` method while
...
def patch_query_aggregation():
	"""Patch aggregation functions to itds.qb"""
	from itds.query_builder.functions import _avg, _max, _min, _sum
