"""
ItdsClient is a library that helps you connect with other itds systems
"""
...
class ItdsClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		itds_authorization_source=None,
	):
		import requests
		...
		self.itds_authorization_source = itds_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.itds_authorization_source:
				auth_source = {"Itds-Authorization-Source": self.itds_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": itds.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return itds._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "itds.client.insert_many", "docs": itds.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": itds.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return itds._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "itds.client.bulk_update", "docs": itds.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "itds.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "itds.client.submit", "doc": itds.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "itds.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": itds.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "itds.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not itds.db.exists("User", doc.get("owner")):
				itds.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = itds.db.get_single_value(doctype, "modified")
		itds.get_doc(doc).insert()
	...
	def post_process(self, response):
		...
		if rjson and (rjson.get("exc") or rjson.get("exc_type") or rjson.get("errors")):
			try:
				...
				exc = "ItdsClient Request Failed\n\n" + exception
			except Exception:
				exc = rjson.get("exc")

			raise ItdsException(exc)
		if "message" in rjson:
...
class ItdsOAuth2Client(ItdsClient):
