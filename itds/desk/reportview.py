...
from itds.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
from itds.model.base_document import get_controller
from itds.model.db_query import DatabaseQuery
from itds.model.utils import is_virtual_doctype
from itds.utils import add_user_info, cint, format_duration
...
def get():
	...
	if is_virtual_doctype(args.doctype):
		...
		data = compress(itds.call(controller.get_list, args=args, **args))
	else:
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		data = itds.call(controller.get_list, args=args, **args)
	else:
		# uncompressed (refactored from itds.model.db_query.get_list)
		data = execute(**args)
...
def get_count() -> int | None:
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		return itds.call(controller.get_count, args=args, **args)
	...
	timeout_clause = "SET STATEMENT max_statement_time=1 FOR" if itds.db.db_type == "mariadb" else ""

	try:
		count = itds.db.sql(f"{timeout_clause} select count(*) from ( {partial_query} ) p")[0][0]
	except Exception as e:
		if itds.db.is_statement_timeout(e):  # Skip fetching accurate count
			count = None
		else:
			raise

	if count == args.limit or count is None:
		itds.local.response_headers.set("Cache-Control", "private,max-age=600,stale-while-revalidate=10800")
...
def get_form_params():
	...
	data = itds._dict(itds.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = itds.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			itds.throw(_("Invalid aggregate function"))

		if itds.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	itds.throw(_("Field not permitted in query") + f": {fieldname}", itds.DataError)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = itds.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from itds.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = itds.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if itds.db.exists("Report", name):
		report = itds.get_doc("Report", name)
		if report.is_standard == "Yes":
			itds.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			itds.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != itds.session.user and not report.has_permission("write"):
			itds.throw(_("Insufficient Permissions for editing Report"), itds.PermissionError)
	else:
	...
	itds.msgprint(
		_("Report {0} saved").format(itds.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
@itds.whitelist(methods=["POST", "DELETE"])
def delete_report(name):
	"""Delete reports of type Report Builder from Report View"""

	report = itds.get_doc("Report", name)
	if report.is_standard == "Yes":
		itds.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		itds.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != itds.session.user and not report.has_permission("delete"):
		itds.throw(_("Insufficient Permissions for deleting Report"), itds.PermissionError)

	report.delete(ignore_permissions=True)
	itds.msgprint(
		_("Report {0} deleted").format(itds.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from itds.desk.utils import get_csv_bytes, pop_csv_params, provide_binary_file
	...
	if not itds.permissions.can_export(doctype):
		if itds.permissions.can_export(doctype, is_owner=True):
			for row in ret:
				if row[-1] != itds.session.user:
	...
	if itds.local.lang == "en" or not translate_values:
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(itds.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from itds.utils.xlsxutils import make_xlsx
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = itds.get_meta(parenttype).get_field(fieldname)
			if df and df.fieldtype in ("Data", "Select", "Small Text", "Text"):
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = getattr(df, "translatable", False)
			elif df and df.fieldtype == "Link" and itds.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(itds.form_dict.get("items")), reverse=True)
	doctype = itds.form_dict.get("doctype")

	if len(items) > 10:
		itds.enqueue("itds.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			itds.flags.in_bulk_delete = True
			itds.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_sidebar_stats(stats, doctype, filters=None):
	...
	if is_virtual_doctype(doctype):
		...
		data = itds.call(controller.get_stats, args=args, **args)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = itds.db.get_table_columns(doctype)
	except (itds.db.InternalError, itds.db.ProgrammingError):
