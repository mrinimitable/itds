...
def validate_route_conflict(doctype, name):
	...
	for _doctype in ["Page", "Workspace", "DocType"]:
		all_names.extend(
			[slug(d) for d in itds.get_all(_doctype, pluck="name") if (doctype != _doctype and d != name)]
		)

	if slug(name) in all_names:
		itds.msgprint(itds._("Name already taken, please set a new name"))
		raise itds.NameError
...
def provide_binary_file(filename: str, extension: str, content: bytes) -> None:
	...
	itds.response["filecontent"] = content
	itds.response["filename"] = f"{_(filename)}.{extension}"
