...
from itds import _, is_whitelisted, validate_and_sanitize_search_inputs
from itds.database.schema import SPECIAL_CHAR_PATTERN
from itds.model.db_query import get_order_by
from itds.permissions import has_permission
from itds.utils import cint, cstr, unique
from itds.utils.data import make_filter_tuple
...
def sanitize_searchfield(searchfield: str):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		itds.throw(_("Invalid Search Field {0}").format(searchfield), itds.DataError)
...
def search_widget(
	...
	standard_queries = itds.get_hooks().standard_queries or {}
	...
	if query:  # Query = custom search query i.e. python function
		try:
			is_whitelisted(itds.get_attr(query))
			return itds.call(
				query,
				doctype,
				txt,
		...
		except (itds.PermissionError, itds.AppNotInstalledError, ImportError):
			if itds.local.conf.developer_mode:
				raise
			else:
	...
	if not meta.translated_doctype:
		_txt = itds.db.escape((txt or "").replace("%", "").replace("@", ""))
...
def get_names_for_mentions(search_term):
	users_for_mentions = itds.cache.get_value("users_for_mentions", get_users_for_mentions)
	user_groups = itds.cache.get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = itds.utils.get_url_to_form(
...
def get_user_groups():
	return itds.get_all("User Group", fields=["name as id", "name as value"], update={"is_group": True})
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return itds.db.get_value(doctype, docname, meta.title_field)
