...
class NotificationLog(Document):
	...
	def after_insert(self):
		itds.publish_realtime("notification", after_commit=True, user=self.for_user)
	...
	def clear_old_logs(days=180):
		...
		table = itds.qb.DocType("Notification Log")
...
def get_permission_query_conditions(for_user):
	...
	return f"""(`tabNotification Log`.for_user = {itds.db.escape(for_user)})"""


def get_title(doctype, docname, title_field=None):
	if not title_field:
		title_field = itds.get_meta(doctype).get_title_field()
	return docname if title_field == "name" else itds.db.get_value(doctype, docname, title_field)
...
def enqueue_create_notification(users: list[str] | str, doc: dict):
	...
	itds.enqueue(
		"itds.desk.doctype.notification_log.notification_log.make_notification_logs",
		doc=doc,
		users=users,
		now=itds.in_test,
		enqueue_after_commit=not itds.in_test,
	)
...
def send_notification_email(doc: NotificationLog):
	from itds.utils import get_url_to_form, strip_html

	user = itds.db.get_value("User", doc.for_user, fieldname=["email", "language"], as_dict=True)
	if not user:
		return
...
def get_notification_logs(limit=20):
	notification_logs = itds.db.get_list(
...
def mark_all_as_read():
	unread_docs_list = itds.get_all(
		"Notification Log", filters={"read": 0, "for_user": itds.session.user}
	)
	unread_docnames = [doc.name for doc in unread_docs_list]
	if unread_docnames:
		filters = {"name": ["in", unread_docnames]}
		itds.db.set_value("Notification Log", filters, "read", 1, update_modified=False)
...
def mark_as_read(docname: str):
	...
	if docname:
		itds.db.set_value("Notification Log", str(docname), "read", 1, update_modified=False)


@itds.whitelist()
def trigger_indicator_hide():
	itds.publish_realtime("indicator_hide", user=itds.session.user)


def set_notifications_as_unseen(user):
	try:
		itds.db.set_value("Notification Settings", user, "seen", 0, update_modified=False)
