...
import itds
from itds.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from itds.model.document import Document
from itds.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from itds.utils.caching import redis_cache
from itds.utils.data import add_to_date
from itds.utils.scheduler import (
	...
	def suppress_exception(func: Callable):
		...
		def wrapper(*args, **kwargs):
			try:
				...
				if itds.in_test:
					raise
				itds.log(itds.get_traceback())
				# nosemgrep
				itds.msgprint(
					f"System Health check step {itds.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from itds.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from itds.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from itds.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from itds.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from itds.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = itds.enqueue("itds.ping", at_front=True).id
		self.background_jobs_check = "queued"
		workers = itds.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		for job_type in itds.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = itds.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = itds.db.count("Email Queue", filters)
		self.pending_emails = itds.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = itds.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = itds.db.count("Unhandled Email", filters)
		self.handled_emails = itds.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = itds.db.count("Error Log", filters)

		top_errors = itds.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = itds.db.db_type
		self.db_storage_usage = sum(table.size or 0.0 for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)

		if itds.db.db_type == "sqlite":
			self.database_version = itds.db.sql("select sqlite_version()")[0][0]
		else:
			self.database_version = itds.db.sql("select version()")[0][0]

		if itds.db.db_type == "mariadb":
			self.bufferpool_size = itds.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = itds.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(itds.cache.get_keys(""))
		self.cache_memory_usage = itds.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from itds.desk.page.backups.backups import get_context
		...
		self.onsite_backups = len(get_context(itds._dict()).get("files", []))

	@health_check("Users")
	def fetch_user_stats(self):
		threshold = add_to_date(None, days=-30, as_datetime=True)
		self.total_users = itds.db.count("User", {"enabled": 1})
		self.new_users = itds.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = itds.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = itds.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	itds.only_for("System Manager")
	try:
		return itds.get_doc("RQ Job", job_id).status
	except Exception:
		itds.clear_messages()
...
def _get_directory_size(*path):
	folder = os.path.abspath(itds.get_site_path(*path))
