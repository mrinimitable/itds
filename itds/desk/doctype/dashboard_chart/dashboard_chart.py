...
from itds.utils import cint, flt, get_datetime, getdate, has_common, now_datetime, nowdate
from itds.utils.dashboard import cache_source
from itds.utils.data import format_date
from itds.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_reports = [itds.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
...
def has_permission(doc, ptype, user):
	...
	if doc.roles:
		...
		allowed_doctypes = itds.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart: DashboardChart = itds.get_doc("Dashboard Chart", chart_name)
	else:
		chart = itds._dict(itds.parse_json(chart))
	...
	filters = itds.parse_json(filters) or itds.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = itds.new_doc("Dashboard Chart")
	...
	if itds.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard_link = itds.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = itds.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_group_by_chart_config(chart, filters) -> dict | None:
	...
	group_by_field_field = itds.get_meta(doctype).get_field(
		group_by_field
	)  # get info about @group_by_field

	if data and group_by_field_field.fieldtype == "Link":  # if @group_by_field is link
		title_field = itds.get_meta(group_by_field_field.options)  # get title field
		if title_field.title_field:  # if has title_field
			for item in data:  # replace chart labels from name to title value
				item.name = itds.get_value(group_by_field_field.options, item.name, title_field.title_field)
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": itds.session.user, "is_public": 1}
	return itds.db.get_list(
...
class DashboardChart(Document):
	...
	if TYPE_CHECKING:
		...
		from itds.desk.doctype.dashboard_chart_field.dashboard_chart_field import DashboardChartField
	...
	def on_update(self):
		itds.cache.delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not itds.conf.developer_mode and self.is_standard:
			itds.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			itds.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and itds.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			itds.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				itds.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				itds.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				itds.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if itds.get_meta(self.document_type).issingle:
			itds.throw(_("You cannot create a dashboard chart from single DocTypes"))
