...
class FormTour(Document):
	...
	if TYPE_CHECKING:
		from itds.desk.doctype.form_tour_step.form_tour_step import FormTourStep
	...
	def before_save(self):
		if self.is_standard and not self.module:
			if self.workspace_name:
				self.module = itds.db.get_value("Workspace", self.workspace_name, "module")
			elif self.dashboard_name:
				dashboard_doctype = itds.db.get_value("Dashboard", self.dashboard_name, "module")
				self.module = itds.db.get_value("DocType", dashboard_doctype, "module")
			else:
				self.module = "Desk"
		if not self.ui_tour:
			meta = itds.get_meta(self.reference_doctype)
			for step in self.steps:
				if step.is_table_field and step.parent_fieldname:
					...
					field_df = itds.get_meta(step.child_doctype).get_field(step.fieldname)
...
def reset_tour(tour_name):
	for user in itds.get_all("User", pluck="name"):
		onboarding_status = itds.parse_json(itds.db.get_value("User", user, "onboarding_status"))
		onboarding_status.pop(tour_name, None)
		itds.db.set_value(
			"User", user, "onboarding_status", itds.as_json(onboarding_status), update_modified=False
		)
		itds.cache.hdel("bootinfo", user)

	itds.msgprint(_("Successfully reset onboarding status for all users."), alert=True)
...
def update_user_status(value, step):
	...
	step = itds.parse_json(step)
	tour = itds.parse_json(value)

	capture(
		itds.scrub(f"{step.parent}_{step.title}"),
		app="itds_ui_tours",
		properties={"is_completed": tour.is_completed},
	)
	itds.db.set_value("User", itds.session.user, "onboarding_status", value, update_modified=False)

	itds.cache.hdel("bootinfo", itds.session.user)
...
def get_onboarding_ui_tours():
	...
	ui_tours = itds.get_all("Form Tour", filters={"ui_tour": 1}, fields=["page_route", "name"])
