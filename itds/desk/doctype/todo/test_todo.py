...
import itds
from itds.core.doctype.doctype.doctype import clear_permissions_cache
from itds.model.db_query import DatabaseQuery
from itds.permissions import add_permission, reset_perms
...
class TestToDo(IntegrationTestCase):
	def test_delete(self):
		todo = itds.get_doc(doctype="ToDo", description="test todo", assigned_by="Administrator").insert()

		itds.db.delete("Deleted Document")
		todo.delete()

		deleted = itds.get_doc(
	...
	def test_fetch(self):
		todo = itds.get_doc(doctype="ToDo", description="test todo", assigned_by="Administrator").insert()
		self.assertEqual(
			todo.assigned_by_full_name, itds.db.get_value("User", todo.assigned_by, "full_name")
		)

	def test_fetch_setup(self):
		itds.db.delete("ToDo")

		todo_meta = itds.get_meta("ToDo")
	...
	def test_doc_read_access(self):
		...
		test_user = itds.get_doc("User", "test4@example.com")
...
def create_new_todo(description, assigned_by):
	...
	return itds.get_doc(todo).insert()
