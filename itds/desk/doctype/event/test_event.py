...
class TestEvent(IntegrationTestCase):
	...
	def test_allowed_public(self):
		itds.set_user(self.test_user)
		doc = itds.get_doc("Event", itds.db.get_value("Event", {"subject": "_Test Event 1"}))
		self.assertTrue(itds.has_permission("Event", doc=doc))

	def test_not_allowed_private(self):
		itds.set_user(self.test_user)
		doc = itds.get_doc("Event", itds.db.get_value("Event", {"subject": "_Test Event 2"}))
		self.assertFalse(itds.has_permission("Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = itds.db.get_value("Event", {"subject": "_Test Event 3"})
		itds.share.add("Event", name, self.test_user, "read")
		itds.set_user(self.test_user)
		doc = itds.get_doc("Event", name)
		self.assertTrue(itds.has_permission("Event", doc=doc))
		itds.set_user("Administrator")
		itds.share.remove("Event", name, self.test_user)
	...
	def test_revert_logic(self):
		ev = itds.get_doc(self.globalTestRecords["Event"][0]).insert()
		name = ev.name

		itds.delete_doc("Event", ev.name)
