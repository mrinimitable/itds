...
import itds.share
from itds import _
from itds.contacts.doctype.contact.contact import get_default_contact
...
from itds.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from itds.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			itds.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			itds.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		for communication in communications:
			itds.delete_doc("Communication", communication, force=True)
	...
	def sync_communication(self):
		...
		for participant in self.event_participants:
			if communications := itds.get_all(
				"Communication",
				filters=[
				...
				for comm in communications:
					communication = itds.get_doc("Communication", comm)
					self.update_communication(participant, communication)
			else:
				meta = itds.get_meta(participant.reference_doctype)
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				itds.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)
...
def delete_communication(event, reference_doctype, reference_docname):
	...
	deleted_participant = itds.get_doc(reference_doctype, reference_docname)
	...
	for comm in comms:
		itds.delete_doc("Communication", comm)


def get_permission_query_conditions(user):
	if not user:
		user = itds.session.user
	query = f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={itds.db.escape(user)})"""
	if shared_events := itds.share.get_shared("Event", user=user):
		query += f" or `tabEvent`.`name` in ({', '.join([itds.db.escape(e) for e in shared_events])})"
	return query
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			itds.set_user_lang(user.name, user.language)
			...
			itds.sendmail(
				...
				subject=itds._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[itds._("Events in Today's Calendar"), "blue"],
			)
...
def get_events(
	...
	EventLikeDict: TypeAlias = Event | itds._dict
	...
	event_candidates: list[EventLikeDict] = itds.db.sql(
		"""
...
def delete_events(ref_type, ref_name, delete_event=False):
	...
	if participations:
		for participation in participations:
			if delete_event:
				itds.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = itds.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					itds.db.delete("Event", {"name": participation.parent})
					itds.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = itds.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			itds.db.set_value("Event", event.name, "status", "Closed")
