...
import itds
from itds import _
from itds.build import scrub_html_template
from itds.model.meta import Meta
from itds.model.utils import render_include
from itds.modules import get_module_path, load_doctype_module, scrub
from itds.utils import get_shashi_path, get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not itds.conf.developer_mode
	key = f"doctype_form_meta::{doctype}"
	if cached:
		meta = itds.client_cache.get_value(key)
		if not meta:
		...
		meta = FormMeta(doctype, cached=itds.conf.developer_mode)
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(itds.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = itds.get_system_settings("country")
...
"""
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			itds.bold(df.fieldname), itds.bold(df.options)
		)
		...
		itds.throw(msg, title=_("Missing DocType"))

	def load_print_formats(self):
		print_formats = itds.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and itds.db.exists("Workflow", workflow_name):
			workflow = itds.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			workflow_docs.extend(itds.get_doc("Workflow State", d.state) for d in workflow.get("states"))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(itds.get_app_path(app, path))
	...
	def load_workspaces(self):
		Shortcut = itds.qb.DocType("Workspace Shortcut")
		Workspace = itds.qb.DocType("Workspace")
		shortcut = (
			itds.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = itds.qb.DocType("Workspace Link")
			link = (
				itds.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in itds.get_installed_apps():
		code_hook = itds.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = itds.get_app_path(app_name, *file.strip("/").split("/"))
