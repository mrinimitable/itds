...
from itds.desk.form.load import run_onload
...
from itds.utils.telemetry import capture_doc


@itds.whitelist(methods=["POST", "PUT"])
def savedocs(doc, action):
	"""save / submit / update doclist"""
	doc = itds.get_doc(json.loads(doc))
	...
	itds.msgprint(itds._(status_message), indicator="green", alert=True)
...
def cancel(doctype=None, name=None, workflow_state_fieldname=None, workflow_state=None):
	...
	doc.cancel()
	send_updated_docs(doc)
	itds.msgprint(itds._("Cancelled"), indicator="red", alert=True)
...
def discard(doctype: str, name: str | int):
	...
	doc.discard()
	send_updated_docs(doc)
	itds.msgprint(itds._("Discarded"), indicator="red", alert=True)
