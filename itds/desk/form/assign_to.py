...
import itds.utils
from itds import _
from itds.desk.doctype.notification_log.notification_log import (
	enqueue_create_notification,
	get_title,
	get_title_html,
)
from itds.desk.form.document_follow import follow_document
from itds.utils.data import strip_html


class DuplicateToDoError(itds.ValidationError):
	pass
...
def add(args=None, *, ignore_permissions=False):
	...
	for assign_to in itds.parse_json(args.get("assign_to")):
		filters = {
		...
		if not ignore_permissions:
			itds.get_doc(args["doctype"], args["name"]).check_permission()

		if itds.get_all("ToDo", filters=filters):
			users_with_duplicate_todo.append(assign_to)
		else:
			from itds.utils import nowdate
			...
			d = itds.get_doc(
				{
					...
					"assigned_by": args.get("assigned_by", itds.session.user),
					"assignment_rule": args.get("assignment_rule"),
				}
			).insert(ignore_permissions=True)

			# set assigned_to if field exists
			if itds.get_meta(args["doctype"]).get_field("assigned_to"):
				itds.db.set_value(args["doctype"], args["name"], "assigned_to", assign_to)

			doc = itds.get_doc(args["doctype"], args["name"])

			# if assignee does not have permissions, share or inform
			if not itds.has_permission(doc=doc, user=assign_to):
				if itds.get_system_settings("disable_document_sharing"):
					...
					itds.throw(msg, title=_("Missing Permission"))
				else:
					itds.share.add(doc.doctype, doc.name, assign_to)
					shared_with_users.append(assign_to)

			# make this document followed by assigned user
			if itds.get_cached_value("User", assign_to, "follow_assigned_documents"):
	...
	if users_with_duplicate_todo:
		...
		itds.msgprint(_("Already in the following Users ToDo list:{0}").format(user_list, alert=True))
...
def close(doctype: str, name: str, assign_to: str, ignore_permissions=False):
	if assign_to != itds.session.user:
		itds.throw(_("Only the assignee can complete this to-do."))
...
def set_status(doctype, name, todo=None, assign_to=None, status="Cancelled", ignore_permissions=False):
	...
	if not ignore_permissions:
		itds.get_doc(doctype, name).check_permission()
	try:
		if not todo:
			todo = itds.db.get_value(
				"ToDo",
				{
	...
	if itds.get_meta(doctype).get_field("assigned_to") and status in ("Cancelled", "Closed"):
		itds.db.set_value(doctype, name, "assigned_to", None)
...
def notify_assignment(assigned_by, allocated_to, doc_type, doc_name, action="CLOSE", description=None):
	...
	assigned_user = itds.db.get_value("User", allocated_to, ["language", "enabled"], as_dict=True)
	...
	user_name = itds.get_cached_value("User", itds.session.user, "full_name")
	...
	if action == "CLOSE":
		subject = _("Your assignment on {0} {1} has been removed by {2}", lang=assigned_user.language).format(
			itds.bold(_(doc_type)), get_title_html(title), itds.bold(user_name)
		)
	else:
		user_name = itds.bold(user_name)
		document_type = itds.bold(_(doc_type, lang=assigned_user.language))
