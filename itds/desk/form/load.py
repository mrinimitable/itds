...
from itds.desk.form.document_follow import is_document_followed
from itds.model.utils.user_settings import get_user_settings
from itds.permissions import check_doctype_permission, get_doc_permissions, has_permission
...
def getdoc(doctype, name):
	...
	if (key := itds.can_cache_doc((doctype, name))) and itds.cache.exists(key):
		itds._set_document_in_cache(key, doc)
	...
	doc.add_seen()
	set_link_titles(doc)
	if itds.response.docs is None:
		itds.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False):
	...
	if with_parent and (parent_dt := itds.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		itds.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	itds.response["user_settings"] = get_user_settings(parent_dt or doctype)

	itds.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [itds.desk.form.meta.get_meta(doctype)]
	bundle.extend(
		itds.desk.form.meta.get_meta(df.options)
		for df in bundle[0].fields
		if df.fieldtype in itds.model.table_fields
	)
	return bundle
...
def get_docinfo(doc=None, doctype=None, name=None):
	from itds.share import _get_users as get_docshares
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, itds.session.user),
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		match c.comment_type:
			case "Comment":
				c.content = itds.utils.markdown(c.content)
...
def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[itds._dict]:
...
def get_tags(doctype: str, name: str) -> str:
	from itds.desk.doctype.tag_link.tag_link import has_tags
...
def get_document_email(doctype, name):
	from itds.email.doctype.email_account.email_account import get_automatic_email_link
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = itds.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(itds.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		meta = itds.get_meta(doctype) if doctype else None
		if not meta or not meta.title_field or not meta.show_title_field_in_link:
			continue

		link_title = itds.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def send_link_titles(link_titles):
	"""Append link titles dict in `itds.local.response`."""
	if "_link_titles" not in itds.local.response:
		itds.local.response["_link_titles"] = {}

	itds.local.response["_link_titles"].update(link_titles)
...
def update_user_info(docinfo):
	...
	itds.utils.add_user_info(users, docinfo.user_info)
...
def get_user_info_for_viewers(users):
	...
	for user in json.loads(users):
		itds.utils.add_user_info(user, user_info)
