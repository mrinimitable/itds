...
def get_notifications():
	...
	if itds.flags.in_install or not itds.is_setup_complete():
		return out
	...
	for name in groups:
		count = itds.cache.hget("notification_count:" + name, itds.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = itds.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						...
						result = itds.get_attr(condition)()

				except itds.PermissionError:
					itds.clear_messages()
					pass
				# itds.msgprint("Permission Error in notifications for {0}".format(d))
					...
					itds.cache.hset("notification_count:" + d, itds.session.user, result)
...
def clear_notifications(user=None):
	...
	if user:
		itds.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		itds.cache.delete_value([f"notification_count:{name}" for name in groups])


def clear_notification_config(user):
	itds.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	itds.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(itds.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	user = itds.session.user or "Guest"
	...
	return itds.cache.hget("notification_config", user, _get)
...
def _get_linked_document_counts(doctype: str, name: str, items=None):
	...
	if not meta.custom:
		module = itds.get_meta_module(doctype)
...
def get_doc_count(doctype, filters) -> int | Literal["?"]:
	try:
		...
		if itds.db.is_statement_timeout(e):  # Skip fetching correct count if it's too slow
			return "?"
		raise
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(itds.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			itds.bold(sender_fullname), itds.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = itds.get_cached_doc("User Group", mention["data-id"])
