...
# Author - Shivam Mishra <shivam@itds.io>

from functools import wraps
from json import JSONDecodeError, dumps, loads

import itds
from itds import DoesNotExistError, ValidationError, _, _dict
from itds.boot import get_allowed_pages, get_allowed_reports
from itds.cache_manager import (
...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.workspace_manager = "Workspace Manager" in itds.get_roles()

		self.user = itds.get_user()
		self.allowed_modules = self.get_cached("user_allowed_modules", self.get_allowed_modules)

		self.doc = itds.get_cached_doc("Workspace", self.page_name)
		if (
			self.doc
			and self.doc.module
		...
		self.restricted_doctypes = (
			itds.cache.get_value("domain_restricted_doctypes") or build_domain_restricted_doctype_cache()
		)
		self.restricted_pages = (
			itds.cache.get_value("domain_restricted_pages") or build_domain_restricted_page_cache()
		)
	...
	def get_cached(self, cache_key, fallback_fn):
		value = itds.cache.get_value(cache_key, user=itds.session.user)
		...
		itds.cache.set_value(cache_key, value, itds.session.user, 21600)
		return value
	...
	def get_onboarding_doc(self, onboarding):
		...
		if not itds.get_system_settings("enable_onboarding"):
			return None

		if not self.onboarding_list:
			return None

		if itds.db.get_value("Module Onboarding", onboarding, "is_complete"):
			return None

		doc = itds.get_doc("Module Onboarding", onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(itds.get_roles())
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and itds.db.exists(name):
			if not itds.db.get_value("DocType", name, "issingle"):
				exists = bool(itds.get_all(name, limit=1))
			else:
				exists = True
	...
	def _prepare_item(self, item):
		...
		if item.get("link_type") == "DocType":
			item["description"] = itds.get_meta(item.link_to).description
	...
	def is_custom_block_permitted(self, custom_block_name):
		...
		allowed = [
			d.role for d in itds.get_all("Has Role", fields=["role"], filters={"parent": custom_block_name})
		]
	...
	def get_links(self):
		...
		default_country = itds.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if itds.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts

			for chart in charts:
				if itds.has_permission("Dashboard Chart", doc=chart.chart_name):
					# Translate label
	...
	def get_shortcuts(self):
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in itds.get_active_domains()
	...
	def get_onboarding_steps(self, onboarding_doc):
		...
		for doc in onboarding_doc.get_steps():
			...
			if step.action == "Create Entry":
				step.is_submittable = itds.db.get_value(
	...
	def get_number_cards(self):
		all_number_cards = []
		if itds.has_permission("Number Card", throw=False):
			number_cards = self.doc.number_cards
			for number_card in number_cards:
				if itds.has_permission("Number Card", doc=number_card.number_card_name):
					# Translate label
					number_card.label = (
	...
	def get_custom_blocks(self):
		all_custom_blocks = []
		if itds.has_permission("Custom HTML Block", throw=False):
			custom_blocks = self.doc.custom_blocks

			for custom_block in custom_blocks:
				if itds.has_permission("Custom HTML Block", doc=custom_block.custom_block_name):
...
def get_desktop_page(page):
	...
	try:
		...
		itds.log_error("Workspace Missing")
		return {}
...
def get_workspace_sidebar_items():
	...
	has_access = "Workspace Manager" in itds.get_roles()

	# don't get domain restricted pages
	blocked_modules = itds.get_cached_doc("User", itds.session.user).get_blocked_modules()
	blocked_modules.append("Dummy Module")

	# adding None to allowed_domains to include pages without domain restriction
	allowed_domains = [None, *itds.get_active_domains()]
	...
	try:
		workspace_visibilty = loads(
			itds.db.get_single_value("Workspace Settings", "workspace_visibility_json") or "{}"
		)
		...
		try:
			...
			if has_access or workspace.is_permitted():
				if page.public and (has_access or not page.is_hidden) and page.title != "Welcome Workspace":
					pages.append(page)
				elif page.for_user == itds.session.user:
			...
			if not page["app"] and page["module"]:
				page["app"] = itds.db.get_value("Module Def", page["module"], "app_name") or get_module_app(
					page["module"]
				)
			if page["link_type"] == "Report":
				report_type, ref_doctype = itds.db.get_value(
	...
	return {
		"workspace_setup_completed": itds.db.get_single_value(
			"Workspace Settings", "workspace_setup_completed"
		),
		"pages": pages,
		"has_access": has_access,
		"has_create_access": itds.has_permission(doctype="Workspace", ptype="create"),
	}


def get_table_with_counts():
	counts = itds.cache.get_value("information_schema:counts")
	if not counts:
...
def update_onboarding_step(name, field, value):
	...
	itds.db.set_value("Onboarding Step", name, field, value)

	capture(itds.scrub(name), app="itds_onboarding", properties={field: value})
