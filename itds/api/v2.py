...
import itds
import itds.client
from itds import _, cint, cstr, get_newargs, is_whitelisted
from itds.core.doctype.server_script.server_script_utils import get_server_script_map
from itds.handler import is_valid_http_method, run_server_script, upload_file
...
def handle_rpc_call(method: str, doctype: str | None = None):
	from itds.modules.utils import load_doctype_module
	...
	method = itds.override_whitelisted_method(method)
	...
	try:
		...
		itds.throw(_("Failed to get method {0} with {1}").format(method, e))
...
def document_list(doctype: str) -> list[dict[str, Any]]:
	...
	Query Parameters (accessible via itds.form_dict):
	...
	Response:
		itds.response["data"]: List of document records as dicts
		itds.response["has_next_page"]: Indicates if more pages are available
	...
	Controller Customization:
		...
		Example:
			class Project(Document):
				@staticmethod
				def get_list(query):
					Project = itds.qb.DocType("Project")
					if user_has_role("Project Owner"):
						query = query.where(Project.owner == itds.session.user)
					else:
	...
	args = itds.form_dict
	fields: list | None = itds.parse_json(args.get("fields", None))
	filters: dict | None = itds.parse_json(args.get("filters", None))
	...
	if hasattr(controller, "get_list"):
		try:
			...
			itds.throw(_("Error in {0}.get_list: {1}").format(doctype, str(e)))

	data = query.run(as_dict=as_dict, debug=debug)
	itds.response["has_next_page"] = len(data) > limit
	return data[:limit]


def count(doctype: str) -> int:
	from itds.desk.reportview import get_count
...
def copy_doc(doctype: str, name: str, ignore_no_copy: bool = True):
	...
	copy = itds.copy_doc(doc, ignore_no_copy=ignore_no_copy)
...
def delete_doc(doctype: str, name: str):
	itds.client.delete_doc(doctype, name)
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	doc.check_permission(PERMISSION_MAP[itds.request.method])
	result = doc.run_method(method, **itds.form_dict)
	itds.response.docs.append(doc.as_dict())
	return result
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	if isinstance(document, str):
		document = itds.parse_json(document)

	if kwargs is None:
		kwargs = {}

	doc = itds.get_doc(document)
	...
	itds.response.docs.append(doc)  # send modified document and result both.
	return response
