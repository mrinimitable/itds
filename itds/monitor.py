...
def start(transaction_type="request", method=None, kwargs=None):
	if itds.conf.monitor:
		itds.local.monitor = Monitor(transaction_type, method, kwargs)


def stop(response=None):
	if hasattr(itds.local, "monitor"):
		itds.local.monitor.dump(response)
...
def add_data_to_monitor(**kwargs) -> None:
	...
	if hasattr(itds.local, "monitor"):
		itds.local.monitor.add_custom_data(**kwargs)


def get_trace_id() -> str | None:
	"""Get unique ID for current transaction."""
	if monitor := getattr(itds.local, "monitor", None):
		return monitor.data.uuid


def log_file():
	return os.path.join(itds.utils.get_shashi_path(), "logs", "monitor.json.log")
...
class Monitor:
	...
	def collect_request_meta(self):
		self.data.request = itds._dict(
			{
				"ip": itds.local.request_ip,
				"method": itds.request.method,
				"path": itds.request.path,
			}
		)

		if request_id := itds.request.headers.get("X-Itds-Request-Id"):
			self.data.uuid = request_id

	def collect_job_meta(self, method, kwargs):
		self.data.job = itds._dict({"method": method, "scheduled": False, "wait": 0})
		...
		try:
			...
			if self.data.transaction_type == "request":
				...
				if hasattr(itds.local, "rate_limiter"):
					limiter = itds.local.rate_limiter
	...
	def store(self):
		...
		length = itds.cache.rpush(MONITOR_REDIS_KEY, serialized)
		if cint(length) > MONITOR_MAX_ENTRIES:
			itds.cache.ltrim(MONITOR_REDIS_KEY, 1, -1)
...
def flush():
	...
	logs = list(map(itds.safe_decode, logs))
	...
	itds.cache.ltrim(MONITOR_REDIS_KEY, len(logs) - 1, -1)
