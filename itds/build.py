...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "itds", "dist", type)
	...
	assets_json = itds.read_file("assets/assets.json")
	if assets_json:
		assets_json = itds.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/itds"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["itds/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="itds")


def get_assets_link(itds_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/itds && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(itds_head)
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/mrinimitable/itds/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.itdsframework.com/{itds_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {itds_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, itds_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="itds-assets-", suffix=itds_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Itds assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./itds-shashi/sites", "")
...
def download_itds_assets(verbose=True) -> bool:
	"""Download and set up Itds assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	itds_head = getoutput("cd ../apps/itds && git rev-parse HEAD")

	if not itds_head:
		return False

	try:
		url = get_assets_link(itds_head)
		assets_archive = fetch_assets(url, itds_head)
		...
		# TODO: log traceback in shashi.log
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{itds.generate_hash()}"
...
def setup():
	...
	for app in itds.get_all_apps(True):
		try:
			pymodules.append(itds.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(itds.local.sites_path, "assets")
...
def bundle(
	...
	itds_app_path = itds.get_app_source_path("itds")
	itds.commands.popen(command, cwd=itds_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = itds.utils.cint(os.environ.get("LIVE_RELOAD", itds.conf.live_reload))
	...
	check_node_executable()
	itds_app_path = itds.get_app_source_path("itds")
	itds.commands.popen(command, cwd=itds_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in itds.get_all_apps():
		app_doc_path = None

		pymodule = itds.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `itds.templates`."""
	return """itds.templates["{key}"] = '{content}';\n""".format(
