"""
This module provides functionality for discovering and organizing tests in the Itds framework.
...
logger = logging.getLogger("itds.testing.discovery")
...
def discover_doctype_tests(doctypes: list[str], runner, app: str, force: bool = False) -> "TestRunner":
	...
	for doctype in doctypes:
		try:
			...
			doctype_app = itds.db.get_value("Module Def", module, "app_name")
			if app is None:
				_app = doctype_app
			...
			test_module = itds.modules.utils.get_module_name(doctype, module, "test_")
			force and itds.db.delete(doctype)
	...
	for test in runner._iterate_suite(test_suite):
		...
		match test:
			...
			case _:
				...
				if any(b.__name__ == "ItdsTestCase" for b in test.__class__.__bases__):
					from itds.deprecation_dumpster import deprecation_warning

					deprecation_warning(
						"2024-20-08",
						"v17",
						"accurate categorization of ItdsTestCase will be removed from this runner",
					)
					category = "old-itds-test-class-category"
