...
import itds
from itds.modules.utils import get_module_name
from itds.testing.config import TestParameters


class ItdsTestLoader(unittest.TestLoader):
	...
	def discover_tests(self, params: TestParameters) -> unittest.TestSuite:
		...
		if self.params.tests:
			...
			files = self.get_files(itds.get_installed_apps())
			...
			module = itds.get_cached_value("DocType", self.params.doctype, "module")
			app = itds.get_cached_value("Module Def", module, "app_name")
