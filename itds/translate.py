...
import itds
from itds.query_builder import DocType, Field
from itds.utils import cstr, get_shashi_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `itds.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = itds.session.user != "Guest"

	# fetch language from form_dict
	if itds.form_dict._lang:
		language = get_lang_code(itds.form_dict._lang or get_parent_language(itds.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(itds.request.accept_languages.values())
	...
	return itds.get_system_settings("language") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set itds.local.lang from user preferences on session beginning or resumption"""
	user = user or itds.session.user

	# User.language => Session Defaults => itds.local.lang => 'en'
	return (
		itds.get_cached_value("User", user, "language")
		or itds.get_system_settings("language")
		or itds.local.lang
		or "en"
	)
...
def get_lang_code(lang: str) -> str | None:
	return itds.db.get_value("Language", {"name": lang}) or itds.db.get_value(
...
def set_default_language(lang):
	...
	if itds.db.get_default("lang") != lang:
		itds.db.set_default("lang", lang)
	itds.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		itds.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(itds.local.lang)


@itds.whitelist(allow_guest=True)
def get_app_translations():
	if itds.session.user != "Guest":
		language = itds.db.get_value("User", itds.session.user, "language")
	else:
		language = itds.db.get_single_value("System Settings", "language")
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	try:
		return itds.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		...
		itds.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from itds.gettext.translate import get_translations_from_mo

	for app in apps or itds.get_installed_apps(_ensure_on_shashi=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(itds.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				itds.log_error(message=msg, title="Error in translation file")
				if throw:
					itds.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return itds.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`itds.cache`"""
	itds.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [itds.unscrub(m) for m in itds.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			itds.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			itds.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = itds.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from itds.gettext.extractors.utils import is_translatable

	messages = []
	meta = itds.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = itds.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = itds.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = itds.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Workflow":
				workflows = itds.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(itds.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = itds.db.get_values(
			document_state,
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`itds.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`itds.core.doctype.Report`."""
	from itds.gettext.extractors.utils import is_translatable

	report = itds.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, itds.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = itds.db.get_value(doctype, name, "module")

	doc_path = itds.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, itds.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`itds._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(itds.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	from itds.utils.jinja_globals import bundled_asset

	messages = []
	app_include_js = itds.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = itds.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(itds.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else itds.get_installed_apps(_ensure_on_shashi=True):
		if os.path.exists(itds.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(itds.get_app_path(app, "public")):  # noqa: B007
				if "itds/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from itds.gettext.extractors.utils import extract_messages_from_code

	itds.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in itds.flags.scanned_files:
		return []

	itds.flags.scanned_files.add(path)
	...
	with open(path) as sourcefile:
		...
		return [
			(os.path.relpath(path, shashi_path), message, context, line)
...
def extract_messages_from_javascript_code(code: str) -> list[tuple[int, str, str | None]]:
	...
	messages = []
	from itds.gettext.extractors.javascript import extract_javascript
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		itds.get_file_items(untranslated_file, ignore_empty_lines=False),
		itds.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in itds.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in itds.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in itds.translate.get_messages_for_app(target_app)]
	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = itds.translate.get_all_languages()

	source_app_translations_dir = itds.get_app_path(source_app, "translations")
	target_app_translations_dir = itds.get_app_path(target_app, "translations")
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = itds.get_app_path(app, "translations")
	itds.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `itds.local.response`"""
	if "__messages" not in itds.local.response:
		itds.local.response["__messages"] = {}

	itds.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = itds.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_all_language_with_name():
		return itds.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return itds.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		languages = itds.client_cache.get_value("languages")
		if not languages:
			languages = itds.get_all("Language", filters={"enabled": 1}, pluck="name")
			itds.client_cache.set_value("languages", languages)
		return languages


def get_preferred_language_cookie():
	return itds.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = itds.get_all("DocType", {"translated_doctype": 1}, pluck="name")
	custom_dts = itds.get_all(
...
def print_language(language: str):
	...
	```
	"""
	if not language or language == itds.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = itds.local.lang
	_jenv = itds.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	itds.local.lang = language
	itds.local.jenv = None

	yield

	# restore original values
	itds.local.lang = _lang
	itds.local.jenv = _jenv
