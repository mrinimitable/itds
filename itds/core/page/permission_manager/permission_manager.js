itds.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = itds.ui.make_app_page({
		parent: wrapper,
	...
	$(itds.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new itds.PermissionEngine(wrapper);
};

itds.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

itds.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			change: function () {
				itds.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return itds.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (itds.get_route()[1]) {
			this.doctype_select.set_value(itds.get_route()[1]);
		} else if (itds.route_options) {
			if (itds.route_options.doctype) {
				this.doctype_select.set_value(itds.route_options.doctype);
			}
			if (itds.route_options.role) {
				this.role_select.set_value(itds.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = itds.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return itds
				.call({
					module: "itds.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(itds.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${itds.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return itds.call({
					...
					callback: (r) => {
						if (r.exc) {
							itds.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			itds.route_options = { allow: this.get_doctype() || "" };
	...
	get_link_fields(doctype) {
		return itds.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
