...
from itds.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		itds.cache.hdel("user_permissions", self.user)
		itds.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_default_permission(self):
		...
		if overlap_exists:
			ref_link = itds.get_desk_link(self.doctype, overlap_exists[0].name)
			itds.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if itds.request and itds.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = itds.cache.hget("user_permissions", user)
	...
	try:
		for perm in itds.get_all(
			...
			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = itds.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False, False)

		out = itds._dict(out)
		itds.cache.hset("user_permissions", user, out)
	except itds.db.SQLError as e:
...
def clear_user_permissions(user, for_doctype):
	itds.only_for("System Manager")
	total = itds.db.count("User Permission", {"user": user, "allow": for_doctype})
...
def insert_user_perm(
	...
	user_perm = itds.new_doc("User Permission")
	user_perm.user = user
