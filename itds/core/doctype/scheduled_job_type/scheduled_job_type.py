...
from itds.utils import get_datetime, now_datetime
from itds.utils.background_jobs import enqueue, is_job_enqueued
...
class ScheduledJobType(Document):
	...
	def validate(self):
		...
		if self.frequency == "Cron":
			if not self.cron_format:
				itds.throw(_("Cron format is required for job types with Cron frequency."))
			try:
	...
	def enqueue(self, force=False) -> bool:
		...
		if self.is_event_due() or force:
			if not self.is_job_in_queue():
				enqueue(
					"itds.core.doctype.scheduled_job_type.scheduled_job_type.run_scheduled_job",
				...
				itds.logger("scheduler").error(
					f"Skipped queueing {self.method} because it was found in queue for {itds.local.site}"
				)
	...
	def get_next_execution(self):
		...
		maintenance_offset = int(hashlib.sha1(itds.local.site.encode()).hexdigest(), 16) % 60
		...
		CRON_MAP = {
			...
			"All": f"*/{(itds.get_conf().scheduler_interval or 240) // 60} * * * *",
		}
	...
	def execute(self):
		if itds.job:
			itds.job.frequency = self.frequency
			itds.job.cron_format = self.cron_format

		self.scheduler_log = None
		try:
			self.log_status("Start")
			if self.server_script:
				script_name = itds.db.get_value("Server Script", self.server_script)
				if script_name:
					itds.get_doc("Server Script", script_name).execute_scheduled_method()
			else:
				itds.get_attr(self.method)()
			itds.db.commit()
	...
	def log_status(self, status):
		# log file
		itds.logger("scheduler").info(f"Scheduled Job {status}: {self.method} for {itds.local.site}")
	...
	def update_scheduler_log(self, status):
		...
		if not self.scheduler_log:
			self.scheduler_log = itds.get_doc(
		...
		if status == "Failed":
			self.scheduler_log.db_set("details", itds.get_traceback(with_context=True))
		if status == "Start":
			self.db_set("last_execution", now_datetime(), update_modified=False)
		itds.db.commit()
	...
	def on_trash(self):
		itds.db.delete("Scheduled Job Log", {"scheduled_job_type": self.name})
...
def execute_event(doc: str):
	...
	doc = json.loads(doc)
	itds.get_doc("Scheduled Job Type", doc.get("name")).enqueue(force=True)
	return doc
...
def skip_next_execution(doc: str):
	itds.only_for("System Manager")
	doc = json.loads(doc)
	doc: ScheduledJobType = itds.get_doc("Scheduled Job Type", doc.get("name"))
...
def run_scheduled_job(scheduled_job_type: str, job_type: str | None = None):
	...
	if itds.conf.maintenance_mode:
		raise itds.InReadOnlyMode("Scheduled jobs can't run in maintenance mode.")
	try:
		itds.get_doc("Scheduled Job Type", scheduled_job_type).execute()
	except Exception:
		print(itds.get_traceback())


def sync_jobs(hooks: dict | None = None):
	itds.reload_doc("core", "doctype", "scheduled_job_type")
	scheduler_events = hooks or itds.get_hooks("scheduler_events")
...
def insert_single_event(frequency: str, event: str, cron_format: str | None = ""):
	...
	if job_name := itds.db.exists("Scheduled Job Type", {"method": event}):
		doc = itds.get_doc("Scheduled Job Type", job_name)
