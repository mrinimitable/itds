...
class TestScheduledJobType(IntegrationTestCase):
	def setUp(self):
		itds.db.rollback()
		itds.db.truncate("Scheduled Job Type")
		sync_jobs()
		itds.db.commit()
	...
	def test_throws_on_duplicate_job(self):
		job_config = dict(
			...
			method="itds.desk.notifications.clear_notifications",
			frequency="Weekly",
		)
		itds.get_doc(job_config).insert()

		duplicate_job = itds.get_doc(job_config)
	...
	def test_sync_jobs(self):
		all_job = itds.get_doc("Scheduled Job Type", dict(method="itds.email.queue.flush"))
		self.assertEqual(all_job.frequency, "All")

		daily_job = itds.get_doc(
			"Scheduled Job Type", dict(method="itds.desk.notifications.clear_notifications")
		)
		self.assertEqual(daily_job.frequency, "Daily Maintenance")

		# check if cron jobs are synced
		cron_job = itds.get_doc("Scheduled Job Type", dict(method="itds.deferred_insert.save_to_db"))
		...
		updated_scheduler_events = {"hourly": ["itds.email.queue.flush"]}
		sync_jobs(updated_scheduler_events)
		updated_scheduled_job = itds.get_doc("Scheduled Job Type", {"method": "itds.email.queue.flush"})
	...
	def test_weekly_job(self):
		job = itds.get_doc(
			"Scheduled Job Type",
			dict(method="itds.desk.form.document_follow.send_weekly_updates"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_monthly_job(self):
		job = itds.get_doc(
			"Scheduled Job Type",
			dict(method="itds.email.doctype.auto_email_report.auto_email_report.send_monthly"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cron_job(self):
		# runs every 10 mins
		job = itds.get_doc(
			"Scheduled Job Type", dict(method="itds.email.doctype.email_account.email_account.pull")
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cold_start(self):
		...
		job = itds.new_doc("Scheduled Job Type")
		job.frequency = "Daily"
