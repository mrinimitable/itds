...
from itds.utils import cint, get_fullname
...
class DocShare(Document):
	...
	def get_doc(self):
		if not getattr(self, "_doc", None):
			self._doc = itds.get_lazy_doc(self.share_doctype, self.share_name)
		return self._doc
	...
	def validate_user(self):
		if self.everyone:
			...
			itds.throw(_("User is mandatory for Share"), itds.MandatoryError)

	def check_share_permission(self):
		if not self.flags.ignore_share_permission and not itds.has_permission(
			self.share_doctype, "share", self.get_doc()
		):
			itds.throw(_('You need to have "Share" permission'), itds.PermissionError)

	def check_is_submittable(self):
		if self.submit and not cint(itds.db.get_value("DocType", self.share_doctype, "is_submittable")):
			itds.throw(
				_("Cannot share {0} with submit permission as the doctype {1} is not submittable").format(
					itds.bold(self.share_name), itds.bold(self.share_doctype)
				)
			)
...
def on_doctype_update():
	...
	itds.db.add_index("DocShare", ["user", "share_doctype"])
	itds.db.add_index("DocShare", ["share_doctype", "share_name"])
