...
from itds.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(IntegrationTestCase):
	...
	def test_exports_specified_fields(self):
		if not itds.db.exists(doctype_name, "Test"):
			doc = itds.get_doc(
			...
			doc = itds.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(itds.response["doctype"], doctype_name)
