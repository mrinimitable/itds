...
import itds
from itds import _
from itds.model import display_fieldtypes, no_value_fields
from itds.model import table_fields as table_fieldtypes
from itds.utils import flt, format_duration, groupby_metric
from itds.utils.csvutils import build_csv_response
from itds.utils.xlsxutils import build_xlsx_response
...
class Exporter:
	...
	def serialize_exportable_fields(self):
		...
		for key, exportable_fields in self.exportable_fields.items():
			for _df in exportable_fields:
				...
				if isinstance(_df, itds.core.doctype.docfield.docfield.DocField):
					df = _df.as_dict()
				else:
					df = _df.copy()
	...
	def get_data_to_export(self):
		...
		if not itds.permissions.can_export(self.doctype):
			if itds.permissions.can_export(self.doctype, is_owner=True):
				for doc in data:
					if doc.get("owner") != itds.session.user:
