...
import itds
from itds import _
from itds.core.doctype.data_import.exporter import Exporter
from itds.core.doctype.data_import.importer import Importer
from itds.model import CORE_DOCTYPES
from itds.model.document import Document
from itds.modules.import_file import import_file_by_path
from itds.utils.background_jobs import enqueue, is_job_enqueued
from itds.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def set_delimiters_flag(self):
		if self.import_file:
			itds.flags.delimiter_options = self.delimiter_options or ","

	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			itds.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		run_now = itds.in_test or itds.conf.developer_mode
		if is_scheduler_inactive() and not run_now:
			itds.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
	...
	def on_trash(self):
		itds.db.delete("Data Import Log", {"data_import": self.name})
...
def get_preview_from_template(
	...
	di: DataImport = itds.get_doc("Data Import", data_import)
...
def start_import(data_import):
	...
	data_import = itds.get_doc("Data Import", data_import)
	try:
	...
	itds.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(doctype, export_fields=None, export_records=None, export_filters=None, file_type="CSV"):
	...
	"""
	itds.has_permission(doctype, "read", throw=True)

	export_fields = itds.parse_json(export_fields)
	export_filters = itds.parse_json(export_filters)
...
def download_errored_template(data_import_name: str):
	data_import: DataImport = itds.get_doc("Data Import", data_import_name)
...
def get_import_logs(data_import: str):
	doc = itds.get_doc("Data Import", data_import)
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	...
	data_import = itds.new_doc("Data Import")
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(itds.get_doc(doctype, name).as_dict())
	elif itds.db.get_value("DocType", doctype, "issingle"):
		out.append(itds.get_doc(doctype).as_dict())
	else:
		for doc in itds.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(itds.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(itds.as_json(out, ensure_ascii=False))


def export_csv(doctype, path):
	from itds.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(itds.response.result.encode("utf-8"))
