itds.ui.form.on("System Settings", {
	refresh: function (frm) {
		itds.call({
			method: "itds.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				itds.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", itds.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					itds.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					itds.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, itds.boot.sysdefaults.language, itds.boot.user.language],
			[frm.doc.rounding_method, itds.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == itds.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		itds.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", itds.boot.sysdefaults.rounding_method);
			}
		);
	},
