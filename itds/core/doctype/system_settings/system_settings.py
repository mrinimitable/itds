...
from itds.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from itds.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			itds.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				itds.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not itds.db.get_single_value("SMS Settings", "sms_gateway_url"):
					itds.throw(
		...
		itds.flags.update_last_reset_password_date = False
		if self.force_user_to_reset_password and not cint(
			itds.db.get_single_value("System Settings", "force_user_to_reset_password")
		):
			itds.flags.update_last_reset_password_date = True
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = itds.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = itds.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = itds.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			itds.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		if itds.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from itds.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				itds.db.set_default(df.fieldname, self.get(df.fieldname))
...
@itds.whitelist()
def load():
	from itds.utils.momentjs import get_all_timezones

	if "System Manager" not in itds.get_roles():
		itds.throw(_("Not permitted"), itds.PermissionError)

	all_defaults = itds.db.get_defaults()
	defaults = {}

	for df in itds.get_meta("System Settings").get("fields"):
...
def get_system_settings(key: str):
	...
	if not (system_settings := getattr(itds.local, "system_settings", None)):
		try:
			system_settings = itds.client_cache.get_doc("System Settings")
			itds.local.system_settings = system_settings
		except itds.DoesNotExistError:  # possible during new install
...
def clear_system_settings_cache():
	itds.client_cache.delete_value(itds.get_document_cache_key("System Settings", "System Settings"))
	itds.cache.delete_value("system_settings")
	itds.cache.delete_value("time_zone")


def sync_system_settings():
	if itds.db.get_single_value("System Settings", "currency") is None:
		itds.db.set_single_value("System Settings", "currency", itds.defaults.get_defaults()["currency"])
