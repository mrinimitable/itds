...
class LogSettings(Document):
	...
	if TYPE_CHECKING:
		from itds.core.doctype.logs_to_clear.logs_to_clear import LogsToClear
	...
	def remove_unsupported_doctypes(self):
		for entry in list(self.logs_to_clear):
			...
			msg = _("{} does not support automated log clearing.").format(itds.bold(entry.ref_doctype))
			if itds.conf.developer_mode:
				msg += "<br>" + _("Implement `clear_old_logs` method to enable auto error clearing.")
			itds.msgprint(msg, title=_("DocType not supported by Log Settings."))
			self.remove(entry)
	...
	def add_default_logtypes(self):
		...
		added_logtypes = set()
		default_logtypes_retention = itds.get_hooks("default_log_clearing_doctypes", {})
		...
		for logtype, retentions in default_logtypes_retention.items():
			if logtype not in existing_logtypes and _supports_log_clearing(logtype):
				if not itds.db.exists("DocType", logtype):
					continue
		...
		if added_logtypes:
			itds.msgprint(_("Added default log doctypes: {}").format(",".join(added_logtypes)), alert=True)
	...
	def clear_logs(self):
		...
		for entry in self.logs_to_clear:
			...
			kwargs = itds.get_newargs(func, {"days": entry.days})
			func(**kwargs)
			itds.db.commit()
...
def has_unseen_error_log():
	if itds.get_all("Error Log", filters={"seen": 0}, limit=1):
		return {
			"show_alert": True,
...
def get_log_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	doctypes = itds.get_list("DocType", filters=filters, pluck="name")
...
def clear_log_table(doctype, days=90):
	...
	try:
		itds.db.sql_ddl(f"CREATE TABLE `{temporary}` LIKE `{original}`")
		...
		itds.db.sql_ddl(f"RENAME TABLE `{original}` TO `{backup}`, `{temporary}` TO `{original}`")
	except Exception:
		itds.db.rollback()
		itds.db.sql_ddl(f"DROP TABLE IF EXISTS `{temporary}`")
		raise
	else:
		itds.db.sql_ddl(f"DROP TABLE `{backup}`")
