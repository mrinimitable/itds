itds.listview_settings["DocType"] = {
	...
	new_doctype_dialog(args) {
		...
		let non_developer = itds.session.user !== "Administrator" || !itds.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new itds.ui.Dialog({
			...
			primary_action(values) {
				...
				itds.db
					...
					.then((doc) => {
						itds.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (itds.get_route()[0] === "Form") {
