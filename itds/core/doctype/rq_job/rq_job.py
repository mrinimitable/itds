...
from itds.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			itds.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> itds._dict:
	...
	if job_name == "itds.utils.background_jobs.run_doc_method":
	...
	return itds._dict(
		...
		exc_info=exc_info,
		arguments=itds.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == itds.local.site
...
@itds.whitelist()
def stop_job(job_id):
	itds.get_doc("RQ Job", job_id).stop_job()
