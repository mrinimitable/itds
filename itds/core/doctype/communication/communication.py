...
import itds
from itds import _
from itds.automation.doctype.assignment_rule.assignment_rule import (
	apply as apply_assignment_rule,
)
from itds.contacts.doctype.contact.contact import get_contact_name
from itds.core.doctype.comment.comment import update_comment_in_doc
from itds.core.doctype.communication.email import validate_email
from itds.core.doctype.communication.mixins import CommunicationEmailMixin
from itds.core.utils import get_parent_doc
...
class Communication(Document, CommunicationEmailMixin):
	...
	if TYPE_CHECKING:
		from itds.core.doctype.communication_link.communication_link import CommunicationLink
	...
	def onload(self):
		...
		if (
			...
			email_flag_queue = itds.db.get_value(
	...
	def after_insert(self):
		...
		if self.reference_doctype == "Communication" and self.sent_or_received == "Sent":
			itds.db.set_value("Communication", self.reference_name, "status", "Replied")
	...
	def set_signature_in_email_content(self):
		...
		signature = user_email_signature or itds.db.get_value(
			"Email Account",
	...
	def mark_email_as_spam(self):
		if (
			...
			and self.communication_medium == "Email"
			and self.sent_or_received == "Received"
			and itds.db.exists("Email Rule", {"email_id": self.sender, "is_spam": 1})
		):
	...
	def find_one_by_filters(cls, *, order_by=None, **kwargs):
		name = itds.db.get_value(cls.DOCTYPE, kwargs, order_by=order_by)
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				self.sender_full_name = itds.db.get_value("User", "Administrator", "full_name")
				self.sender = itds.db.get_value("User", "Administrator", "email")
				...
				if not self.sender_full_name:
					self.sender_full_name = itds.db.get_value("User", self.sender, "full_name")

				if not self.sender_full_name:
					first_name, last_name = itds.db.get_value(
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(
			itds.get_all("Email Queue", pluck="status", filters={"communication": self.name})
		)
	...
	def parse_email_for_timeline_links(self):
		if not itds.db.get_value("Email Account", filters={"enable_automatic_linking": 1}):
			return
		...
		for doctype, docname in parse_email([self.recipients, self.cc, self.bcc]):
			...
			doctype = itds.db.get_value("DocType", doctype)
			if doctype:
				docname = itds.db.get_value(doctype, docname, ignore=True)
	...
	def set_timeline_links(self):
		contacts = []
		create_contact_enabled = self.email_account and itds.db.get_value(
...
def on_doctype_update():
	...
	itds.db.add_index("Communication", ["reference_doctype", "reference_name"])
	itds.db.add_index("Communication", ["status", "communication_type"])
	itds.db.add_index("Communication", ["message_id(140)"])
...
def get_contacts(email_strings: list[str], auto_create_contact=False) -> list[str]:
	...
	for email in email_addrs:
		...
		if not contact_name and email and auto_create_contact:
			...
			first_name = itds.unscrub(email_parts[0])
...
def parse_email(email_strings):
	...
	for email_string in email_strings:
		...
		for email in email_string.split(","):
			...
			doctype = itds.unscrub(unquote_plus(document_parts[0]))
...
def get_email_without_link(email):
	...
	see: RFC5233
	"""
	if not itds.get_all("Email Account", filters={"enable_automatic_linking": 1}):
		return email
...
def update_first_response_time(parent, communication):
	if parent.meta.has_field("first_response_time") and not parent.get("first_response_time"):
		if (
			...
			or itds.get_cached_value("User", itds.session.user, "user_type") == "System User"
		):
			if (
