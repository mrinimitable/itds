...
def make(
	...
	**kwargs,
	...
	"""
	if kwargs:
		from itds.utils.commands import warn

		warn(
			f"Options {kwargs} used in itds.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name:
		itds.has_permission(doctype, doc=name, ptype="email", throw=True)
...
def _make(
	...
	sender = sender or get_formatted_email(itds.session.user)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = itds.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by itds.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
...
def mark_email_as_seen(name: str | None = None):
	...
	itds.response.update(itds.utils.get_imaginary_pixel_response())
...
def _mark_email_as_seen(name):
	try:
		...
		itds.log_error("Unable to mark as seen", None, "Communication", name)
...
def update_communication_as_read(name):
	...
	communication = itds.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
