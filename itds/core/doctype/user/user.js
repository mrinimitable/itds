itds.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, itds.session.user]],
	...
	before_load: function (frm) {
		let update_tz_options = function () {
			frm.fields_dict.time_zone.set_data(itds.all_timezones);
		};

		if (!itds.all_timezones) {
			itds.call({
				method: "itds.core.doctype.user.user.get_timezones",
				callback: function (r) {
					itds.all_timezones = r.message.timezones;
					update_tz_options();
				},
			});
		} else {
	...
	module_profile: function (frm) {
		if (frm.doc.module_profile) {
			itds.call({
				method: "itds.core.doctype.user.user.get_module_profile",
				args: {
	...
	onload: function (frm) {
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				frm.roles_editor = new itds.RoleEditor(
					role_area,
					frm,
				...
				if (frm.doc.user_type == "System User") {
					...
					frm.module_editor = new itds.ModuleEditor(frm, module_area);
				}
			} else {
				frm.roles_editor.show();
			}
	...
	refresh: function (frm) {
		...
		itds.xcall("itds.apps.get_apps").then((r) => {
			let apps = r?.map((r) => r.name) || [];
			frm.set_df_property("default_app", "options", [" ", ...apps]);
		});

		if (frm.is_new()) {
			frm.set_value("time_zone", itds.sys_defaults.time_zone);
		}
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						itds.set_route("query-report", "Permitted Documents For User", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
				...
				frm.add_custom_button(
					...
					() =>
						itds.set_route("query-report", "User Doctype Permissions", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
			...
			frm.add_custom_button(
				...
				function () {
					itds.call({
						method: "itds.core.doctype.user.user.reset_password",
						args: {
							user: frm.doc.name,
						},
					});
			...
			if (itds.user.has_role("System Manager")) {
				itds.db.get_single_value("LDAP Settings", "enabled").then((value) => {
								...
								const d = new itds.ui.Dialog({
									...
									primary_action: (values) => {
										...
										if (values.new_password !== values.confirm_password) {
											itds.throw(__("Passwords do not match!"));
										}
										itds.call(
											"itds.integrations.doctype.ldap_settings.ldap_settings.reset_password",
											{
												user: frm.doc.email,
			...
			if (
				cint(itds.boot.sysdefaults.enable_two_factor_auth) &&
				(itds.session.user == doc.name || itds.user.has_role("System Manager"))
			) {
				frm.add_custom_button(
					__("Reset OTP Secret"),
					function () {
						itds.call({
							method: "itds.twofactor.reset_otp_secret",
							args: {
								user: frm.doc.name,
							},
						});
			...
			if (itds.session.user == doc.name) {
				// update display settings
				if (doc.user_image) {
					itds.boot.user_info[itds.session.user].image = itds.utils.get_file_link(
						doc.user_image
					);
				}
			}
		}
		if (frm.doc.user_emails && itds.model.can_create("Email Account")) {
			var found = 0;
			...
			if (!found) {
				frm.add_custom_button(__("Create User Email"), function () {
					if (!frm.doc.email) {
						itds.msgprint(__("Email is mandatory to create User Email"));
						return;
					}
		...
		if (itds.route_flags.unsaved === 1) {
			delete itds.route_flags.unsaved;
	...
	create_user_email: function (frm) {
		itds.call({
			method: "itds.core.doctype.user.user.has_email_account",
			args: {
				email: frm.doc.email,
			},
			callback: function (r) {
				if (!Array.isArray(r.message) || !r.message.length) {
					...
					itds.model.with_doctype("Email Account", function (doc) {
						doc = itds.model.get_new_doc("Email Account");
						itds.route_flags.linked_user = frm.doc.name;
						itds.route_flags.delete_user_from_locals = true;
						itds.set_route("Form", "Email Account", doc.name);
					});
				} else {
					itds.route_flags.create_user_account = frm.doc.name;
					itds.set_route("Form", "Email Account", r.message[0]["name"]);
				}
			},
		});
	},
	generate_keys: function (frm) {
		itds.call({
			method: "itds.core.doctype.user.user.generate_keys",
			args: {
				user: frm.doc.name,
			},
			callback: function (r) {
				if (r.message) {
					itds.msgprint(__("Save API Secret: {0}", [r.message.api_secret]));
					frm.reload_doc();
				}
			},
		});
	...
	after_save: function (frm) {
		...
		if (doc.name === itds.session.user && attr_tuples.some(has_effectively_changed)) {
			itds.msgprint(__("Refreshing..."));
		...
		if (
			...
			frm.add_custom_button(__("Impersonate"), () => {
				...
				itds.prompt(
					...
					(values) => {
						itds
							.xcall("itds.core.doctype.user.user.impersonate", {
								user: frm.doc.name,
...
function has_access_to_edit_user() {
	return has_common(itds.user_roles, get_roles_for_editing_user());
}
