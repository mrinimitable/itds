...
import itds.share
from itds import STANDARD_USERS, _, msgprint, throw
from itds.apps import get_default_path
from itds.auth import MAX_PASSWORD_SIZE
from itds.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from itds.sessions import clear_sessions
...
from itds.utils.data import sha256_hash
from itds.utils.html_utils import sanitize_html
from itds.utils.password import check_password, get_password_reset_limit
from itds.utils.password import update_password as _update_password
from itds.utils.user import get_system_managers
from itds.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from itds.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from itds.core.doctype.has_role.has_role import HasRole
		from itds.core.doctype.user_email.user_email import UserEmail
		from itds.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from itds.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def after_insert(self):
		...
		itds.cache.delete_key("users_for_mentions")
		itds.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("itds")):
			self.set_social_login_userid("itds", itds.generate_hash(length=39))
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = itds.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = itds.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			itds.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		itds.clear_cache(user=self.name)
		now = itds.in_test or itds.flags.in_install
		self.send_password_notification(self.__new_password)
		itds.enqueue(
			"itds.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			itds.enqueue(
				"itds.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			itds.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = itds.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						itds.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					itds.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == itds.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			itds.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(itds.local, "login_manager", None):
			itds.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not itds.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = itds.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			itds.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		itds.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def send_password_notification(self, new_password):
		...
		except itds.OutgoingEmailError:
	...
	def password_reset_mail(self, link):
		reset_password_template = itds.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = itds.get_hooks("welcome_email")
		if method:
			subject = itds.get_attr(method[-1])()
		if not subject:
			site_name = itds.db.get_default("site_name") or itds.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = itds.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from itds.utils import get_url
		from itds.utils.user import get_user_fullname

		created_by = get_user_fullname(itds.session["user"])
		if created_by == "Guest":
		...
		sender = (
			itds.session.user not in STANDARD_USERS and get_formatted_email(itds.session.user)
		) or None

		if custom_template:
			from itds.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(itds.local, "login_manager", None):
			itds.local.login_manager.logout(user=self.name)

		# delete todos
		itds.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			itds.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		itds.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		itds.db.delete("DocShare", {"user": self.name})
		# unlink contact
		table = DocType("Contact")
		itds.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		itds.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		itds.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		itds.db.delete("OAuth Authorization Code", {"user": self.name})
		itds.db.delete("Token Cache", {"user": self.name})

		# Remove user link from Workflow Action
		itds.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		itds.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = itds.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = itds.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			itds.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		itds.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = itds.db.get_tables()
		for tab in tables:
			desc = itds.db.get_table_columns_description(tab)
		...
		if itds.db.exists("Notification Settings", old_name):
			itds.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		itds.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in itds.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = itds.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				itds.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(itds.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(itds.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			itds.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return itds.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			itds.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(itds.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(itds.get_system_settings("allow_login_using_user_name"))
		...
		users = itds.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
	...
	def check_roles_added(self):
		...
		itds.msgprint(
			_("Newly created user {0} has no roles enabled.").format(itds.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "itds.set_route",
...
def get_all_roles():
	...
	roles = itds.get_all(
		"Role",
		filters={
			"name": ("not in", itds.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return itds.get_roles(itds.form_dict.get("uid", itds.session.user))


@itds.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from itds.permissions import get_all_perms
...
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		itds.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		itds.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or itds.get_system_settings("logout_on_password_reset")
	...
	redirect_to = itds.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		itds.cache.hdel("redirect_after_login", user)

	itds.local.login_manager.login_as(user)

	itds.db.set_value("User", user, "last_password_reset_date", today())
	itds.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	from itds.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = itds.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = itds.db.get_value(
			"User", itds.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(itds.get_system_settings("minimum_password_score"))
...
def has_email_account(email: str):
	return itds.get_list("Email Account", filters={"email_id": email})


@itds.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	from itds.utils import set_default

	password_list = itds.get_all(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				itds.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		itds.local.login_manager.check_password(itds.session.user, old_password)
...
def reset_user_data(user):
	user_doc = itds.get_doc("User", user)
...
def verify_password(password):
	itds.local.login_manager.check_password(itds.session.user, password)


@itds.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		itds.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = itds.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if itds.db.get_creation_count("User", 60) > 300:
		...
		from itds.utils import random_string
		...
		default_role = itds.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			itds.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = itds.get_doc("User", user)
		...
		itds.local.response["http_status_code"] = 404
		itds.clear_messages()
		return "not found"
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters:
		if not (filters.get("ignore_user_type") and itds.session.data.user_type == "System User"):
...
def get_website_users():
	...
	return itds.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(itds.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and itds.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(itds.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = itds.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	itds.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		itds.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if itds.db.get_creation_count("User", 60) > itds.local.conf.get("throttle_user_limit", 60):
		itds.throw(_("Throttled"))


@itds.whitelist()
def get_module_profile(module_profile: str):
	module_profile = itds.get_doc("Module Profile", {"module_profile_name": module_profile})
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	if not contact_name:
		...
		try:
			contact = itds.get_doc("Contact", contact_name)
		...
		except itds.TimestampMismatchError:
			raise itds.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = itds.get_doc("User", user)
	api_secret = itds.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = itds.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		itds.db.set_value("User", itds.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = itds.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return itds.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = itds.session.user
	itds.get_doc(
		{
