...
class TestUserType(IntegrationTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = itds.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = itds.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = itds.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if itds.db.exists("User Type", user_type):
		itds.delete_doc("User Type", user_type)

	user_type_limit = {itds.scrub(user_type): 1}
...
def create_role():
	if not itds.db.exists("Role", "_Test User Type"):
		itds.get_doc(
