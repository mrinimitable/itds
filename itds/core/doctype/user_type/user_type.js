...
itds.ui.form.on("User Type", {
	refresh: function (frm) {
		if (frm.is_new() && !itds.boot.developer_mode) frm.set_value("is_standard", 1);
		...
		frm.set_query("apply_user_permission_on", function () {
			return {
				query: "itds.core.doctype.user_type.user_type.get_user_linked_doctypes",
			};
		});
	},
	...
	get_user_id_fields: function (frm) {
		if (frm.doc.apply_user_permission_on) {
			itds.call({
				method: "itds.core.doctype.user_type.user_type.get_user_id",
				args: {
