itds.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			itds.xcall("itds.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new itds.ui.FileUploader({
				...
				on_success: (file) => {
					...
					itds.call({
						method: "itds.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			itds.call({
				method: "itds.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return itds.xcall("itds.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		itds.xcall("itds.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		itds.prompt(
			[
				...
				},
				{
					...
					description: `This will be used for filtering paths which will be recorded.
						...
						e.g. <code>/api/method/okayblue</code>. Leave it empty to record every request.`,
				},
				{
					fieldtype: "Column Break",
			...
			(values) => {
				itds.xcall("itds.recorder.start", values).then(() => {
					listview.refresh();
