...
from itds.custom.doctype.customize_form.customize_form import reset_customization
from itds.desk.query_report import add_total_row, run, save_report
from itds.desk.reportview import delete_report
from itds.desk.reportview import save_report as _save_report
...
class TestReport(IntegrationTestCase):
	...
	def test_report_builder(self):
		if itds.db.exists("Report", "User Activity Report"):
			itds.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			itds.get_doc(json.loads(f.read())).insert()

		report = itds.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = itds.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			itds.set_user("test_report_owner@example.com")
			self.assertRaises(itds.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = itds.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=itds.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = itds.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		...
		if not itds.db.exists("Role", "Test Has Role"):
			itds.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		# create report "Test Report"
		if not itds.db.exists("Report", "Test Report"):
			report = itds.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = itds.get_doc("Report", "Test Report")

		with self.set_user("test@example.com"):
			# remove role "Test Has Role" from user if found
			itds.db.delete("Has Role", {"parent": itds.session.user, "role": "Test Has Role"})
			self.assertNotEqual(report.is_permitted(), True)

	def test_report_custom_permissions(self):
		# delete custom role if exists
		itds.db.delete("Custom Role", {"report": "Test Custom Role Report"})

		# create report if not exists
		if not itds.db.exists("Report", "Test Custom Role Report"):
			report = itds.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = itds.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if itds.db.exists("Report", "User Activity Report Without Sort"):
			itds.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			itds.get_doc(json.loads(f.read())).insert()

		report = itds.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not itds.db.exists("Report", report_name):
			report = itds.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in itds.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if itds.db.exists("Report", report_name):
			itds.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = itds.get_all(doctype="Report", limit=1)
		...
		doc = itds.get_doc("Report", report_name)
		with self.assertRaises(itds.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if itds.db.db_type == "mariadb":
			...
			self.assertRaises(itds.PermissionError, report.execute_query_report, filters={})
