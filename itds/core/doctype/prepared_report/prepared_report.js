...
itds.ui.form.on("Prepared Report", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.status == "Completed") {
			frm.page.set_primary_action(__("Show Report"), () => {
				itds.route_options = { prepared_report_name: frm.doc.name };
				itds.set_route("query-report", frm.doc.report_name);
			});
			let csv_attached = (frm.get_files() || []).some((f) => f.file_url.endsWith(".csv"));
			if (!csv_attached) {
				frm.add_custom_button(__("Download as CSV"), function () {
					itds.call({
						method: "itds.core.doctype.prepared_report.prepared_report.enqueue_json_to_csv_conversion",
						args: {
