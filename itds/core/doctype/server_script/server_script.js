...
itds.ui.form.on("Server Script", {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new itds.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}

		setTimeout(() => {
			itds
				.call("itds.core.doctype.server_script.server_script.get_autocompletion_items")
				.then((r) => r.message)
				.then((items) => {
	...
	check_safe_exec(frm) {
		itds.xcall("itds.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://itdsframework.com/docs/user/en/desk/scripting/server-script";
...
<p>Payment processing events have a special state. See the <a href="https://github.com/mrinimitable/payments/blob/develop/payments/controllers/payment_controller.py">PaymentController in Itds Payments</a> for details.</p>
<pre>
	<code>
...
if itds.form_dict.message == "ping":
	itds.response['message'] = "pong"
else:
	itds.response['message'] = "ok"
</code></pre>
