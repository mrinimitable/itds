...
class DomainSettings(Document):
	...
	if TYPE_CHECKING:
		from itds.core.doctype.has_domain.has_domain import HasDomain
	...
	def on_update(self):
		for i, d in enumerate(self.active_domains):
			...
			if i >= 1:
				itds.flags.keep_desktop_icons = True
			domain = itds.get_doc("Domain", d.domain)
			domain.setup_domain()
	...
	def restrict_roles_and_modules(self):
		...
		all_domains = list(itds.get_hooks("domains") or {})

		def remove_role(role):
			itds.db.delete("Has Role", {"role": role})
			itds.set_value("Role", role, "disabled", 1)

		for domain in all_domains:
			data = itds.get_domain_data(domain)
			if not itds.db.get_value("Domain", domain):
				itds.get_doc(doctype="Domain", domain=domain).insert()
			if "modules" in data:
				for module in data.get("modules"):
					itds.db.set_value("Module Def", module, "restrict_to_domain", domain)

			if "restricted_roles" in data:
				for role in data["restricted_roles"]:
					if not itds.db.get_value("Role", role):
						itds.get_doc(doctype="Role", role_name=role).insert()
					itds.db.set_value("Role", role, "restrict_to_domain", domain)
			...
			if "custom_fields" in data:
				if domain not in active_domains:
					inactive_domain = itds.get_doc("Domain", domain)
...
def get_active_domains():
	...
	return itds.cache.get_value("active_domains", _get_active_domains)
...
def get_active_modules():
	...
	def _get_active_modules():
		...
		for m in itds.get_all("Module Def", fields=["name", "restrict_to_domain"]):
			if (not m.restrict_to_domain) or (m.restrict_to_domain in active_domains):
				active_modules.append(m.name)
		return active_modules

	return itds.cache.get_value("active_modules", _get_active_modules)
