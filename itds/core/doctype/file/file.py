...
import itds
from itds import _
from itds.database.schema import SPECIAL_CHAR_PATTERN
from itds.exceptions import DoesNotExistError
from itds.model.document import Document
from itds.permissions import SYSTEM_USER_ROLE, get_doctypes_with_read
from itds.utils import call_hook_method, cint, get_files_path, get_hook_method, get_url
from itds.utils.file_manager import is_safe_path
...
class File(Document):
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = itds.generate_hash(length=10)
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			itds.db.after_rollback.add(self.on_rollback)
	...
	def validate(self):
		...
		self.file_size = itds.form_dict.file_size or self.file_size
	...
	def validate_attachment_references(self):
		...
		if not self.attached_to_name or not isinstance(self.attached_to_name, str | int):
			itds.throw(_("Attached To Name must be a string or an integer"), itds.ValidationError)

		if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(self.attached_to_field):
			itds.throw(_("The fieldname you've specified in Attached To Field is invalid"))
	...
	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			itds.throw(_("Cannot delete Home and Attachments folders"))
	...
	def get_successors(self):
		return itds.get_all("File", filters={"folder": self.name}, pluck="name")
	...
	def handle_is_private_changed(self):
		...
		private_file_path = Path(itds.get_site_path("private", "files", file_name))
		public_file_path = Path(itds.get_site_path("public", "files", file_name))
		...
		if itds.get_meta(self.attached_to_doctype).issingle:
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = itds.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(itds.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				itds.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						itds.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype:
			self.folder = itds.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file_on_disk(self):
		...
		if not os.path.exists(full_path):
			itds.throw(_("File {0} does not exist").format(self.file_url), IOError)

	def validate_file_extension(self):
		# Only validate uploaded files, not generated by code/integrations.
		if not self.file_type or not itds.request:
			return

		allowed_extensions = itds.get_system_settings("allowed_file_extensions")
		if not allowed_extensions:
			return

		if self.file_type not in allowed_extensions.splitlines():
			itds.throw(_("File type of {0} is not allowed").format(self.file_type), exc=FileTypeNotAllowed)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = itds.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = itds.get_cached_doc("File", duplicate_file.name)
	...
	def set_file_name(self):
		if not self.file_name and not self.file_url:
			itds.throw(
				_("Fields `file_name` or `file_url` must be set for File"), exc=itds.MandatoryError
			)
	...
	def generate_content_hash(self):
		...
		try:
			...
			itds.throw(_("File {0} does not exist").format(file_path))
	...
	def make_thumbnail(
		...
		path = os.path.abspath(itds.get_site_path("public", thumbnail_url.lstrip("/")))
		...
		try:
			...
			itds.msgprint(_("Unable to write file format for {0}").format(path))
			return
	...
	def validate_empty_folder(self):
		...
		if self.is_folder and itds.get_all("File", filters={"folder": self.name}, limit=1):
			itds.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)
	...
	def validate_protected_file(self):
		...
		try:
			ref_doc = itds.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def _delete_file_on_disk(self):
		...
		on_disk_file_not_shared = self.content_hash and not itds.get_all(
			"File",
			filters={
				"content_hash": self.content_hash,
				"name": ["!=", self.name],
	...
	def unzip(self) -> list["File"]:
		...
		if not self.file_url.endswith(".zip"):
			itds.throw(_("{0} is not a zip file").format(self.file_name))
		...
		with zipfile.ZipFile(zip_path) as z:
			for file in z.filelist:
				...
				file_doc = itds.new_doc("File")
				try:
					file_doc.content = z.read(file.filename)
				except zipfile.BadZipFile:
					itds.throw(_("{0} is a not a valid zip file").format(self.file_name))
		...
		itds.delete_doc("File", self.name)
		return files
	...
	def get_content(self, encodings=None) -> bytes | str:
		if self.is_folder:
			itds.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if file_path.startswith("/private/files/"):
			...
			itds.throw(_("There is some problem with the file url: {0}").format(file_path))

		if not is_safe_path(file_path):
			itds.throw(_("Cannot access file path {0}").format(file_path))

		if os.path.sep in self.file_name:
			itds.throw(_("File name cannot have {0}").format(os.path.sep))
	...
	def save_file(
		...
		if (
			...
			and itds.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if duplicate_file:
			file_doc: File = itds.get_cached_doc("File", duplicate_file.name)
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			...
			if itds.has_permission("System Settings", "write"):
				msg += ".<br>" + _("You can increase the limit from System Settings.")
			itds.throw(msg, exc=MaxFileSizeReachedError)
	...
	def create_attachment_record(self):
		...
		file_url = quote(itds.safe_encode(self.file_url), safe="/:") if self.file_url else self.file_name
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = itds.get_doc(self.attached_to_doctype, self.attached_to_name)
...
def on_doctype_update():
	itds.db.add_index("File", ["attached_to_doctype", "attached_to_name"])
	itds.db.add_index("File", ["file_url(100)"])
...
def has_permission(doc, ptype=None, user=None, debug=False):
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		try:
			ref_doc = itds.get_doc(attached_to_doctype, attached_to_name)
...
def get_permission_query_conditions(user: str | None = None) -> str:
	...
	if SYSTEM_USER_ROLE not in itds.get_roles(user):
		return f""" `tabFile`.`owner` = {itds.db.escape(user)} """

	readable_doctypes = ", ".join(repr(dt) for dt in get_doctypes_with_read())
	return f"""
		(`tabFile`.`is_private` = 0)
		OR (`tabFile`.`attached_to_doctype` IS NULL AND `tabFile`.`owner` = {itds.db.escape(user)})
