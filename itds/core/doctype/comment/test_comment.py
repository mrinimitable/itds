...
import itds
from itds.templates.includes.comments.comments import add_comment
from itds.tests import IntegrationTestCase
from itds.tests.test_model_utils import set_user
from itds.website.doctype.blog_post.test_blog_post import make_test_blog


class TestComment(IntegrationTestCase):
	def test_comment_creation(self):
		test_doc = itds.get_doc(doctype="ToDo", description="test")
		test_doc.insert()
		...
		# Check comment count
		counts = itds.get_all("ToDo", {"name": test_doc.name}, ["*"], with_comment_count=True)
	...
	def test_public_comment(self):
		...
		itds.db.delete("Comment", {"reference_doctype": "Blog Post"})
		add_comment_args = {
	...
	def test_user_not_logged_in(self):
		some_system_user = itds.db.get_value("User", {"name": ("not in", itds.STANDARD_USERS)})
