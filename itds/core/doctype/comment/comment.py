...
from itds.desk.notifications import notify_mentions
from itds.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = itds.session.user
		self.content = itds.utils.sanitize_html(self.content, always_sanitize=True)
...
def on_doctype_update():
	itds.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = itds.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if itds.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or itds.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if itds.db.is_missing_column(e) and getattr(itds.local, "request", None):
			pass
		elif itds.db.is_data_too_long(e):
			raise itds.DataTooLongException
		else:
			raise
	else:
		...
		# Clear route cache
		if route := itds.get_cached_value(reference_doctype, reference_name, "route"):
			clear_cache(route)
