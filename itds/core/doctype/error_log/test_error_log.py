...
from itds.utils.error import _is_ldap_exception, guess_exception_source


class TestErrorLog(IntegrationTestCase):
	def test_error_log(self):
		"""let's do an error log on error log?"""
		doc = itds.new_doc("Error Log")
...
_RAW_EXC = """
   File "apps/itds/itds/model/document.py", line 1284, in runner
     add_to_return_value(self, fn(self, *args, **kwargs))
                               ^^^^^^^^^^^^^^^^^^^^^^^^^
   File "apps/itds/itds/model/document.py", line 933, in fn
...
_THROW_EXC = """
   ...
   File "apps/okayblue/okayblue/selling/doctype/sales_order/sales_order.py", line 58, in onload
     itds.throw("what")
   File "apps/itds/itds/__init__.py", line 550, in throw
     msgprint(
   File "apps/itds/itds/__init__.py", line 518, in msgprint
     _raise_exception()
   File "apps/itds/itds/__init__.py", line 467, in _raise_exception
     raise raise_exception(msg)
 itds.exceptions.ValidationError: what
"""
...
class TestExceptionSourceGuessing(IntegrationTestCase):
	@patch.object(itds, "get_installed_apps", return_value=["itds", "okayblue", "3pa"])
