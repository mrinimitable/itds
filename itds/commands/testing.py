...
if TYPE_CHECKING:
	from itds.testing import TestRunner
...
def main(
	...
	if lightmode:
		from itds.testing.config import TestParameters
	...
	from itds.testing.environment import _cleanup_after_tests, _initialize_test_environment
	...
	testing_module_logger = logging.getLogger("itds.testing")
	...
	try:
		...
		if doctype or doctype_list_path:
			...
			apps = [app] if app else itds.get_installed_apps()
...
def run_tests_in_light_mode(test_params):
	from itds.testing.loader import ItdsTestLoader
	from itds.testing.result import ItdsTestResult
	from itds.tests.utils import toggle_test_mode

	# init environment
	itds.init(test_params.site)
	if not itds.db:
		itds.connect()

	# disable scheduler
	global scheduler_disabled_by_user
	scheduler_disabled_by_user = itds.utils.scheduler.is_scheduler_disabled(verbose=False)
	if not scheduler_disabled_by_user:
		itds.utils.scheduler.disable_scheduler()
	itds.clear_cache()

	toggle_test_mode(True)
	suite = ItdsTestLoader().discover_tests(test_params)
	result = unittest.TextTestRunner(failfast=test_params.failfast, resultclass=ItdsTestResult).run(suite)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(itds.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from itds.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = itds.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from itds.modules import get_module_name
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		itds.init(site)
		allow_tests = itds.conf.allow_tests
...
@click.option("--app", help="For App", default="itds")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app) as cc:
		site = get_site(context)
		if use_orchestrator:
			from itds.parallel_test_runner import ParallelTestWithOrchestrator

			runner = ParallelTestWithOrchestrator(app, site=site)
		else:
			from itds.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	site = get_site(context)
	itds.init(site)
	app_base_path = itds.get_app_source_path(app)
	site_url = itds.utils.get_site_url(site)
	admin_password = itds.get_conf().admin_password
	...
	node_bin = subprocess.getoutput("(cd ../itds && yarn bin)")
	...
	if not (
		...
		itds.commands.popen(f"(cd ../itds && yarn add {packages} --no-lockfile)")
	...
	try:
		itds.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
