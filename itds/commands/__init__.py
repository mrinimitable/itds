...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			itds.exceptions.SiteNotSpecifiedError,
			itds.exceptions.IncorrectSitePath,
			itds.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise itds.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://itdsframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
