...
def build_message_files(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			itds.init(site)
			itds.connect()
			itds.translate.rebuild_all_translation_files()
		finally:
			itds.destroy()
...
@click.argument("app")  # , help="App name eg. itds")
@pass_context
def new_language(context: CliCtxObj, lang_code, app):
	...
	# init site
	itds.init(site=context.sites[0])
	itds.connect()
	itds.translate.write_translations_file(app, lang_code)

	print(f"File created at ./apps/{app}/{app}/translations/{lang_code}.csv")
	print("You will need to add the language in itds/geo/languages.csv, if you haven't done it already.")
...
def get_untranslated(context: CliCtxObj, lang, untranslated_file, app="_ALL_APPS", all=None):
	...
	try:
		itds.init(site)
		itds.connect()
		itds.translate.get_untranslated(lang, untranslated_file, get_all=all, app=app)
	finally:
		itds.destroy()
...
def update_translations(context: CliCtxObj, lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	try:
		itds.init(site)
		itds.connect()
		itds.translate.update_translations(lang, untranslated_file, translated_file, app=app)
	finally:
		itds.destroy()
...
def import_translations(context: CliCtxObj, lang, path):
	...
	try:
		itds.init(site)
		itds.connect()
		itds.translate.import_translations(lang, path)
	finally:
		itds.destroy()
...
def migrate_translations(context: CliCtxObj, source_app, target_app):
	...
	try:
		itds.init(site)
		itds.connect()
		itds.translate.migrate_translations(source_app, target_app)
	finally:
		itds.destroy()
