...
import itds
import itds.commands.scheduler
import itds.commands.site
import itds.commands.utils
import itds.recorder
from itds.installer import add_to_installed_apps, remove_app
...
from itds.utils import add_to_date, get_shashi_path, get_shashi_relative_path, now
from itds.utils.backups import BackupGenerator, fetch_latest_backups
from itds.utils.jinja_globals import bundled_asset
from itds.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QK.test"  # added random string tag to avoid collisions
CLI_CONTEXT = itds._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if itds.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = itds.local.site
	pre_flags = itds.local.flags.copy()
	pre_db = itds.local.db

	try:
		yield
	finally:
		post_site = getattr(itds.local, "site", None)
		if not post_site or post_site != pre_site:
			itds.init(pre_site)
			itds.local.db = pre_db
			itds.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("itds.commands.pass_context", pass_test_context)
...
class BaseTestCommands(IntegrationTestCase):
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": itds.conf.admin_password,
			"db_type": itds.conf.db_type,
			"db_root_username": itds.conf.root_login,
			"db_root_password": itds.conf.root_password,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("shashi --site {site} execute itds.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command accessing a normal attribute
		self.execute("shashi --site {site} execute itds.local.site")
		self.assertEqual(self.returncode, 0)
		self.assertIsNotNone(self.stderr)

		# test 3: execute a command expecting an errored output as lacol won't exist
		self.execute("shashi --site {site} execute itds.lacol.site")
		self.assertEqual(self.returncode, 1)
		self.assertIsNotNone(self.stderr)

		# test 4: execute a command with kwargs
		self.execute(
			"shashi --site {site} execute itds.bold --kwargs '{put_here}'",
			{"put_here": '{"text": "DocType"}'},  # avoid escaping errors
		)
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout, itds.bold(text="DocType"))
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": itds.conf.admin_password,
			"root_login": itds.conf.root_login,
			"root_password": itds.conf.mariadb_root_password or itds.conf.root_password,
			"db_type": itds.conf.db_type,
		}
		...
		with self.switch_site(TEST_SITE):
			public_file = itds.new_doc(
				"File", file_name=f"test_{itds.generate_hash()}", content=itds.generate_hash()
			).insert()
			private_file = itds.new_doc(
				"File", file_name=f"test_{itds.generate_hash()}", content=itds.generate_hash()
			).insert()
		...
		self.execute(
			"shashi --site {test_site} execute itds.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		with self.switch_site(TEST_SITE):
			...
			itds.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		...
		with self.switch_site(TEST_SITE):
			self.assertTrue(itds.db.table_exists("ToDo", cached=False))
		...
		self.execute(
			"shashi --site {test_site} execute" " itds.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
	...
	def test_partial_restore(self):
			...
			itds.get_doc(
				{
					...
					"description": itds.mock("paragraph"),
				}
			).insert()
		itds.db.commit()
		todo_count = itds.db.count("ToDo")
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(itds.db.count("ToDo"), todo_count)
	...
	def test_recorder(self):
		...
		itds.local.cache = {}
		self.assertEqual(itds.recorder.status(), True)

		self.execute("shashi --site {site} stop-recording")
		itds.local.cache = {}
		self.assertEqual(itds.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = itds.get_single("Installed Applications").installed_applications
		if doctype:
			installed_apps = {x.app_name for x in doctype}
		else:
			installed_apps = set(itds.get_installed_apps())
	...
	def test_itds_site_env(self):
		os.putenv("ITDS_SITE", itds.local.site)
		self.execute("shashi execute itds.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		from itds.utils.password import check_password
		...
		original_password = itds.conf.admin_password or "admin"
	...
	@skipIf(
		not (itds.conf.root_password and itds.conf.admin_password and itds.conf.db_type == "mariadb"),
	...
	def test_shashi_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {itds.conf.admin_password} "
			f"--db-root-username {itds.conf.root_login} "
			f"--db-root-password {itds.conf.root_password} "
			f"--db-type {itds.conf.db_type} "
		)
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when itds force installs, we don't have the conf
		self.execute(f"shashi --site {TEST_SITE} set-config admin_password {itds.conf.admin_password}")

		# try installing the itds_docs app again on test site
		self.execute(f"shashi --site {TEST_SITE} install-app {app_name}")
		self.assertIn(f"{app_name} already installed", self.stdout)
		self.assertEqual(self.returncode, 0)

		# force install itds_docs app on the test site
	...
	def test_set_global_conf(self):
		key = "answer"
		value = itds.generate_hash()
		_ = itds.get_site_config()
		self.execute(f"shashi set-config {key} {value} -g")
		conf = itds.get_site_config()
	...
	def test_different_db_username(self):
		...
		kwargs = {
			...
			"db_root_username": itds.conf.root_login,
			"db_root_password": itds.conf.root_password or "",
		}
		self.execute(
	...
	def test_existing_db_username(self):
		...
		if itds.conf.db_type == "mariadb":
			from itds.database.mariadb.setup_db import get_root_connection

			root_conn = get_root_connection()
			root_conn.sql(f"CREATE USER '{user}'@'localhost'")
		else:
			from itds.database.postgres.setup_db import get_root_connection
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = itds.utils.get_site_path("private", "backups")
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			itds.conf.db_name,
			itds.conf.db_name,
			itds.conf.db_password + "INCORRECT PASSWORD",
			db_socket=itds.conf.db_socket,
			db_host=itds.conf.db_host,
			db_port=itds.conf.db_port,
			db_type=itds.conf.db_type,
		)
	...
	def test_clear_log_table(self):
		d = itds.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("creation", "2010-01-01", update_modified=False)
		itds.db.commit()

		tables_before = itds.db.get_tables(cached=False)

		self.execute("shashi --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		itds.db.commit()

		self.assertFalse(itds.db.exists("Error Log", d.name))
		tables_after = itds.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `itds.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`itds.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with itds.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`itds.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring itds.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(IntegrationTestCase):
	def test_delete_modules(self):
		from itds.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = itds.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "itds"})
		test_module.save()

		module_def_linked_doctype = itds.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(itds.db.exists("Module Def", test_module.module_name))
		self.assertFalse(itds.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("itds", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(itds.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		self.assertEqual(self.returncode, 0)
		user = itds.get_doc("User", "test@gmail.com")
...
class TestShashiBuild(IntegrationTestCase):
	def test_build_assets_size_check(self):
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / itds.local.sites_path / bundled_asset(chunk)[1:]
...
class TestDBUtils(BaseTestCommands):
	...
	def test_db_add_index(self):
		...
		itds.db.rollback()
		index_name = itds.db.get_index_name((field,))
		self.assertTrue(itds.db.has_index("tabUser", index_name))
		meta = itds.get_meta("User", cached=False)
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(itds.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)


class TestCommandUtils(IntegrationTestCase):
	def test_shashi_helper(self):
		from itds.utils.shashi_helper import get_app_groups

		app_groups = get_app_groups()
		self.assertIn("itds", app_groups)
		self.assertIsInstance(app_groups["itds"], click.Group)


class TestDBCli(BaseTestCommands):
	@timeout(10)
	def test_db_cli(self):
		if itds.conf.db_type == "sqlite":
			cmd_input = b".quit"
		else:
			cmd_input = rb"\q"
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = itds.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
