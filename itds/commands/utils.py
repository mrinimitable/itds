...
from itds.coverage import CodeCoverage
from itds.exceptions import SiteNotSpecifiedError
from itds.utils import cint, update_progress_bar
...
def build(
	...
	from itds.build import bundle, download_itds_assets
	from itds.gettext.translate import compile_translations
	...
	with filelock("shashi_build", is_global=True, timeout=10):
		...
		if not (force or apps or os.environ.get("CI")):
			# skip building itds if assets exist remotely
			skip_itds = download_itds_assets(verbose=verbose)
		else:
			skip_itds = False

		# don't minify in developer_mode for faster builds
		development = itds.local.conf.developer_mode or itds._dev_server
...
def destroy_all_sessions(context: CliCtxObj, reason=None):
	...
	for site in context.sites:
		try:
			itds.init(site)
			itds.connect()
			itds.sessions.clear_all_sessions(reason)
			itds.db.commit()
		finally:
			itds.destroy()
	if not context.sites:
...
def show_config(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		configuration = itds.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(itds.as_json(sites_config))
...
def reset_perms(context: CliCtxObj):
	...
	from itds.permissions import reset_perms

	for site in context.sites:
		try:
			itds.init(site)
			itds.connect()
			for d in itds.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				itds.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			itds.destroy()
	if not context.sites:
...
def execute(context: CliCtxObj, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = itds.get_attr(method)(*fn_args, **fn_kwargs)
			except Exception:
...
def export_doc(context: CliCtxObj, doctype, docname):
	...
	for site in context.sites:
		try:
			itds.init(site)
			itds.connect()
			itds.modules.export_doc(doctype, docname)
		finally:
			itds.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context: CliCtxObj, doctype, path, name=None):
	...
	from itds.core.doctype.data_import.data_import import export_json
...
def export_csv(context: CliCtxObj, doctype, path):
	...
	from itds.core.doctype.data_import.data_import import export_csv
...
def data_import(
	...
	from itds.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context: CliCtxObj, doctype, path):
	...
	from itds.model.rename_doc import bulk_rename
	from itds.utils.csvutils import read_csv_content
...
def _enter_console(extra_args=None):
	from itds.database import get_command
	from itds.utils import get_site_path
	...
	bin, args, bin_name = get_command(
		...
		user=itds.conf.db_user,
		password=itds.conf.db_password,
...
def jupyter(context: CliCtxObj):
	...
	jupyter_notebooks_path = os.path.abspath(itds.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(itds.get_site_path(".."))
...
Run the following in your first cell to connect notebook to itds
```
import itds
itds.init('{site}', sites_path='{sites_path}')
itds.connect()
itds.local.lang = itds.get_system_settings('language')
itds.db.connect()
```
	"""
	)
...
def store_logs(terminal: "InteractiveShellEmbed") -> None:
	...
	itds.log_level = 20  # info
	with suppress(Exception):
		logger = itds.logger("ipython")
...
def console(context: CliCtxObj, autoreload=False):
	...
	all_apps = itds.get_installed_apps()
	failed_to_import = []
...
def transform_database(context: CliCtxObj, table, engine, row_format, failfast):
	...
	for current, table in enumerate(tables):
		...
		try:
			itds.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def serve(
	...
	with CodeCoverage(with_coverage, "itds"):
		if with_coverage:
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					itds.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			itds.handler.execute_cmd(itds.form_dict.cmd)
...
def make_app(destination, app_name, no_git=False):
	...
	from itds.utils import get_sites
	...
	from itds.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from itds.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	for app in sorted(itds.get_all_apps()):
		module = itds.get_module(app)
		app_hooks = itds.get_module(app + ".hooks")

		app_info = itds._dict()

		try:
			app_info.commit = Repo(itds.get_app_source_path(app)).head.object.hexsha[:7]
...
def rebuild_global_search(context: CliCtxObj, static_pages=False):
	...
	from itds.utils.global_search import (
