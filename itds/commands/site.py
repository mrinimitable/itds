...
import itds
from itds.commands import get_site, pass_context
from itds.exceptions import SiteNotSpecifiedError
from itds.utils import CallbackManager
from itds.utils.shashi_helper import CliCtxObj
...
def new_site(
	...
	"Create a new site"
	from itds.installer import _new_site

	itds.init(site, new_site=True)

	if site in itds.get_all_apps():
		click.secho(
...
def _restore(
	*,
	...
	from itds.installer import extract_files
	from itds.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = itds.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from itds.installer import _new_site, is_downgrade, is_partial, validate_database_sql
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Itds site.",
			fg="red",
		)
		click.secho(
	...
	# Check if the backup is of an older version of itds and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from itds.installer import is_partial, partial_restore
	from itds.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `shashi restore` to restore a Itds Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		itds.connect()
		itds.clear_cache()
		installed = itds.get_installed_apps()
		itds.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from itds.installer import install_app as _install_app
	from itds.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except itds.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{itds.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = itds.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(itds.get_installed_apps())
	...
	if format == "json":
		click.echo(itds.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		itds.connect()
		try:
			itds.db.add_index(doctype, columns)
			itds.db.commit()
		finally:
			itds.destroy()
...
def describe_database_table(context, doctype, column):
	...
	from itds.core.doctype.recorder.recorder import _fetch_table_stats
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		itds.connect()
		try:
			itds.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			itds.db.commit()
		finally:
			itds.destroy()
	if not context.sites:
...
def disable_user(context: CliCtxObj, email):
	...
	site = get_site(context)
	with itds.init_site(site):
		itds.connect()
		user = itds.get_doc("User", email)
		user.enabled = 0
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False):
	...
	from itds.migrate import SiteMigration
...
def migrate_to():
	...
	from itds.integrations.itds_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			itds.connect()
			itds.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			itds.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			itds.init(site)
			itds.connect()
			itds.reload_doc(module, doctype, docname, force=context.force)
			itds.db.commit()
		finally:
			itds.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			itds.init(site)
			itds.connect()
			itds.reload_doctype(doctype, force=context.force)
			itds.db.commit()
		finally:
			itds.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		itds.commands.popen(f"echo '127.0.0.1\t{site}\n::1\t{site}' | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(itds.get_traceback(with_context=True))
			exit_code = 1
			continue
		if itds.get_system_settings("encrypt_backup") and itds.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_itds(app)
	from itds.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_itds(app)
	from itds.installer import remove_app
	from itds.utils.synchronization import filelock
...
def _drop_site(
	...
	from itds.database import drop_user_and_database
	from itds.utils.backups import scheduled_backup
	...
	itds.flags.root_login = db_root_username
	itds.flags.root_password = db_root_password

	drop_user_and_database(itds.conf.db_name, itds.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		itds.utils.get_shashi_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	itds.init(site)
	old_path = itds.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	with itds.init_site(site):
		...
		itds.db.set_value("User", user, "last_active", now_datetime())
		itds.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	...
	from itds import publish_realtime
...
def browse(
	...
	if site not in itds.utils.get_sites():
	...
	if user:
		...
		if itds.conf.developer_mode or user == "Administrator":
			itds.utils.set_request(path="/")
			itds.local.cookie_manager = CookieManager()
			itds.local.login_manager = LoginManager()
			itds.local.login_manager.login_as(user, session_end, user_for_audit)
			sid = f"/app?sid={itds.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{itds.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = itds.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from itds.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from itds.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from itds.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise itds.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			itds.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == itds.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = itds.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				itds.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[itds.local.site] = TABLES_TO_DROP
		itds.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		itds.conf.db_type,
		f"framework_{itds.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from itds.model.meta import trim_tables
	from itds.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {itds.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {itds.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({itds.local.site: data}, indent=1, sort_keys=True))
	else:
		from itds.utils.commands import render_table
...
def ensure_app_not_itds(app: str) -> None:
	"""
	Ensure that the app name passed is not 'itds'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "itds":
		click.secho("You cannot remove or uninstall the app `itds`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from itds.modules.patch_handler import update_patch_log
