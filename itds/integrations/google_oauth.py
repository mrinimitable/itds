...
import itds
from itds.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/itds.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "itds.email.oauth.authorize_google_access",
	"contacts": "itds.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"indexing": "itds.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True, config=None):
		self.google_settings = itds.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			itds.throw(itds._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			itds.throw(itds._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		itds.log_error(itds._(error_title), itds._(response.get("error_description", error_message)))

		if raise_err:
			itds.throw(itds._(error_title), GoogleAuthenticationError, itds._(error_message))
...
@itds.whitelist(methods=["GET"])
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `itds.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to itds site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			itds.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	itds.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
