...
import itds
from itds.integrations.doctype.oauth_client.oauth_client import OAuthClient
from itds.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = itds.flags.integration_request = s.request(
		...
		if itds.flags.integration_request_doc:
			itds.flags.integration_request_doc.log_error()
		else:
			itds.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = itds.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else itds.as_json(obj, indent=1)
...
def validate_dynamic_client_metadata(client: OAuth2DynamicClientMetadata):
	...
	if not itds.conf.developer_mode and any(c.scheme != "https" for c in client.redirect_uris):
...
def create_new_oauth_client(client: OAuth2DynamicClientMetadata):
	doc = cast(OAuthClient, itds.get_doc({"doctype": "OAuth Client"}))
...
def get_oauth_settings():
	...
	settings: dict[str, Any] = itds._dict({"skip_authorization": None})
	if itds.get_cached_value("OAuth Settings", "OAuth Settings", "skip_authorization"):
		settings["skip_authorization"] = "Auto"  # based on legacy OAuth Provider Settings value

	elif value := itds.get_cached_value(
