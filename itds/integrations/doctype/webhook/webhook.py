...
from itds.utils.background_jobs import get_queues_timeout
from itds.utils.jinja import validate_template
from itds.utils.safe_exec import get_safe_globals

WEBHOOK_SECRET_HEADER = "X-Itds-Webhook-Signature"
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from itds.integrations.doctype.webhook_data.webhook_data import WebhookData
		from itds.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def on_update(self):
		itds.client_cache.delete_value("webhooks")

	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = itds.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				itds.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = itds.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				itds.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				itds.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			itds.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			itds.throw(_("Same Field is entered more than once"))
	...
	def validate_secret(self):
		if self.enable_security:
			try:
				...
				itds.throw(_("Invalid Webhook Secret"))
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = itds.get_cached_doc(self.webhook_doctype, preview_document)
			met_condition = itds.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self, preview_document):
		try:
			doc = itds.get_cached_doc(self.webhook_doctype, preview_document)
			return itds.as_json(get_webhook_data(doc, self))
		except Exception as e:
			itds.local.message_log = []
...
def get_context(doc):
	return {"doc": doc, "utils": get_safe_globals().get("itds").get("utils")}


def enqueue_webhook(doc, webhook) -> None:
	request_url = headers = data = r = None
	try:
		webhook: Webhook = itds.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = itds.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		itds.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			itds.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			itds.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = itds.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_doctype": doctype,
			"reference_document": docname,
			"user": itds.session.user if itds.session.user else None,
			"url": url,
			"headers": itds.as_json(headers) if headers else None,
			"data": itds.as_json(data) if data else None,
			"response": res.text if res is not None else None,
			"error": itds.get_traceback(),
		}
	)
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = itds.render_template(webhook.webhook_json, get_context(doc))
