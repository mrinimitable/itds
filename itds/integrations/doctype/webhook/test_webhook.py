...
import itds
from itds.integrations.doctype.webhook import flush_webhook_execution_queue
from itds.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
def get_test_webhook(config):
	wh = itds.get_doc(config)
	if not wh.name:
		wh.name = itds.generate_hash()
	wh.insert()
	wh.reload()
	try:
		yield wh
...
class TestWebhook(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		itds.db.delete("Webhook")
		# Delete existing logs if any
		itds.db.delete("Webhook Request Log")
		super().setUpClass()
	...
	def setUp(self):
		...
		if itds.db.exists("Webhook", webhook_fields):
			self.webhook = itds.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = itds.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = itds.new_doc("User")
		self.user.first_name = itds.mock("name")
		self.user.email = itds.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = itds.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		webhooks = itds.client_cache.get_value("webhooks")
		self.assertTrue("User" in webhooks)
		self.assertEqual(len(webhooks.get("User")), 1)

		# only 1 hook (enabled) must be queued
		self.assertEqual(len(itds.local._webhook_queue), 1)
		execution = itds.local._webhook_queue[0]
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(itds.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		...
		if not itds.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = itds.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = itds.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = itds.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(itds.get_all("Webhook Request Log", pluck="name"))
	...
	def test_webhook_with_array_body(self):
		...
		doc = itds.new_doc("Note")
		doc.title = "Test Webhook Note"
		final_title = itds.generate_hash()
		...
		with get_test_webhook(wh_config):
			...
			log = itds.get_last_doc("Webhook Request Log")
