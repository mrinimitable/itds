...
import itds
from itds.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = itds.new_doc(doctype)
	connected_app.provider_name = "itds"
...
def get_oauth_client():
	oauth_client = itds.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	def setUp(self):
		...
		Itds comes with it's own oAuth2 provider that we can test against. The
		...
		Itds's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/itds.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		itds.db.commit()  # Avoid snapshot violation issues

		delete_if_exists("token_cache")
		delete_if_exists("connected_app")

		if getattr(self, "oauth_client", None):
			tokens = itds.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = itds.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = itds.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = itds.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
