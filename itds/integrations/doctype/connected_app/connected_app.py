...
if any((os.getenv("CI"), itds.conf.developer_mode, itds.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from itds.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from itds.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def validate(self):
		base_url = itds.utils.get_url()
		callback_path = (
			"/api/method/itds.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = itds.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		itds.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if itds.db.exists("Token Cache", token_cache_name):
			token_cache = itds.get_doc("Token Cache", token_cache_name)
...
@itds.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if itds.session.user == "Guest":
		itds.local.response["type"] = "redirect"
		itds.local.response["location"] = "/login?" + urlencode({"redirect-to": itds.request.url})
		return

	path = itds.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		itds.throw(_("Invalid Parameters."))

	connected_app = itds.get_doc("Connected App", path[3])
	token_cache = itds.get_doc("Token Cache", connected_app.name + "-" + itds.session.user)

	if state != token_cache.state:
		itds.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	itds.local.response["type"] = "redirect"
	itds.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@itds.whitelist()
def has_token(connected_app, connected_user=None):
	app = itds.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or itds.session.user)
