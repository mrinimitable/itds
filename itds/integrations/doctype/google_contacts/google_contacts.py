...
class GoogleContacts(Document):
	...
	def validate(self):
		if not itds.db.get_single_value("Google Settings", "enable"):
			itds.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = itds.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = itds.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = itds.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = itds.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	itds.db.set_value("Google Contacts", account.name, "last_sync_on", itds.utils.now_datetime())

	for idx, connection in enumerate(results):
		itds.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=itds.session.user
		)
		# Work-around to fix
		# https://github.com/mrinimitable/itds/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not itds.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		itds.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		itds.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		itds.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		itds.msgprint(
