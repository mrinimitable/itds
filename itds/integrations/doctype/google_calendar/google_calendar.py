...
from itds.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = itds.get_cached_doc("Google Settings")
		if not google_settings.enable:
			itds.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			itds.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			raise itds.ValidationError(
				_("Click on {0} to generate Refresh Token.").format(itds.bold(allow_google_calendar_label))
			)
		...
		try:
			...
			itds.throw(
				...
				).format(itds.bold(allow_google_calendar_label))
			)
...
def authorize_access(g_calendar: str, reauthorize: bool = False):
	...
	"""
	google_calendar = itds.get_doc("Google Calendar", g_calendar)
	...
	if not google_calendar.authorization_code or reauthorize:
		itds.cache.hset("google_calendar", "google_calendar", google_calendar.name)
	...
	if "refresh_token" in r:
		itds.db.set_value("Google Calendar", google_calendar.name, "refresh_token", r["refresh_token"])
		itds.db.commit()

	itds.local.response["type"] = "redirect"
	itds.local.response["location"] = google_calendar.get_url()

	itds.msgprint(_("Google Calendar has been configured."), indicator="green")
...
def google_callback(code=None):
	...
	"""
	google_calendar = itds.cache.hget("google_calendar", "google_calendar")
	itds.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	itds.db.commit()
...
def sync(g_calendar: str | None = None):
	...
	for g in itds.get_list("Google Calendar", filters=filters, pluck="name"):
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = itds.get_cached_doc("Google Settings")
	account: GoogleCalendar = itds.get_doc("Google Calendar", g_calendar)
...
def check_google_calendar(account: GoogleCalendar, google_calendar):
	...
	calendar = {
		...
		"timeZone": itds.get_system_settings("time_zone"),
	}
	try:
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				itds.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				itds.throw(msg)
	...
	for idx, event in enumerate(results):
		itds.publish_realtime(
			"import_google_calendar", {"progress": idx + 1, "total": len(results)}, user=itds.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			# NOTE: Skip if event is already synced; Itds doesn't track individual
			# instances of recurring events, so we need to check if the event is already
			# synced in Itds Calendar
			if event.get("recurringEventId"):
				...
			elif not itds.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Itds Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	e: Event = itds.get_doc(calendar_event)
...
def update_participants_in_event(calendar_event: "Event", google_event: dict):
	...
	in_system_participants = itds.get_all(
...
def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Itds Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = itds.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not itds.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		itds.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		itds.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Itds Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Itds Event is deleted.
	"""

	if not itds.db.exists("Google Calendar", {"name": doc.google_calendar, "push_to_google_calendar": 1}):
		return
