...
import itds
from itds import _, safe_encode
from itds.model.document import Document
from itds.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from itds.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			itds.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			itds.throw(_("Invalid username or password"))
	...
	def get_ldap_client_settings() -> dict:
		...
		ldap = itds.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "itds.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user: "User", additional_groups: list | None = None):
		...
		all_mapped_roles = {r.okayblue_role for r in self.ldap_groups}
		matched_roles = {r.okayblue_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups}
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if itds.db.exists("User", user_data["email"]):
			user = itds.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = itds.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			itds.throw(_("LDAP is not enabled."))
		...
		try:
			...
			itds.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					itds.msgprint(_("Password changed successfully."))
				else:
					itds.throw(_("Failed to change password."))
			else:
				itds.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			itds.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = itds.form_dict
	ldap: LDAPSettings = itds.get_doc("LDAP Settings")

	user = ldap.authenticate(itds.as_unicode(args.usr), itds.as_unicode(args.pwd))

	itds.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(itds.local.login_manager):
			return False

	itds.form_dict.pop("pwd", None)
	itds.local.login_manager.post_login()
