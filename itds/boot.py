...
import itds.desk.desk_page
from itds.core.doctype.installed_applications.installed_applications import (
	get_setup_wizard_completed_apps,
	get_setup_wizard_not_required_apps,
)
from itds.core.doctype.navbar_settings.navbar_settings import get_app_logo, get_navbar_settings
from itds.desk.doctype.changelog_feed.changelog_feed import get_changelog_feed_items
from itds.desk.doctype.form_tour.form_tour import get_onboarding_ui_tours
from itds.desk.doctype.route_history.route_history import frequently_visited_links
from itds.desk.form.load import get_meta_bundle
from itds.email.inbox import get_email_accounts
from itds.integrations.itds_providers.itdscloud_billing import is_fc_site
from itds.model.base_document import get_controller
from itds.permissions import has_permission
...
from itds.utils import add_user_info, cstr, get_system_timezone
from itds.utils.change_log import get_versions
from itds.utils.itdscloud import on_itdscloud
from itds.website.doctype.web_page_view.web_page_view import is_tracking_enabled


def get_bootinfo():
	"""build and return boot info"""
	from itds.translate import get_lang_dict, get_translated_doctypes

	itds.set_user_lang(itds.session.user)
	bootinfo = itds._dict()
	...
	# system info
	bootinfo.sitename = itds.local.site
	bootinfo.sysdefaults = itds.defaults.get_defaults()
	bootinfo.sysdefaults["setup_complete"] = itds.is_setup_complete()

	bootinfo.server_date = itds.utils.nowdate()
	...
	bootinfo.active_domains = itds.get_active_domains()
	bootinfo.all_domains = [d.get("name") for d in itds.get_all("Domain")]
	add_layouts(bootinfo)

	bootinfo.module_app = itds.local.module_app
	bootinfo.single_types = [d.name for d in itds.get_all("DocType", {"issingle": 1})]
	bootinfo.nested_set_doctypes = [
		d.parent for d in itds.get_all("DocField", {"fieldname": "lft"}, ["parent"])
	]
	...
	bootinfo.home_folder = itds.db.get_value("File", {"is_home_folder": 1})
	...
	# ipinfo
	if itds.session.data.get("ipinfo"):
		bootinfo.ipinfo = itds.session["data"]["ipinfo"]
	...
	for method in hooks.boot_session or []:
		itds.get_attr(method)(bootinfo)

	if bootinfo.lang:
		bootinfo.lang = str(bootinfo.lang)
	bootinfo.versions = {k: v["version"] for k, v in get_versions().items()}

	bootinfo.error_report_email = itds.conf.error_report_email
	bootinfo.calendars = sorted(itds.get_hooks("calendars"))
	bootinfo.treeviews = itds.get_hooks("treeviews") or []
	bootinfo.lang_dict = get_lang_dict()
	bootinfo.success_action = get_success_action()
	bootinfo.update(get_email_accounts(user=itds.session.user))
	bootinfo.sms_gateway_enabled = bool(itds.db.get_single_value("SMS Settings", "sms_gateway_url"))
	...
	bootinfo.enable_address_autocompletion = itds.db.get_single_value(
...
def remove_apps_with_incomplete_dependencies(bootinfo):
	...
	for app in bootinfo.setup_wizard_not_required_apps:
		...
		for required_apps in itds.get_hooks("required_apps"):
...
def get_letter_heads():
	...
	if not itds.has_permission("Letter Head"):
		return letter_heads
	for letter_head in itds.get_list("Letter Head", fields=["name", "content", "footer"]):
...
def load_conf_settings(bootinfo):
	...
	for key in ("developer_mode", "socketio_port", "file_watcher_port"):
		if key in itds.conf:
			bootinfo[key] = itds.conf.get(key)


def load_desktop_data(bootinfo):
	from itds.desk.desktop import get_workspace_sidebar_items
	...
	bootinfo.dashboards = itds.get_all("Dashboard")
	bootinfo.app_data = []

	Workspace = itds.qb.DocType("Workspace")
	Module = itds.qb.DocType("Module Def")

	for app_name in itds.get_installed_apps():
		# get app details from app_info (/apps)
		apps = itds.get_hooks("add_to_apps_screen", app_name=app_name)
		app_info = {}
		if apps:
			app_info = apps[0]
			has_permission = app_info.get("has_permission")
			if has_permission and not itds.get_attr(has_permission)():
				continue

		workspaces = [
			r[0]
			for r in (
				itds.qb.from_(Workspace)
				.inner_join(Module)
		...
		bootinfo.app_data.append(
			dict(
				...
				or (
					(
						itds.get_hooks("app_title", app_name=app_name)
						and itds.get_hooks("app_title", app_name=app_name)[0]
					)
					or ""
				)
				or app_name,
				app_route=(
					itds.get_hooks("app_home", app_name=app_name)
					and itds.get_hooks("app_home", app_name=app_name)[0]
				)
				or (workspaces and "/app/" + itds.utils.slug(workspaces[0]))
				or "",
				app_logo_url=app_info.get("logo")
				or itds.get_hooks("app_logo_url", app_name=app_name)
				or itds.get_hooks("app_logo_url", app_name="itds"),
				modules=[m.name for m in itds.get_all("Module Def", dict(app_name=app_name))],
...
def get_user_pages_or_reports(parent, cache=False):
	if cache:
		has_role = itds.cache.get_value("has_role:" + parent, user=itds.session.user)
		if has_role:
			return has_role
	...
	no_of_roles = SubQuery(
		itds.qb.from_(hasRole).select(Count("*")).where(hasRole.parent == parentTable.name)
	)

	# pages and reports with no role are allowed
	rows_with_no_roles = (
		itds.qb.from_(parentTable)
	...
	itds.cache.set_value("has_role:" + parent, has_role, itds.session.user, 21600)
	return has_role
...
def load_translations(bootinfo):
	...
	bootinfo["lang"] = itds.lang
	bootinfo["__messages"] = get_messages_for_boot()


def get_user_info():
	# get info for current user
	user_info = itds._dict()
	add_user_info(itds.session.user, user_info)
...
def get_user(bootinfo):
	"""get user info"""
	bootinfo.user = itds.get_user().load_user()
...
def add_home_page(bootinfo, docs):
	...
	if itds.session.user == "Guest":
		return
	home_page = itds.db.get_default("desktop:home_page")

	if not itds.is_setup_complete():
		bootinfo.setup_wizard_requires = itds.get_hooks("setup_wizard_requires")

	try:
		page = itds.desk.desk_page.get(home_page)
		docs.append(page)
		bootinfo["home_page"] = page.name
	except (itds.DoesNotExistError, itds.PermissionError):
...
def add_timezone_info(bootinfo):
	...
	import itds.utils.momentjs

	bootinfo.timezone_info = {"zones": {}, "rules": {}, "links": {}}
	itds.utils.momentjs.update(system, bootinfo.timezone_info)


def load_print(bootinfo, doclist):
	print_settings = itds.db.get_singles_dict("Print Settings")
...
def load_print_css(bootinfo, print_settings):
	import itds.www.printview

	bootinfo.print_css = itds.www.printview.get_print_style(
		print_settings.print_style or "Redesign", for_legacy=True
	)


def get_success_action():
	return itds.get_all("Success Action", fields=["*"])


def get_link_preview_doctypes():
	from itds.utils import cint

	link_preview_doctypes = [d.name for d in itds.get_all("DocType", {"show_preview_popup": 1})]
	customizations = itds.get_all(
...
def get_additional_filters_from_hooks():
	filter_config = itds._dict()
	filter_hooks = itds.get_hooks("filters_config")
	for hook in filter_hooks:
		filter_config.update(itds.get_attr(hook)())
...
def add_layouts(bootinfo):
	...
	bootinfo.doctype_layouts = itds.get_all("DocType Layout", ["name", "route", "document_type"])


def get_desk_settings():
	from itds.core.doctype.user.user import desk_properties

	return itds.get_value("User", itds.session.user, desk_properties, as_dict=True)


def get_notification_settings():
	return itds.get_cached_doc("Notification Settings", itds.session.user)


def get_link_title_doctypes():
	dts = itds.get_all("DocType", {"show_title_field_in_link": 1})
...
def set_time_zone(bootinfo):
	bootinfo.time_zone = {
		...
		"user": bootinfo.get("user_info", {}).get(itds.session.user, {}).get("time_zone", None)
		or get_system_timezone(),
	}


def load_country_doc(bootinfo):
	country = itds.db.get_default("country")
	if not country:
		return
	try:
		bootinfo.docs.append(itds.get_cached_doc("Country", country))
	except Exception:
		pass


def load_currency_docs(bootinfo):
	currency = itds.qb.DocType("Currency")

	currency_docs = (
		itds.qb.from_(currency)
		.select(
			currency.name,
			currency.fraction,
...
def get_marketplace_apps():
	...
	apps = []
	cache_key = "itds_marketplace_apps"

	if itds.conf.developer_mode or not on_itdscloud():
		return apps

	def get_apps_from_fc():
		remote_site = itds.conf.itdscloud_url or "itdscloud.com"
	...
	try:
		apps = itds.cache.get_value(cache_key, get_apps_from_fc, shared=True)
		...
		itds.cache.set_value(cache_key, apps, shared=True, expires_in_sec=24 * 60 * 60)
...
def get_sentry_dsn():
	...
	return os.getenv("ITDS_SENTRY_DSN")
