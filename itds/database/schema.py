...
import itds
from itds import _
from itds.utils import cint, cstr, flt
...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or itds.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			itds.client_cache.delete_value(f"table_columns::{self.table_name}")
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*itds.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_index_definitions(self):
		return [
			...
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in itds.db.type_map
				and itds.db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			)
		]
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in itds.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			itds._dict({"fieldname": f, "fieldtype": "Data"}) for f in itds.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				itds._dict({"fieldname": f, "fieldtype": "Data"}) for f in itds.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				itds.throw(
					_("Fieldname is limited to 64 characters ({0})").format(itds.bold(col.fieldname))
				)

			if "varchar" in itds.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(itds.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					itds.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						itds.msgprint(info_message)

	def is_new(self):
		return self.table_name not in itds.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in itds.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in itds.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			default = itds.db.escape(self.default)

		if self.not_nullable and null:
			if default is None:
				default = get_not_null_defaults(self.fieldtype)
				if isinstance(default, str):
					default = itds.db.escape(default)
			null = False
	...
	def build_for_alter_table(self, current_def):
		...
		if current_def["type"] != column_type and not (
			...
			current_def["type"] == "longtext" and column_type == "json" and itds.db.db_type == "mariadb"
		):
	...
	def default_changed(self, current_def):
		if "decimal" in current_def["type"]:
			...
			db_field_type = itds.db.type_map.get(fieldtype)
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		itds.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				itds.bold(cstr(n)), special_characters
			),
			itds.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > itds.db.MAX_COLUMN_LENGTH:
		itds.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None, *, options=None):
	d = itds.db.type_map.get(fieldtype)
	...
	if (
		...
		and itds.db.exists("DocType", options)
		and itds.get_meta(options).autoname == "UUID"
	):
		d = ("uuid", None)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = itds.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as itds implements int
...
def add_column(doctype, column_name, fieldtype, precision=None, length=None, default=None, not_null=False):
	...
	if itds.db.db_type == "sqlite":
		if column_name in itds.db.get_table_columns(doctype):
			return
