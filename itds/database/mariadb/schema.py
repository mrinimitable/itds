...
import itds
from itds import _
from itds.database.schema import DBTable
...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		add_index_query = [
			...
			if not itds.db.get_column_index(self.table_name, col.fieldname, unique=False)
		]

		if self.meta.sort_field == "modified" and not itds.db.get_column_index(
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := itds.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := itds.db.get_column_index(self.table_name, col.fieldname, unique=False):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname != "UUID" and itds.db.get_column_type(self.doctype, "name") == "uuid":
			return f"modify name varchar({itds.db.VARCHAR_LEN})"
