...
from itds.database.mariadb.schema import MariaDBTable
from itds.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if itds.conf.local_infile:
			conn_settings["local_infile"] = itds.conf.local_infile

		# Configure SSL settings
		if itds.conf.db_ssl_ca:
			ssl_config = {
				"ca": itds.conf.db_ssl_ca,
				"check_hostname": itds.conf.db_ssl_check_hostname,
			}

			# Add client certificates for mutual SSL if available
			if itds.conf.db_ssl_cert and itds.conf.db_ssl_key:
				ssl_config.update({"cert": itds.conf.db_ssl_cert, "key": itds.conf.db_ssl_key})
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via itds.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = itds.as_unicode(escape_string(itds.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_table_columns_description(self, table_name):
		...
		return self.sql(
			...
			where table_name = '{table_name}'
   			and table_schema = '{itds.db.cur_db_name}' """,
			as_dict=1,
		)
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> itds._dict | None:
	...
	def add_index(self, doctype: str, fields: list, index_name: str | None = None):
		...
		if not self.has_index(table_name, index_name):
			...
			# via code or console.
			if len(fields) == 1 and not (itds.flags.in_install or itds.flags.in_migrate):
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = itds.client_cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			tables = (
				...
				.where(information_schema.tables.table_schema == itds.db.cur_db_name)
				.run(pluck=True)
			)
			itds.client_cache.set_value("db_tables", tables)
