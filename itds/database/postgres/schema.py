import itds
from itds import _
from itds.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = itds.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			itds.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			itds.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{itds.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				itds.db.sql(final_alter_query)
			if change_nullability:
				# nosemgrep
				itds.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
			if create_contraint_query:
				# nosemgrep
				itds.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				itds.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if itds.db.is_duplicate_fieldname(e):
				itds.throw(str(e))
			elif itds.db.is_duplicate_entry(e):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and itds.db.get_column_type(self.doctype, "name") != "uuid":
			if not itds.db.get_value(self.doctype, {}, order_by=None):
		...
		if autoname != "UUID" and itds.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({itds.db.VARCHAR_LEN})"
