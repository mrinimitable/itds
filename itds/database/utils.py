...
from itds.query_builder.functions import Function
...
class LazyMogrify(LazyString):
	...
	def _setup(self) -> str:
		return itds.db.mogrify(self.query, self.values)
...
def dangerously_reconnect_on_connection_abort(func):
	...
	def wrapper(*args, **kwargs):
		try:
			...
			if itds.db.is_interface_error(e) or isinstance(e, itds.db.OperationalError):
				itds.db.connect()
				return func(*args, **kwargs)
			raise
	...
	def run(self):
		...
		while self._functions:
			...
			try:
				...
				itds.log_error(title="Error executing commit_after_response callback")
...
def commit_after_response(func):
	...
	request = getattr(itds.local, "request", False)
	if not request or itds.in_test:
		func()
		return
