...
from itds import _, _dict
from itds.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	FilterValue,
...
from itds.exceptions import DoesNotExistError, ImplicitCommitError
from itds.monitor import get_trace_id
from itds.query_builder import Case
...
from itds.utils import cast as cast_fieldtype
...
class Database:
	...
	class InvalidColumnName(itds.ValidationError):
		pass
	...
	def __init__(
		...
		self.logger = itds.logger("database")
	...
	def sql(
		...
		*,
		...
		Examples:
		        ...
		        itds.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        itds.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        itds.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		                {"name": "a%", "owner":"test@example.com"})

		"""
		if isinstance(query, QueryBuilder):
			itds.log("Use run method to execute SQL queries generated by Query Builder")
		...
		if trace_id := get_trace_id():
			query += f" /* ITDS_TRACE_ID: {trace_id} */"

		try:
			self.execute_query(query, values)
		except Exception as e:
			if self.is_syntax_error(e):
				itds.log(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise itds.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise itds.QueryTimeoutError(e) from e
				...
				itds.throw(
					...
					exc=itds.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				itds.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			itds.log(f"Execution time: {(time_end - time_start) * 1000:.3f} ms")
	...
	def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
		while result := self._transform_result(self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)):
			if pluck:
				...
				itds.throw(_("`as_iterator` only works with `as_list=True` or `as_dict=True`"))
	...
	def _log_query(
		...
		if itds.local.flags.in_migrate:
			_query = _query or str(mogrified_query)
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {
					k: itds.db.escape(v) if isinstance(v, str) else v for k, v in values.items()
				}
			elif isinstance(values, list | tuple):
				return query % tuple(itds.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=EmptyQueryValues):
		...
		itds.log("--- query explain ---")
		try:
			results = self.sql(f"EXPLAIN {query}", values, as_dict=1)
		except Exception as e:
			itds.log(f"error in query explain: {e}")
		else:
			itds.log(json.dumps(results, indent=1))
			itds.log("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = itds.db.sql_list("select name from DocType")
		"""
		...
		if query_type in WRITE_QUERY_TYPES:
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise itds.TooManyWritesError(msg)
	...
	def clear_db_table_cache(query_type: str):
		if query_type in CREATE_OR_DROP:
			itds.client_cache.delete_value("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        itds.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        itds.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = itds.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        itds.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = itds.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = itds.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (
						itds.db.is_missing_column(e)
						or itds.db.is_table_missing(e)
						or str(e).startswith("Invalid DocType")
					):
						out = None
					elif (not ignore) and itds.db.is_table_missing(e):
	...
	def get_values_from_single(
		...
		*,
		...
		from itds.model.meta import get_default_df
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = itds.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = itds.qb.get_query(
			"Singles",
	...
	def get_all(*args, **kwargs):
		return itds.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return itds.get_list(*args, **kwargs)
	...
	def _get_update_dict(
		...
		if update_modified:
			...
			modified_by = modified_by or itds.session.user
	...
	def set_single_value(
		...
		*,
		...
		Example:
		        ...
		        itds.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
		...
		itds.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data).run(debug=debug)
		itds.clear_document_cache(doctype, doctype)
	...
	def get_single_value(
		...
		*,
		...
		Example:
		        ...
		        company = itds.db.get_single_value('Global Defaults', 'default_company')
		"""
		from itds.model.meta import get_default_df
		...
		df = itds.get_meta(doctype).get_field(fieldname) or get_default_df(fieldname)

		if not df:
			itds.throw(
				_("Field {0} does not exist on {1}").format(
					itds.bold(fieldname), itds.bold(doctype), self.InvalidColumnName
				)
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		"""
		from itds.model.utils import is_single_doctype
		...
		if isinstance(dn, FilterValue):
			itds.clear_document_cache(dt, convert_to_value(dn))
		else:
	...
	def _build_and_run_bulk_update_query(
		...
		update_query = itds.qb.update(dt)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		itds.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		itds.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = itds.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(itds.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or itds.flags.read_only
	...
	def savepoint(self, save_point):
		...
		Changes can be undone to a save point by doing itds.db.rollback(save_point)
	...
	def a_row_exists(self, doctype):
		...
		return itds.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = itds.client_cache.get_value(key)
		if columns is None:
			information_schema = itds.qb.Schema("information_schema")

			columns = (
				itds.qb.from_(information_schema.columns)
			...
			if columns:
				itds.cache.set_value(key, columns)
	...
	def get_system_setting(self, key):
		return itds.get_system_settings(key)
	...
	from itds.deprecation_dumpster import is_column_missing as _is_column_missing

	is_column_missing = staticmethod(_is_column_missing)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from itds.utils.nestedset import get_descendants_of
	...
	def log_touched_tables(self, query, query_type):
		...
		touched_tables = itds.local.flags.touched_tables

		if touched_tables is None:
			touched_tables = set()
			itds.local.flags.touched_tables = touched_tables
	...
	def bulk_insert(
		...
		*,
		...
		query = itds.qb.into(table).columns(fields)

		if ignore_duplicates:
			# Pypika does not have same api for ignoring duplicates
			if itds.conf.db_type in ("mariadb", "sqlite"):
				query = query.ignore()
	...
	def create_sequence(self, *args, **kwargs):
		from itds.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from itds.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from itds.database.sequence import get_next_val
	...
	def unbuffered_cursor(self):
		...
		Usage:
		        with itds.db.unbuffered_cursor():
		                for row in itds.db.sql("query with huge result", as_iterator=True):
...
def savepoint(catch: type | tuple[type, ...] = Exception):
	...
	try:
		...
		itds.db.release_savepoint(savepoint)
...
def get_query_execution_timeout() -> int:
	...
	if not itds.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(itds.local, "request", None):
			timeout = itds.conf.http_timeout or 300
...
def get_print_sql_flag() -> bool:
	flag_value = itds.client_cache.get_value("flag_print_sql")
	if flag_value is None:
		flag_value = False
		itds.client_cache.set_value("flag_print_sql", flag_value)
