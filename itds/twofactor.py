# Copyright (c) 2017, Itds Technologies Pvt. Ltd. and Contributors
...
from itds.utils import cint, get_datetime, get_url, time_diff_in_seconds
from itds.utils.background_jobs import enqueue
from itds.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return itds.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	itds.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	itds.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = itds.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = cint(itds.get_system_settings("enable_two_factor_auth"))
	if enabled:
		bypass_two_factor_auth = cint(itds.get_system_settings("bypass_2fa_for_retricted_ip_users"))
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and itds.local.request_ip:
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = itds.qb.DocType("Role")
	no_of_users = itds.db.count(
		role_doctype,
...
def get_verification_method():
	return itds.get_system_settings("two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from itds.auth import get_login_attempt_tracker

	if not otp:
		otp = itds.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = itds.form_dict.get("tmp_id")
	hotp_token = itds.cache.get(tmp_id + "_token")
	otp_secret = itds.cache.get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			itds.cache.delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = itds.get_system_settings("otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = itds.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		itds.get_system_settings("otp_issuer_name")
	)
	return itds.render_template(subject_template, kwargs_dict)
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	return itds.render_template(body_template, kwargs_dict)
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = itds.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(itds.get_system_settings("lifespan_qrcode_image")) or 240
	itds.cache.set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	itds.cache.set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from itds.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = itds.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = itds.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def create_barcode_folder():
	...
	folder = itds.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = itds.get_doc({"doctype": "File", "file_name": folder_name, "is_folder": 1, "folder": "Home"})
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = itds.get_doc("File", barcode.name)
		itds.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = itds.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = itds.get_doc("File", barcode)
	lifespan = itds.get_system_settings("lifespan_qrcode_image") or 240
...
def disable():
	itds.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@itds.whitelist()
def reset_otp_secret(user: str):
	if itds.session.user != user:
		itds.only_for("System Manager", message=True)

	settings = itds.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Itds Framework"
	user_email = itds.get_cached_value("User", user, "email")
	...
	itds.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
