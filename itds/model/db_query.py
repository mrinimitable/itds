...
from itds.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from itds.model import OPTIONAL_FIELDS, get_permitted_fields
from itds.model.meta import get_table_columns
from itds.model.utils import is_virtual_doctype
from itds.model.utils.user_settings import get_user_settings, update_user_settings
from itds.query_builder.utils import Column
from itds.types import Filters, FilterSignature, FilterTuple
...
from itds.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or itds.session.user
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = itds.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			itds.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return itds.call(controller.get_list, args=kwargs, **kwargs)
	...
	def build_and_run(self):
		...
		if itds.db.db_type == "postgres" and args.order_by and args.group_by:
...
{limit}""".format(**args)
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {itds.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			itds.throw(_("Use of sub-query or function is restricted"), itds.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					itds.throw(
						_("Use of function {0} in field is restricted").format(function), exc=itds.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					itds.throw(_("Illegal SQL Query"))
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if itds.only_has_select_perm(doctype) else "read"
		itds.has_permission(
			doctype,
			ptype=ptype,
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `itds.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Itds's meta field like `name`, `owner`, etc.
		"""
		from itds.desk.reportview import extract_fieldnames
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or itds internals
			if column[0] in {"'", '"'}:
				continue
			...
			if doctype and doctype != self.doctype:
				if wrap_grave_quotes(table) not in self.query_tables:
					raise itds.PermissionError(doctype)
	...
	def prepare_filter_condition(self, ft: FilterTuple) -> str:
		...
		from itds.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = itds.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [itds.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [itds.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				df
				and (db_type := cstr(itds.db.type_map.get(df.fieldtype, " ")[0]))
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if itds.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{itds.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and itds.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		role_permissions = itds.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = itds.share.get_shared(self.doctype, self.user)
			if not self.shared:
				itds.throw(_("No permission to read {0}").format(_(self.doctype)), itds.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {itds.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = itds.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(itds.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if itds.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(itds.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		conditions = []
		hooks = itds.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])
		for method in condition_methods:
			if c := itds.call(itds.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)

		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = itds.get_doc("Server Script", permission_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			itds.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			if field.count('"') % 2 or field.count("'") % 2 or field.count("`") % 2:
				itds.throw(_("Invalid field name: {0}").format(field))
			field = field.strip()
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					itds.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
			...
			for func in blacklisted_sql_functions:
				if re.search(r"\b" + re.escape(func) + r"\s*\(", field.lower()):
					itds.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			itds.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or itds.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise itds.PermissionError

		if itds.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = itds.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = itds.utils.nowdate()
	to_date = itds.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{itds.db.format_datetime(from_date)}' AND '{itds.db.format_datetime(to_date)}'"
	else:
		cond = f"'{itds.db.format_date(from_date)}' AND '{itds.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = itds._dict(itds.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = itds._dict(option)
