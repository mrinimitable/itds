...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `itds.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[itds._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		itds.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			itds.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					itds.bold(doctype), itds.bold(m), itds.bold(_as_str(method))
				),
