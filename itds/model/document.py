...
import itds
from itds import _, is_whitelisted, msgprint
from itds.core.doctype.file.utils import relink_mismatched_files
from itds.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from itds.database.utils import commit_after_response
from itds.desk.form.document_follow import follow_document
from itds.integrations.doctype.webhook import run_webhooks
from itds.model import optional_fields, table_fields
from itds.model.base_document import BaseDocument, D, get_controller
from itds.model.docstatus import DocStatus
from itds.model.naming import set_new_name, validate_name
from itds.model.utils import is_virtual_doctype, simple_singledispatch
from itds.model.workflow import set_workflow_state_on_action, validate_workflow
from itds.types import DF
from itds.types.filter import FilterSignature
from itds.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
from itds.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from itds.utils.global_search import update_global_search
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `itds.new_doc` instead."""
	pass
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `itds.model.Document` object.
...
class Document(BaseDocument):
	...
	def load_from_db(self) -> "Self":
		...
		if not is_doctype and self.meta.issingle:
			single_doc = itds.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = itds.new_doc(self.doctype, as_dict=True)
			...
			if not is_doctype and isinstance(self.name, str | int):
				for_update = ""
				if self.flags.for_update and itds.db.db_type != "sqlite":
			...
			if not d:
				itds.throw(
					...
					itds.DoesNotExistError(doctype=self.doctype),
				)
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `itds.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self._handle_permission_failure(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `itds.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return itds.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)
	...
	def insert(
		...
		if not (itds.flags.in_migrate or itds.local.flags.in_install or itds.flags.in_setup_wizard):
			if itds.get_cached_value("User", itds.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, itds.session.user)
		return self
	...
	def check_if_locked(self):
		...
		if file_lock.lock_age(self.get_signature()) > DOCUMENT_LOCK_SOFT_EXPIRY:
			primary_action = {
				"label": "Force Unlock",
				"server_action": "itds.model.document.unlock_document",
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = itds.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if itds.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(itds.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			itds.throw(message, title=_("Amendment Not Allowed"))
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `itds.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		itds.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in itds.db.value_cache:
			itds.db.value_cache.pop(self.doctype, None)

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = itds.session.user
		...
		if self.is_new() and not (
			itds.flags.in_install or itds.flags.in_patch or itds.flags.in_migrate
		):
		...
		itds.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					itds.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), itds.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				itds.throw(msg, itds.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					itds.throw(
						_("Value cannot be changed for {0}").format(
							itds.bold(self.meta.get_label(field.fieldname))
						),
						exc=itds.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += itds.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in itds.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or itds.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = itds.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = itds.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = itds.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = itds.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			itds.msgprint(
				...
				raise_exception=itds.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.DRAFT:
			if self.docstatus.is_draft():
				...
				raise itds.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise itds.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise itds.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if itds.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise itds.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			itds.throw(_("Could not find {0}").format(msg), itds.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			itds.throw(_("Cannot link cancelled document: {0}").format(msg), itds.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			method == "onload"
			or (itds.flags.in_import and itds.flags.mute_emails)
			or itds.flags.in_patch
		...
		from itds.email.doctype.notification.notification import evaluate_alert
		...
		notifications = itds.client_cache.get_value(
	...
	def _rename(
		...
		"""Rename the document. Triggers itds.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus.is_draft():
			raise itds.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = itds.get_doc(self.doctype, self.name, for_update=True)
		except itds.DoesNotExistError:
			if raise_exception:
				raise

			return itds.clear_last_message()
	...
	def run_post_save_methods(self):
		...
		if not (itds.flags.in_import and self.is_new()):
			self.clear_cache()
		...
		if (self.doctype, self.name) in itds.flags.currently_saving:
			itds.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		itds.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			itds.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([itds.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if (
			itds.flags.in_import
			or itds.flags.in_patch
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": itds.session.user}
			itds.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run itds.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in itds.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", itds.session.user)
	...
	def db_get(self, fieldname):
		...
		return itds.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from itds.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or itds.flags.in_install
			or (not self._doc_before_save and itds.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = itds.get_doc(self.doctype, amended_from)
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						itds.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			itds.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or itds.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = itds.get_system_settings("rounding_method")
		...
		return itds.get_doc(
			{
				...
				"comment_email": comment_email or itds.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not itds.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = itds.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				commit_after_response(
					lambda: itds.db.set_value(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = itds.get_doc("Document Share Key", existing_key)
		else:
			doc = itds.new_doc("Document Share Key")
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"itds.model.document.execute_action",
			...
			if timeout:
				for _ in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise itds.DocumentLockedError
		file_lock.create_lock(signature)
		itds.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in itds.local.locked_documents:
			itds.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = _("{0} row #{1}: ").format(
					_(itds.unscrub(self.parentfield)),
					self.idx,
				)
			...
			itds.throw(
				...
				+ _("{0} must be after {1}").format(
					itds.bold(_(self.meta.get_label(to_date_field))),
					itds.bold(_(self.meta.get_label(from_date_field))),
				),
				itds.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = itds.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from itds.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from itds.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = itds.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if itds.message_log:
			msg = itds.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + itds.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	for child_table in child_table_fields:
		valid_column_map[child_table.options] = itds.get_meta(child_table.options).get_valid_columns()
	...
	while document_batch := list(itertools.islice(documents, chunk_size)):
		...
		for dt, docs in values_map.items():
			itds.db.bulk_insert(dt, valid_column_map[dt], docs, ignore_duplicates=ignore_duplicates)
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = itds.session.user
...
def unlock_document(doctype: str, name: str):
	itds.get_lazy_doc(doctype, name).unlock()
	itds.msgprint(itds._("Document Unlocked"), alert=True)


def get_lazy_controller(doctype):
	lazy_controllers = itds.lazy_controllers.setdefault(itds.local.site, {})
...
def copy_doc(doc: "Document", ignore_no_copy: bool = True) -> "Document":
	...
	from itds.model.base_document import BaseDocument
...
def new_doc(
	...
	**kwargs,
	...
	from itds.model.create_new import get_new_doc
...
def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `itds.get_doc`, but return from cache if available."""
	if (key := can_cache_doc(args)) and (doc := itds.cache.get_value(key)):
		return doc
...
def _set_document_in_cache(key: str, doc: "Document") -> None:
	itds.cache.set_value(key, doc, expires_in_sec=3600)
...
def clear_document_cache(doctype: str, name: str | None = None) -> None:
	itds.db.value_cache.pop(doctype, None)

	def clear_in_redis():
		if name is not None:
			itds.cache.delete_value(get_document_cache_key(doctype, name))
		else:
			itds.cache.delete_keys(get_document_cache_key(doctype, ""))

	clear_in_redis()
	if hasattr(itds.db, "after_commit"):
		itds.db.after_commit.add(clear_in_redis)
		itds.db.after_rollback.add(clear_in_redis)

	if doctype == "System Settings" and hasattr(itds.local, "system_settings"):
		delattr(itds.local, "system_settings")

	if doctype == "Website Settings" and hasattr(itds.local, "website_settings"):
		delattr(itds.local, "website_settings")
...
def get_cached_value(
	...
	if isinstance(fieldname, str):
		if as_dict:
			itds.throw("Cannot make dict for single fieldname")
		return doc.get(fieldname)

	values = [doc.get(f) for f in fieldname]
	if as_dict:
		return itds._dict(zip(fieldname, values, strict=False))
	return values
...
def get_single_value(setting: str, fieldname: str, /, *, as_dict: bool = False):
	...
	Usage:
		telemetry_enabled = itds.get_single_value("System Settings", "telemetry_enabled")
	"""
...
def get_last_doc(
	...
	*,
	...
	d = itds.get_all(doctype, filters=filters, limit_page_length=1, order_by=order_by, pluck="name")
	if d:
...
def get_single(doctype):
	"""Return a `itds.model.document.Document` object of the given Single doctype."""
