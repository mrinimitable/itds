...
from itds.model.dynamic_links import invalidate_distinct_link_doctypes
from itds.model.naming import set_new_name
from itds.model.utils.link_count import notify_link_count
...
from itds.utils.defaults import get_not_null_defaults
...
def get_controller(doctype):
	...
	For `custom` type, return `itds.model.document.Document`.

	:param doctype: DocType name as string.
	"""

	site_controllers = itds.controllers.setdefault(itds.local.site, {})
...
def import_controller(doctype):
	from itds.model.document import Document
	from itds.utils.nestedset import NestedSet

	module_name = "Core"
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = itds.db.get_value("DocType", doctype, ("module", "custom", "is_tree"), as_dict=True)
		if doctype_info:
	...
	module_path = None
	class_overrides = itds.get_hooks("override_doctype_class")
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = itds.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = itds.bold(f"{class_.__module__}.{class_.__name__}")
			itds.throw(
				f"{doctype}: {itds.bold(import_path)} must be a subclass of {original_class_path}",
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return itds.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return itds.db.get_value(self.doctype, self.name, key)
	...
	def _init_child(self, value, key):
		...
		if not __dict.get("name"):
			__dict["__islocal"] = 1
			__dict["__temporary_name"] = itds.generate_hash(length=10)
			...
			if df:
				if is_virtual_field:
					...
					if (prop := getattr(type(self), fieldname, None)) and is_a_property(prop):
						...
						from itds.utils.safe_exec import get_safe_globals

						value = itds.safe_eval(
							code=options,
				...
				if isinstance(value, list) and fieldtype not in table_fields:
					itds.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		valid_columns_cache = itds.local.valid_columns

		if self.doctype not in valid_columns_cache:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from itds.model.meta import get_table_columns
	...
	def as_json(self):
		return itds.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and itds.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = itds.session.user
		...
		try:
			...
			if itds.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					itds.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), itds.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise itds.DuplicateEntryError(self.doctype, self.name, e)

			elif itds.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if itds.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if itds.db.db_type == "mariadb":
			...
			itds.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise itds.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			itds.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			itds.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
	...
	def get_invalid_links(self, is_submittable=False):
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if df.fieldtype == "Link":
				doctype = df.options
				if not doctype:
					itds.throw(_("Options not set for link field {0}").format(df.fieldname))
			else:
				assert df.fieldtype == "Dynamic Link"
				doctype = self.get(df.options)
				if not doctype:
					itds.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
			...
			check_docstatus = is_submittable and itds.get_meta(doctype).is_submittable
			...
			if not meta.get("is_virtual"):
				...
				values = itds.get_doc(doctype, docname).as_dict()
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			...
			fetch_from_df = get_default_df(fetch_from_fieldname) or itds.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (itds.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		# data_field options defined in itds.model.data_field_options
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in itds.STANDARD_USERS) and (data in itds.STANDARD_USERS):
					continue
	...
	def _validate_constants(self):
		if itds.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = itds.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				itds.throw(
					...
					itds.CannotChangeConstantError,
				)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				itds.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				itds.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		itds.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, itds.bold(_(df.label, context=df.parent)), max_length, value
			),
			itds.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = itds.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					itds.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							itds.bold(db_value),
							itds.bold(self_value),
						),
						itds.UpdateAfterSubmitError,
	...
	def _save_passwords(self):
		...
		from itds.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def precision(self, fieldname, parentfield=None) -> int | None:
		...
		if fieldname not in self._precision[cache_key]:
			...
			df = itds.get_meta(doctype).get_field(fieldname)
	...
	def get_formatted(
		...
		if (
			...
			currency = itds.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = itds.new_doc(self.doctype)
			else:
