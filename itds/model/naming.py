...
from itds.utils import cint, cstr, now_datetime
...
class InvalidNamingSeriesError(itds.ValidationError):
	pass


class InvalidUUIDValue(itds.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			itds.throw(
				_("Invalid naming series {}: dot (.) missing").format(itds.bold(self.series)),
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			itds.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = itds.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if itds.db.get_value("Series", prefix, "name", order_by="name") is None:
			itds.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(itds.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(itds.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() not in ("prompt", "uuid") and not itds.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = itds.db.get_next_sequence_val(doc.doctype)
		return
	...
	if meta.autoname == "UUID":
		if not doc.name:
			...
			try:
				...
				itds.throw(_("Invalid value specified for UUID: {}").format(doc.name), InvalidUUIDValue)
		return
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			itds.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from itds.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = itds.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		itds.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		itds.throw(itds._("Naming Series mandatory"))
...
def _generate_random_string(length=10):
	"""Better version of itds.generate_hash for naming.
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = itds.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return itds.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	# series created ?
	# Using itds.qb as itds.get_values does not allow order_by=None
	series = DocType("Series")
	current = (itds.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		itds.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		itds.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (itds.qb.from_(series).where(series.name == prefix).for_update().select("current")).run()

	if current and current[0][0] == count:
		itds.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = itds.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
	if not name:
		itds.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			itds.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		itds.throw(_("Invalid name type (integer) for varchar name column"), itds.NameError)

	if name.startswith("New " + doctype):
		itds.throw(
			_("There were some errors setting the name, please contact the administrator"), itds.NameError
		)
	name = name.strip()

	if not itds.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		itds.throw(_("Name of {0} cannot be {1}").format(doctype, name), itds.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		itds.throw(_("Name cannot contain special characters like {0}").format(message), itds.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = itds.db.exists(doctype, filters)
	...
	if exists:
		last = itds.db.sql(
			...
			WHERE `{fieldname}` {itds.db.REGEX_CHARACTER} %s
...
def _set_amended_name(doc):
	amend_naming_rule = itds.db.get_value(
		"Amended Document Naming Settings", {"document_type": doc.doctype}, "action", cache=True
	)
	if not amend_naming_rule:
		amend_naming_rule = itds.get_single_value("Document Naming Settings", "default_amend_naming")
	...
	if itds.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		itds.throw(_("Please set the document name"))
