"""
Traced Fields for Itds

This module provides utilities for creating traced fields in Itds documents,
...
- Seamlessly integrate with Itds's document model

Example of standard usage:
		from itds.model.trace import TracedDocument, traced_field
...
See itds.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in Itds documents.

	This class allows for monitoring and validating changes to specific fields
	in a Itds document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if itds.in_test:
				itds.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				itds.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if itds.in_test:
					itds.throw(str(e), AssertionError)
				else:
					itds.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for Itds documents with traced fields.

	This class extends Itds's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from itds.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in Itds documents.
...
from itds.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
