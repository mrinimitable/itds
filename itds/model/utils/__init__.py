...
def set_field_property(filters, key, value):
	...
	docs = [
		itds.get_doc("DocType", d.parent)
		for d in itds.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(itds.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(itds.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if itds.flags.in_install or itds.flags.in_migrate:
		return (
			itds.db.get_value("DocType", doctype, "is_virtual")
			if itds.db.has_column("DocType", "is_virtual")
			else False
		)

	return getattr(itds.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if itds.flags.in_install or itds.flags.in_migrate:
		return itds.db.get_value("DocType", doctype, "issingle")
	else:
		return getattr(itds.get_meta(doctype), "issingle", False)
