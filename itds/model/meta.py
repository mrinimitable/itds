...
from itds.model.base_document import (
...
from itds.utils import cached_property, cast, cint, cstr
from itds.utils.caching import site_cache
from itds.utils.data import add_to_date, get_datetime
...
def get_meta(doctype: "str | DocType", cached: bool = True) -> "_Meta":
	...
	if (
		...
		and (meta := itds.client_cache.get_value(f"doctype_meta::{doctype}"))
	):
		return meta

	meta = Meta(doctype)
	key = f"doctype_meta::{meta.name}"
	itds.client_cache.set_value(key, meta)
	return meta
...
def clear_meta_cache(doctype: str = "*"):
	...
	if doctype == "*":
		itds.client_cache.delete_keys(key)
	else:
		itds.client_cache.delete_value(key)
...
def get_table_columns(doctype):
	return itds.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = itds.scrub(doctype)
	with open(itds.get_app_path("itds", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		itds._dict(fieldname="creation", fieldtype="Datetime"),
		itds._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(itds._dict(fieldtype="Data", fieldname="name", label="Name"))
	...
	def _valid_columns(self):
		table_exists = itds.db.table_exists(self.name)
	...
	def _valid_fields(self):
		if (itds.flags.in_install or itds.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not itds.db.table_exists("Custom Field"):
			return

		custom_fields = itds.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not itds.db.table_exists("Property Setter"):
			return

		property_setters = itds.db.get_values(
			"Property Setter",
	...
	def check_if_large_table(self):
		...
		if self.istable or not itds.db.table_exists(self.name):  # During install, new migrate
			return

		if itds.db.estimate_count(self.name) > LARGE_TABLE_SIZE_THRESHOLD:
			recent_change = itds.db.get_value(self.name, {}, "creation", order_by="creation desc")
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(itds._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if itds.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if itds.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = itds.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Itds applications via hooks.
		"""
		data = itds._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = itds._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in itds.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = itds._dict(itds.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = itds.scrub(self.module)
		doctype = itds.scrub(self.name)
		template_path = itds.get_module_path(
...
def get_parent_dt(dt):
	if not itds.is_table(dt):
		return ""

	return (
		itds.db.get_value(
			"DocField",
			{"fieldtype": ("in", itds.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	itds.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(itds.local, "field_currency", None):
		itds.local.field_currency = itds._dict()

	if not (
		itds.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and itds.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = itds.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if itds.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = itds.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			itds.local.field_currency.setdefault((doc.doctype, ref_docname), itds._dict()).setdefault(
				df.fieldname, currency
			)

	return itds.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and itds.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from itds.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(itds.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(itds.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return itds._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return itds._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return itds._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return itds._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in itds.get_all("DocType", filters=filters, pluck="name"):
		try:
			dropped_columns = trim_table(doctype, dry_run=dry_run)
			if dropped_columns:
				UPDATED_TABLES[doctype] = dropped_columns
		except itds.db.TableMissingError:
			if quiet:
				continue
...
def trim_table(doctype, dry_run=True):
	...
	itds.cache.delete_value(key)
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = itds.db.get_table_columns(doctype)
	fields = itds.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		itds.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
