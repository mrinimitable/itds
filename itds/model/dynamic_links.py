...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(itds.local, "dynamic_link_map", None) is None or itds.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		itds.local.dynamic_link_map = dynamic_link_map
	return itds.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += itds.db.sql(query, as_dict=True)
	return df
...
def fetch_distinct_link_doctypes(doctype: str, fieldname: str):
	...
	doctypes = itds.cache.get_value(key)

	if doctypes is None:
		doctypes = itds.db.sql(f"""select distinct `{fieldname}` from `tab{doctype}`""", pluck=True)
		itds.cache.set_value(key, doctypes, expires_in_sec=12 * 60 * 60)
...
def invalidate_distinct_link_doctypes(doctype: str, fieldname: str, linked_doctype: str):
	...
	if linked_doctype not in doctypes:
		...
		itds.db.after_commit.add(lambda: itds.cache.delete_value(key))
