"""This file houses all Itds specific optimizations and hooks that run on startup or during fork.
...
def optimize_all():
	...
	# Note:
	# - This function is ALWAYS executed as soon as `import itds` ends.
...
def optimize_gc_parameters():
	from itds.utils import sbool

	if not bool(sbool(os.environ.get("ITDS_TUNE_GC", True))):
		return

	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for itds where a single query can potentially create 700+
	# objects easily.
...
def freeze_gc():
	...
	#
	# Refer to issue for more info: https://github.com/mrinimitable/itds/issues/18927
	gc.collect()
	gc.freeze()
...
def optimize_for_gil_contention():
	if not os.environ.get("ITDS_PERF_PIN_WORKERS"):
		return

	if "gunicorn" not in str(sys.argv[0]):
		return

	if os.environ.get("ITDS_PERF_PIN_WORKERS_DETERMINISTIC"):
		# Ensure same pinning order every time.
		# This is only useful for shashimarking, DO NOT enable this in production.
		global worker_num
		worker_num = 0
...
def parse_thread_siblings() -> list[tuple[int, int]] | None:
	try:
		...
		logger = itds.logger(with_more_info=True)
