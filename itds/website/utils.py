...
def delete_page_cache(path=None):
	if path:
		itds.cache.delete_value(f"{WEBSITE_PAGE_CACHE_PREFIX}{path}")
	else:
		itds.cache.delete_keys(WEBSITE_PAGE_CACHE_PREFIX)
...
def can_cache(no_cache=False):
	if itds.flags.force_website_cache:
		return True
	if itds.conf.disable_website_cache or itds.conf.developer_mode:
		return False
	if getattr(itds.local, "no_cache", False):
		return False
	if itds.request and itds.request.query_string:
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = itds.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", itds.session.user], ["published", "=", 1]],
	)

	communications = itds.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{itds.session.user}%"],
			["cc", "like", f"%{itds.session.user}%"],
			["bcc", "like", f"%{itds.session.user}%"],
		],
	)
...
def get_home_page():
	if itds.local.flags.home_page and not itds.in_test:
		return itds.local.flags.home_page
	...
	def _get_home_page():
		...
		if itds.session.user != "Guest":
			# by role
			for role in itds.get_roles():
				home_page = itds.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = itds.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = itds.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if itds.session.user == "Guest" else "me"
	...
	return itds.cache.hget("home_page", itds.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = itds.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = itds.get_attr(home_page_method[-1])(itds.session.user)
	elif itds.get_hooks("website_user_home_page"):
		home_page = itds.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = itds.get_hooks("role_home_page")
		if role_home_page:
			for role in itds.get_roles():
	...
	if not home_page:
		home_page = itds.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			"float_precision": cint(itds.get_system_settings("float_precision")) or 3,
			"date_format": get_date_format(),
			"time_format": get_time_format(),
			"first_day_of_the_week": get_first_day_of_the_week(),
			"number_format": get_number_format().string,
			"currency": itds.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": itds.get_cached_value("User", itds.session.user, "time_zone")
			or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": itds.local.site,
		"is_fc_site": 1 if is_fc_site() else 0,
	}


def is_signup_disabled():
	return itds.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return itds.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from itds.website.router import get_pages

	if not itds.local.flags.children_map:
		...
		children_map = itds.cache.get_value("website_full_index", _build)

		itds.local.flags.children_map = children_map

	return itds.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		itds.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		itds.clear_cache("Guest")
		delete_page_cache()
		keys += [
			"portal_menu_items",
			"home_page",
	...
	itds.cache.delete_value(keys)

	for method in itds.get_hooks("website_clear_cache"):
		itds.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = itds.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else itds.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = itds.cache.hget("portal_menu_items", itds.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = itds.get_roles()
		portal_settings = itds.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = itds.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		itds.cache.hset("portal_menu_items", itds.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			itds.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		...
		no_cache = itds.request and itds.request.cache_control.no_cache
		if can_cache(no_cache):
			html = None
			page_cache = itds.cache.get_value(cache_key)
			if page_cache and itds.local.lang in page_cache:
				html = page_cache[itds.local.lang]
			if html:
				itds.local.response.from_cache = True
				itds.local.response_headers.update(cache_headers)
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = itds.cache.get_value(cache_key) or {}
			page_cache[itds.local.lang] = html
			itds.cache.set_value(cache_key, page_cache, expires_in_sec=30 * 60)
			itds.local.response_headers.update(cache_headers)
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = itds.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in itds.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in itds.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in itds.local.preload_assets["icons"]
	)
