...
import itds
from itds.website.page_renderers.base_template_page import BaseTemplatePage
from itds.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in reversed(itds.get_installed_apps()):
	...
	def get_index_path_options(search_path):
		return (
			itds.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = itds.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = itds.get_module(self.pymodule_name)
		...
		self.context.developer_mode = itds.conf.developer_mode
	...
	def get_raw_template(self):
		return itds.get_jloader().get_source(itds.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = itds.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "itds"
		self.app_path = itds.get_app_path("itds")
		self.path = path
	...
	def set_user_info(self):
		from itds.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(itds.session.user)
