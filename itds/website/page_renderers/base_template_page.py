import itds
from itds.website.doctype.website_settings.website_settings import get_website_settings
from itds.website.page_renderers.base_renderer import BaseRenderer
from itds.website.website_components.metatags import MetaTags
...
class BaseTemplatePage(BaseRenderer):
	...
	def init_context(self):
		self.context = itds._dict()
		self.context.update(get_website_settings())
		self.context.update(itds.local.conf.get("website_context") or {})

	def add_csrf_token(self, html):
		if itds.local.session:
			csrf_token = itds.local.session.data.csrf_token
			return html.replace(
				"<!-- csrf_token -->", f'<script>itds.csrf_token = "{csrf_token}";</script>'
			)
	...
	def set_base_template_if_missing(self):
		if not self.context.base_template_path:
			app_base = itds.get_hooks("base_template")
	...
	def set_missing_values(self):
		# set using itds.respond_as_web_page
		if hasattr(itds.local, "response") and itds.local.response.get("context"):
			self.context.update(itds.local.response.context)

		# to be able to inspect the context dict
		# Use the macro "inspect" from macros.html
		self.context.canonical = itds.utils.get_url(itds.utils.escape_html(self.path))
		...
		self.context.pathname = getattr(itds.local, "path", None) if hasattr(itds, "local") else self.path

	def update_website_context(self):
		# apply context from hooks
		update_website_context = itds.get_hooks("update_website_context")
		for method in update_website_context:
			values = itds.get_attr(method)(self.context)
			if values:
