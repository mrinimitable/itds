...
itds.provide("website");
itds.provide("itds.awesome_bar_path");
window.cur_frm = null;
...
$.extend(itds, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => itds.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = itds.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (itds._assets_loaded.includes(link)) return resolve();
			let el;
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					itds.msgprint(__("Not found"));
				},
				403: function () {
					itds.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			itds.process_response(opts, data);
		});
	},
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return itds.call({
			type: "GET",
			method: "itds.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (itds.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", itds.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				itds.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				itds.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				itds.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	...
	unfreeze: function () {
		if (!itds.freeze_count) return; // anything open?
		itds.freeze_count--;
		if (!itds.freeze_count) {
	...
	trigger_ready: function () {
		itds.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = itds.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		itds.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				itds.do_search(val);
	...
	do_search: function (val) {
		var path =
			(itds.awesome_bar_path && itds.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		itds.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return itds.get_cookie("user_id") !== "Guest" && itds.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (itds.session.user === "Guest" && window.show_language_picker) {
			itds
				.call("itds.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = itds.get_cookie("preferred_language");
					let language_codes = [];
...
itds.setup_search = function (target, search_scope) {
...
window.valid_email = function (id) {
	// copied regex from itds/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = itds.get_cookie("full_name");
	var logged_in = itds.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	itds.bind_navbar_search();

	// add apps link
	let apps = itds.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = itds.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && itds.add_apps(obj);
	}

	// switch to app link
	if (itds.get_cookie("system_user") === "yes" && logged_in) {
		itds.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(itds, itds.get_cookies());
	itds.session = { user: itds.user_id };

	itds.datetime.refresh_when();
	itds.trigger_ready();
	itds.bind_filters();
	itds.highlight_code_blocks();
	itds.add_link_to_headings();
	itds.make_navbar_active();
	// scroll to hash
...
itds.ready(function () {
	itds.show_language_picker();
	itds.setup_videos();
	itds.realtime.init(window.socketio_port, true); // lazy connection
});
