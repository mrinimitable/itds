...
import itds
from itds.website.page_renderers.document_page import DocumentPage
from itds.website.page_renderers.list_page import ListPage
from itds.website.page_renderers.not_found_page import NotFoundPage
from itds.website.page_renderers.print_page import PrintPage
from itds.website.page_renderers.redirect_page import RedirectPage
...
from itds.website.page_renderers.web_form import WebFormPage
from itds.website.router import evaluate_dynamic_routes
from itds.website.utils import can_cache, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		if hasattr(itds.local, "request"):
			request = itds.local.request or request
		...
		if request.url and can_cache() and itds.cache.hget("website_404", request.url):
			return self.path, NotFoundPage(self.path)

		try:
			resolve_redirect(self.path, request.query_string)
		except itds.Redirect as e:
			return itds.flags.redirect_location, RedirectPage(self.path, e.http_status_code)

		if itds.get_hooks("website_path_resolver"):
			for handler in itds.get_hooks("website_path_resolver"):
				endpoint = itds.get_attr(handler)(self.path)
		else:
			try:
				endpoint = resolve_path(self.path)
			except werkzeug.routing.exceptions.RequestRedirect as e:
				itds.flags.redirect_location = e.new_url
				return itds.flags.redirect_location, RedirectPage(e.new_url, e.code)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in itds.get_hooks("page_renderer") or []:
			try:
				renderer = itds.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	redirect_to = itds.cache.hget("website_redirects", path or "/")
	if redirect_to:
		if isinstance(redirect_to, dict):
			itds.flags.redirect_location = redirect_to["path"]
			raise itds.Redirect(redirect_to["status_code"])
		itds.flags.redirect_location = redirect_to
		raise itds.Redirect

	if redirect_to is False:
		return

	redirects = itds.get_hooks("website_redirects")
	redirects += [
		{"source": r.source, "target": r.target, "redirect_http_status": r.redirect_http_status}
		for r in (itds.get_website_settings("route_redirects") or [])
	]
	...
	for rule in redirects:
		...
		if query_string and rule.get("match_with_query_string"):
			path_to_match = path + "?" + itds.safe_decode(query_string)

		try:
			match = re.match(pattern, path_to_match)
		except re.error:
			itds.log_error("Broken Redirect: " + pattern)
		...
		if match:
			...
			raise itds.Redirect(status_code)

	itds.cache.hset("website_redirects", path_to_match or "/", False)
...
def get_website_rules():
	...
	def _get():
		rules = itds.get_hooks("website_route_rules")
		for d in itds.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
	...
	return itds.cache.get_value("website_route_rules", _get)


def validate_path(path: str):
	if not PathResolver(path).is_valid_path():
		itds.throw(itds._("Path {0} it not a valid path").format(itds.bold(path)))
