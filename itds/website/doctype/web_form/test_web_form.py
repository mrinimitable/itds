...
from itds.website.doctype.web_form.web_form import accept
...
class TestWebForm(IntegrationTestCase):
	def setUp(self):
		itds.conf.disable_website_cache = True

	def tearDown(self):
		itds.conf.disable_website_cache = False
	...
	def test_accept(self):
		...
		self.event_name = itds.db.get_value("Event", {"subject": "_Test Event Web Form"})
	...
	def test_edit(self):
		...
		self.assertNotEqual(
			itds.db.get_value("Event", self.event_name, "description"), doc.get("description")
		)

		accept("manage-events", json.dumps(doc))

		self.assertEqual(itds.db.get_value("Event", self.event_name, "description"), doc.get("description"))
	...
	def test_webform_html_meta_is_added(self):
		...
		self.assertIn(
			self.normalize_html('<meta property="og:image" content="https://itds.io/files/itds.png">'),
			content,
		)
