itds.ui.form.on("Web Form", {
	setup: function () {
		itds.meta.docfield_map["Web Form Field"].fieldtype.formatter = (value) => {
			const prefix = {
				"Page Break": "--red-600",
		...
		itds.meta.docfield_map["Web Form Field"].fieldname.formatter = (value) => {
			if (!value) return;
			return itds.unscrub(value);
		};

		itds.meta.docfield_map["Web Form List Column"].fieldname.formatter = (value) => {
			if (!value) return;
			return itds.unscrub(value);
		};
	},
	...
	refresh: function (frm) {
		...
		frm.sidebar
			...
			.attr("href", "#")
			.on("click", () => {
				const url = itds.urllib.get_full_url(frm.doc.route);
				const code = `<iframe src="${url}" style="border: none; width: 100%; height: inherit;"></iframe>`;
				itds.utils.copy_to_clipboard(code, __("Embed code copied"));
			});

		if (frm.doc.is_standard && !itds.boot.developer_mode) {
			frm.disable_form();
			itds.show_alert(
	...
	validate: function (frm) {
		...
		if (!frm.doc.web_form_fields) {
			...
			itds.throw(__("At least one field is required in Web Form Fields Table"));
		}
		...
		if (page_break_count >= 10) {
			itds.throw(__("There can be only 9 Page Break fields in a Web Form"));
		}
	},
	...
	add_get_fields_button(frm) {
		frm.add_custom_button(__("Get Fields"), () => {
			let webform_fieldtypes = itds.meta
...
itds.ui.form.on("Web Form List Column", {
	fieldname: function (frm, doctype, name) {
		let doc = itds.get_doc(doctype, name);
		let df = itds.meta.get_docfield(frm.doc.doc_type, doc.fieldname);
		if (!df) return;
...
itds.ui.form.on("Web Form Field", {
...
function get_fields_for_doctype(doctype) {
	return new Promise((resolve) => itds.model.with_doctype(doctype, resolve)).then(() => {
		return itds.meta.get_docfields(doctype).filter((df) => {
			return (
