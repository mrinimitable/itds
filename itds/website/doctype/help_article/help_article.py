# Copyright (c) 2013, Itds and contributors
# License: MIT. See LICENSE

import itds
from itds import _
from itds.rate_limiter import rate_limit
from itds.utils import cint, is_markdown, markdown
from itds.website.utils import get_comment_list
...
class HelpArticle(WebsiteGenerator):
	...
	def set_route(self):
		...
		if not self.route:
			self.route = "/".join(
				[itds.get_value("Help Category", self.category, "route"), self.scrub(self.title)]
			)
	...
	def update_category(self):
		cnt = itds.db.count("Help Article", filters={"category": self.category, "published": 1})
		cat = itds.get_doc("Help Category", self.category)
	...
	def get_context(self, context):
		...
		context.category = itds.get_doc("Help Category", self.category)
...
def get_list_context(context=None):
	...
	category = itds.db.get_value("Help Category", {"route": itds.local.path})
	...
	list_context = itds._dict(
		...
		hide_filters=True,
		filters=filters,
		category=itds.local.form_dict.category,
		no_breadcrumbs=True,
	)

	if itds.local.form_dict.txt:
		list_context.blog_subtitle = _('Filtered by "{0}"').format(itds.local.form_dict.txt)
	#
	# list_context.update(itds.get_doc("Blog Settings", "Blog Settings").as_dict())
	return list_context
...
def get_sidebar_items():
	...
	return itds.cache.get_value("knowledge_base:category_sidebar", _get)


def clear_knowledge_base_cache():
	itds.cache.delete_value("knowledge_base:category_sidebar")
	itds.cache.delete_value("knowledge_base:faq")
...
def add_feedback(article: str, helpful: str):
	...
	value = cint(itds.db.get_value("Help Article", article, field))
	itds.db.set_value("Help Article", article, field, value + 1, update_modified=False)
