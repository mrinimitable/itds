...
class TestWebTemplate(IntegrationTestCase):
	def test_render_web_template_with_values(self):
		doc = itds.get_doc("Web Template", "Hero with Right Image")
		values = {
			"title": "Test Hero",
	...
	def test_custom_stylesheet(self):
		...
		itds.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not itds.db.exists("Web Page", "test-web-template"):
			itds.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not itds.db.exists("Website Theme", "Custom"):
			theme = itds.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = itds.get_doc("Website Theme", "Custom")
		return theme
