...
from itds.utils import random_string, set_request
from itds.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(IntegrationTestCase):
	...
	def test_generator_not_found(self):
		pages = itds.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{itds.generate_hash(length=5)}"

		itds.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = itds.safe_decode(blog_page_response.get_data())
		...
		category_page_html = itds.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		itds.delete_doc("Blog Post", blog.name)
		itds.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = itds._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			itds.delete_doc(blog.doctype, blog.name)
		itds.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		itds.flags.force_website_cache = True
		print(itds.session.user)
	...
	def test_like_dislike(self):
		...
		itds.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from itds.templates.includes.likes.likes import like
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not itds.db.exists("Blog Category", category_name):
		itds.get_doc(doctype="Blog Category", title=category_title).insert()
	if not itds.db.exists("Blogger", "test-blogger"):
		itds.get_doc(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger").insert()
