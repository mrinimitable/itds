...
import itds
from itds import _, safe_encode, task
from itds.core.utils import html2text
from itds.database.database import savepoint
from itds.email.doctype.email_account.email_account import EmailAccount
from itds.email.email_body import add_attachment, get_email, get_formatted_html
from itds.email.itdsmail import ItdsMail
from itds.email.queue import get_unsubcribed_url, get_unsubscribe_message
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from itds.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if itds.session.user != "Administrator":
			itds.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = itds.get_doc(values)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = itds.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return itds.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			itds.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(itds.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(
		...
		itds_mail_client: ItdsMail = None,
		...
		with SendMailContext(self, smtp_server_instance, itds_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not itds.in_test or itds.flags.testing_email:
					if ctx.email_account_doc.service == "Itds Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.itds_mail_client.send_raw(
							sender=self.sender,
			...
			if itds.in_test and not itds.flags.testing_email:
				itds.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = itds.qb.DocType("Email Queue")
		email_recipient = itds.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			itds.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			itds.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from itds.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		self.itds_mail_client: ItdsMail = itds_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Itds Mail":
			if not self.itds_mail_client:
				self.itds_mail_client = self.email_account_doc.get_itds_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": itds.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/itds.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = itds.get_doc("File", file_filters)
				...
				print_format_file = itds.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not itds.get_system_settings("store_attached_pdf_document"):
			return

		file_data = itds._dict(file_name=file_name, is_private=1)
		...
		if itds.db.exists("File", file_data):
			return

		file = itds.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not itds.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = itds.qb.DocType("Email Queue")
	itds.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, itds.session.user
...
def toggle_sending(enable):
	...
	itds.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	itds.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	itds.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(itds.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/itds.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/itds.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				itds.qb.from_(EmailUnsubscribe)
	...
	def prepare_email_content(self):
		...
		if self.in_reply_to:
			if message_id := itds.db.get_value("Communication", self.in_reply_to, "message_id"):
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in itds.utils.create_batch(final_recipients, 1000):
				itds.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=itds.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=itds.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		itds_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not itds_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Itds Mail":
					itds_mail_client = email_account.get_itds_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, itds_mail_client=itds_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except itds.InvalidEmailAddressError:
