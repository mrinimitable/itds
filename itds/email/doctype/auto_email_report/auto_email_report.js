...
itds.ui.form.on("Auto Email Report", {
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Download"), function () {
				var w = window.open(
					itds.urllib.get_full_url(
						"/api/method/itds.email.doctype.auto_email_report.auto_email_report.download?" +
							"name=" +
			...
			frm.add_custom_button(__("Send Now"), function () {
				itds.call({
					method: "itds.email.doctype.auto_email_report.auto_email_report.send_now",
					args: { name: frm.doc.name },
					callback: function () {
						itds.msgprint(__("Scheduled to send"));
					},
				});
			});
		} else {
			if (!frm.doc.user) {
				frm.set_value("user", itds.session.user);
			}
			if (!frm.doc.email_to) {
				frm.set_value("email_to", itds.session.user);
			}
		}
	...
	fetch_report_filters(frm) {
		if (
			...
			itds.call({
				method: "itds.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report,
				},
				callback: function (r) {
					itds.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: async function (frm) {
		...
		wrapper.empty();
		let reference_report = itds.query_reports[frm.doc.report];
		if (!reference_report || !reference_report.filters) {
			reference_report = await itds.model.with_doc("Report", frm.doc.report);
		}
		if (
			...
			if (
				...
				if (frm.doc.filters) {
					...
					itds.db.get_value("Report", frm.doc.report, "json", (r) => {
						if (r && r.json) {
				...
				report_filters = itds.query_reports[frm.doc.reference_report].filters;
			} else {
				filters = JSON.parse(frm.doc.filters || "{}");
				...
				if (val.fieldtype != "Break") {
					if (val.fieldtype === "MultiSelectList") {
						val.get_data = (txt) => {
							...
							const doctype_link =
								itds.scrub(val.options) === val.options
									? dialog.get_value(val.options)
									: val.options;

							return doctype_link
								? itds.db.get_link_options(doctype_link, txt)
								: [];
						};
					}
			...
			report_filters.forEach((f) => {
				...
				$("<td>" + itds.format(filters[f.fieldname], f) + "</td>")
					.css(css)
					.appendTo(row);
			});
