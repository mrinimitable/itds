...
import itds
from itds import _
from itds.desk.query_report import build_xlsx_data
from itds.email.email_body import get_formatted_html
...
from itds.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	...
	def autoname(self):
		...
		if itds.db.exists("Auto Email Report", self.name):
	...
	def sender_email(self):
		return itds.db.get_value("Email Account", self.sender, "email_id")
	...
	def validate_report_count(self):
		count = itds.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(itds.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(itds.get_system_settings("max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			itds.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			itds.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					itds.bold(self.format), itds.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = itds.parse_json(self.filters) if self.filters else {}
		filter_meta = itds.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = [
	...
	def get_report_content(self):
		...
		report = itds.get_doc("Report", self.report)

		self.filters = itds.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, itds._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			itds.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):
		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = itds.db.get_value("Report", self.report, "ref_doctype")

		return itds.render_template(
			"itds/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = itds.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			itds.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = itds.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		itds.msgprint(_("No Data"))
		return

	itds.local.response.filecontent = data
	itds.local.response.type = "download"
	itds.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = itds.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		itds.enqueue(
			"itds.email.doctype.auto_email_report.auto_email_report.process_auto_email_report",
			report=report,
			queue="long",
		)
...
def process_auto_email_report(report):
	...
	auto_email_report = itds.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in itds.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		itds.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = None
				if doc_name and col.get("parent") and not itds.get_meta(col.parent).istable:
					if itds.db.exists(col.parent, doc_name):
						doc = itds.get_doc(col.parent, doc_name)

				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = itds.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
