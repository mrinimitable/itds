...
def get_test_notification(config):
	try:
		notification = itds.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
		itds.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		...
		if not itds.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = itds.new_doc("Notification")
		...
		if not itds.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = itds.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		communication = itds.new_doc("Communication")
		...
		self.assertEqual(itds.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = itds.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = itds.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(itds.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		from itds.utils import add_to_date, now_datetime
		...
		with get_test_notification(notification) as n:
			itds.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, itds.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(itds.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		itds.get_doc(
			"Scheduled Job Type",
			dict(method="itds.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = itds.utils.add_days(itds.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		itds.db.delete("User", {"email": "test_jinja@example.com"})
		itds.db.delete("Email Queue")
		itds.db.delete("Email Queue Recipient")

		test_user = itds.new_doc("User")
		...
		self.assertTrue(itds.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = itds.utils.add_days(user.birth_date, 1).date()
			user.save()

			user.reload()
			user.birth_date = itds.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, itds.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		itds.delete_doc_if_exists("Notification", "ToDo Status Update")
		itds.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = itds.get_doc(
			{
				...
				"message": "{% set val = itds.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
...
❯ shashi run-tests --module itds.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		itds.db.delete("Event", {"name": self.event.name})
# 		itds.db.delete("Notification", {"name": self.notification["name"]})
# 		itds.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							itds.db.count("Notification Log", {"subject": n.subject}),
