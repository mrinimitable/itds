...
itds.notification = {
	setup_fieldname_select: function (frm) {
		...
		itds.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (itds.model.table_fields.includes(df.fieldtype)) {
						let child_fields = itds.get_doc("DocType", df.options).fields;
			...
			let fields = itds.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return itds.model.no_value_type.includes(d.fieldtype)
					? null
...
itds.ui.form.on("Notification", {
	...
	refresh: function (frm) {
		itds.notification.setup_fieldname_select(frm);
		itds.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(itds.boot.developer_mode);
		frm.trigger("event");
		if (frm.doc.document_type) {
	...
	view_properties: function (frm) {
		itds.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			itds.call({
				method: "itds.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						itds.msgprint(r.message.toString());
					} else {
						itds.msgprint(__("No alerts for today"));
					}
				},
			});
		});
