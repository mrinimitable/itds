...
import itds
from itds import _
from itds.core.doctype.role.role import get_info_based_on_role, get_user_info
from itds.core.doctype.sms_settings.sms_settings import send_sms
from itds.desk.doctype.notification_log.notification_log import enqueue_create_notification
from itds.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from itds.model.document import Document
from itds.modules.utils import export_module_json, get_doc_module
from itds.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from itds.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = itds.get_cached_doc(self.document_type, preview_document)
			context = get_context(doc)
			return _("Yes") if itds.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = itds.render_template(self.message, context)
			if self.channel == "SMS":
				return itds.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return itds.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			itds.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				itds.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				itds.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			itds.throw(_("Please specify which value field must be checked"))
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not itds.conf.developer_mode:
			itds.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = itds.new_doc(self.document_type)
		if self.condition:
			try:
				itds.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				itds.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			itds.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = itds.get_lazy_doc(self.document_type, d.name)

			if self.condition and not itds.safe_eval(self.condition, None, get_context(doc)):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		Example:
		              ...
		              ```python
		              notification = itds.get_doc("Notification", "My Notification", ignore_permissions=True)
		...
		return enqueue(
			"itds.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
			now=itds.in_test,
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in itds.model.numeric_fieldtypes:
						value = itds.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = itds.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": itds.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from itds.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = itds.render_template(self.message, context)
		...
		if doc.doctype != "Communication":
			...
			comm = itds.get_lazy_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=itds.utils.strip_html_tags(itds.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not itds.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = itds.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": itds.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), itds.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return itds.utils.md_to_html(template)
...
def clear_notification_cache():
	itds.client_cache.delete_keys("notifications::")


@itds.whitelist()
def get_documents_for_today(notification):
	notification = itds.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if itds.flags.in_import or itds.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = itds.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = itds.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not itds.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not itds.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			itds.utils.get_link_to_form("Notification", alert.name)
		)
		itds.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = itds.get_traceback(with_context=True)
		itds.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		itds.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	Itds = namedtuple("Itds", ["itds"])
	itds = Itds(itds=get_safe_globals().get("itds"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"itds": itds.itds,
...
def get_emails_from_template(template, context):
	...
	emails = itds.render_template(template, context) if "{" in template else template
