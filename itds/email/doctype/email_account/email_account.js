...
function oauth_access(frm) {
	itds.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = itds.get_doc("Connected App", frm.doc.connected_app);
		return itds.call({
			doc: connected_app,
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		itds.call({
			method: "itds.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
itds.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(itds.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(itds.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"itds.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "itds.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (itds.route_flags.delete_user_from_locals && itds.route_flags.linked_user) {
			delete itds.route_flags.delete_user_from_locals;
			delete locals["User"][itds.route_flags.linked_user];
		}

		if (!frm.is_dirty() && frm.doc.enable_incoming) {
			frm.add_custom_button(__("Pull Emails"), () => {
				itds.dom.freeze(__("Pulling emails..."));
				frm.call({
					method: "pull_emails",
					...
					if (!(r._server_messages && r._server_messages.length)) {
						itds.show_alert({ message: __("Emails Pulled"), indicator: "green" });
					}
				});
			});
		}
	...
	validate_itds_mail_settings: function (frm) {
		if (frm.doc.service == "Itds Mail") {
			itds.call({
				doc: frm.doc,
				method: "validate_itds_mail_settings",
			});
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			itds.call({
				method: "itds.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: itds.utils.debounce((frm) => {
		if (frm.doc.domain) {
			itds.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			itds.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			itds.confirm(msg, null, function () {
				...
				itds.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
