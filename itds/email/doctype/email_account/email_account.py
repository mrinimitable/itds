...
import itds
from itds import _, are_emails_muted, safe_encode
from itds.desk.doctype.notification_log.notification_log import enqueue_create_notification
from itds.desk.form import assign_to
from itds.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from itds.email.itdsmail import ItdsMail
from itds.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from itds.email.smtp import SMTPServer
from itds.email.utils import get_port
from itds.model.document import Document
from itds.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from itds.utils.background_jobs import enqueue, get_jobs
from itds.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(itds.local, cache_name):
				setattr(itds.local, cache_name, {})

			cached_accounts = getattr(itds.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from itds.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		service: DF.Literal[
			"", "Itds Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				itds.throw(_("Login Id is required"))
		else:
			self.login_id = None
		...
		if (
			not itds.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "Itds Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						itds.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				itds.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						itds.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@itds.whitelist()
	def validate_itds_mail_settings(self):
		if self.service == "Itds Mail":
			itds_mail_client = self.get_itds_mail_client()
			itds_mail_client.validate()

	def validate_smtp_conn(self):
		if not self.smtp_server:
			itds.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					itds.bold(_("Default Incoming")),
					itds.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					itds.bold(_("Default Outgoing")),
					itds.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def clear_cache(self):
		super().clear_cache()
		itds.client_cache.delete_value("automatic_linking_email")
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in itds.get_all("Email Account", filters={field: 1}):
	...
	def get_domain_values(self, domain: str):
		return itds.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			itds.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(itds._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive:
				...
				description = itds.message_log.pop() if itds.message_log else "Socket Error"
	...
	@property
	def _password(self):
		raise_exception = not (self.auth_method == "OAuth" or self.no_smtp_authentication or itds.in_test)
	...
	def from_record(cls, record):
		email_account = itds.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return itds.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = itds.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not itds.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Itds"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [itds.conf.get(k) for k in conf_names if itds.conf.get(k)]
	...
	def get_itds_mail_client(self):
		return self._itds_mail_client

	@functools.cached_property
	def _itds_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return ItdsMail(self.itds_mail_site, self.email_id, access_token=access_token)

			itds.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					itds.bold(self.email_account_name)
				),
				title=_("Itds Mail OAuth Error"),
			)
		else:
			return ItdsMail(
				self.itds_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			itds.enqueue(self._disable_broken_incoming_account, description=description)
		else:
	...
	def get_failed_attempts_count(self):
		return cint(itds.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, itds.get_traceback())
					exceptions.append(itds.get_traceback())
				except Exception:
					itds.db.rollback()
				else:
					itds.db.commit()
		...
		if exceptions:
			raise Exception(itds.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "Itds Mail":
				itds_mail_client = self.get_itds_mail_client()
				messages = itds_mail_client.pull_raw(last_synced_at=self.last_synced_at)
	...
	def send_auto_reply(self, communication, email):
		...
		from itds.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			itds.sendmail(
				...
				or itds.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = itds.qb.DocType("Communication")
		itds.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		itds.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				itds.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if itds.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				itds.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = itds.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in itds.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in itds.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in itds.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in itds.get_all(
				...
				comm = itds.get_doc("Communication", comm.name)

				if itds.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from itds.integrations.doctype.connected_app.connected_app import has_token

	doctype = itds.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=itds.local.site, key="job_name")[itds.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	itds.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		itds.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = itds.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from itds.core.doctype.user.user import ask_pass_update
	...
	user_names = itds.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = itds.qb.DocType("User Email")
		itds.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([itds.bold(user.get("name")) for user in user_names])
		itds.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = itds.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = itds.get_doc("Email Account", email_account)
...
def get_automatic_email_link():
	...
	return itds.client_cache.get_value("automatic_linking_email", generator=check_db)
