...
def sendmail_to_system_managers(subject, content):
	itds.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_contact_list(txt, page_length=20, extra_filters: str | None = None) -> list[dict]:
	...
	if extra_filters:
		extra_filters = itds.parse_json(extra_filters)
	...
	# https://github.com/mrinimitable/itds/blob/6c6a89bcdd9454060a1333e23b855d0505c9ebc2/itds/public/js/itds/form/controls/autocomplete.js#L29-L35
	return [
		itds._dict(
			value=d.email_id,
			label=d.email_id,
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = itds.utils.user.UserPermissions(itds.session.user)
	...
	com_doctypes = []
	if len(txt) < 2:
		for name in itds.get_hooks("communication_doctypes"):
			try:
		...
		com_doctypes = [
			d[0] for d in itds.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})
		]
...
def sendmail(
	...
	if as_markdown:
		from itds.utils import md_to_html
