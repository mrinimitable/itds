...
from itds.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(itds.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			itds.msgprint(
				...
				raise_exception=itds.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				itds.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=itds.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					itds.msgprint(res[1], raise_exception=itds.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if itds.request and hasattr(itds.request, "after_response"):
			itds.request.after_response.add(self.quit)
		elif itds.job:
			itds.job.after_job.add(self.quit)
		elif not itds.in_test:
			# Console?
			import atexit
