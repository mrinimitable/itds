...
def get_apps():
	...
	for app in apps:
		if (
			...
			and "System Manager" not in itds.get_roles()
		):
			continue

		if app == "itds":
			continue
		app_details = itds.get_hooks("add_to_apps_screen", app_name=app)
		if not len(app_details):
			continue
		for app_detail in app_details:
			try:
				has_permission_path = app_detail.get("has_permission")
				if has_permission_path and not itds.get_attr(has_permission_path)():
					continue
				app_list.append(
					{
						"name": app,
				...
				itds.log_error(f"Failed to call has_permission hook ({has_permission_path}) for {app}")
	return app_list
...
def get_default_path():
	apps = get_apps()
	_apps = [app for app in apps if app.get("name") != "itds"]

	if len(_apps) == 0:
		return None

	system_default_app = itds.get_system_settings("default_app")
	user_default_app = itds.get_cached_value("User", itds.session.user, "default_app")
...
def set_app_as_default(app_name):
	if itds.db.get_value("User", itds.session.user, "default_app") == app_name:
		itds.db.set_value("User", itds.session.user, "default_app", "")
	else:
		itds.db.set_value("User", itds.session.user, "default_app", app_name)
