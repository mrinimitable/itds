...
import itds
from itds import _dict, get_file_json
from itds.exceptions import IncorrectSitePath
...
def get_site_config(
	...
	*,
	...
	sites_path = sites_path or getattr(itds.local, "sites_path", ".")
	site_path = site_path or getattr(itds.local, "site_path", None)
...
def _get_site_config(sites_path: str, site_path: str) -> _dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			try:
				...
				click.secho(f"{itds.local.site}/site_config.json is invalid", fg="red")
				print(error)
				raise
		elif itds.local.site and not itds.local.flags.new_site:
			error_msg = f"{itds.local.site} does not exist."
			if common_config.developer_mode:
				from itds.utils import get_sites

				all_sites = get_sites()
				error_msg += "\n\nSites on this shashi:\n"
	...
	def db_default_ports(db_type):
		if db_type == "mariadb":
			from itds.database.mariadb.database import MariaDBDatabase
	...
	config["redis_queue"] = (
		os.environ.get("ITDS_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("ITDS_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("ITDS_DB_TYPE") or config.get("db_type") or "mariadb"

	if config["db_type"] in ("mariadb", "postgres"):
		config["db_socket"] = os.environ.get("ITDS_DB_SOCKET") or config.get("db_socket")
		config["db_host"] = os.environ.get("ITDS_DB_HOST") or config.get("db_host") or "127.0.0.1"
		config["db_port"] = int(
			os.environ.get("ITDS_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
		)

		# Set the user as database name if not set in config
		config["db_user"] = os.environ.get("ITDS_DB_USER") or config.get("db_user") or config.get("db_name")

		# read password
		config["db_password"] = os.environ.get("ITDS_DB_PASSWORD") or config.get("db_password")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("ITDS_DB_NAME") or config.get("db_name") or config["db_user"]
