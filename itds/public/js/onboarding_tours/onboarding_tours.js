itds.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new itds.Driver({
			className: "itds-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				itds.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!itds.boot.user.onboarding_status[this.tour.name] &&
								(itds.boot.user.onboarding_status[this.tour.name] = {});
							itds.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								itds.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							itds.call({
								method: "itds.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(itds.boot.user.onboarding_status),
				...
				if ($input) itds.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await itds.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: itds.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
itds.ui.init_onboarding_tour = () => {
	...
	typeof itds.boot.onboarding_tours == "undefined" && itds.boot.onboarding_tours == [];
	typeof itds.boot.user.onboarding_status == "undefined" &&
		itds.boot.user.onboarding_status == {};
	let route = itds.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		itds.boot.onboarding_tours &&
			itds.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (itds.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (itds.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(itds.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == itds.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == itds.ui.next_form_tour);
	if (current_tour) {
		...
		itds.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (itds.ui.currentTourInstance?.driver) {
		itds.ui.currentTourInstance.driver_steps = [];
		itds.ui.currentTourInstance.driver.reset(true);
		itds.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (itds.ui.currentTourInstance = new itds.ui.OnboardingTour());
