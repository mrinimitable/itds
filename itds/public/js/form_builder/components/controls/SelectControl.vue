...
function get_options() {
	...
	if (props.df.fieldname == "fieldtype") {
		if (!in_list(itds.model.layout_fields, props.modelValue)) {
			options =
				options &&
				options.filter((opt) => !in_list(itds.model.layout_fields, opt.value));
		} else {
...
<template>
	<div v-if="slots.label" class="control itds-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div class="select-input">
			...
			<div class="select-icon" v-html="itds.utils.icon('select', 'sm')"></div>
		</div>
