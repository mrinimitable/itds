...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = itds.boot.time_zone
		? itds.boot.time_zone.user
		: itds.sys_defaults.time_zone;
...
<template>
	<div class="control itds-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="itds.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="itds.utils.icon('down', 'sm')"
		/>
	</div>
</template>
