...
itds.provide("itds.model.docinfo");
...
itds.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

itds.ui.form.Form = class ItdsForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new itds.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = itds.get_meta(doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = itds.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		...
		this.perm = itds.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = itds.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new itds.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});

		this.viewers = new itds.ui.form.FormViewers({
			frm: this,
		...
		if (!this.meta.hide_toolbar && itds.boot.desk_settings.timeline) {
			// this.footer_tab = new itds.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new itds.ui.form.Footer({
				frm: this,
	...
	add_form_keyboard_shortcuts() {
		...
		itds.ui.keys.add_shortcut({
			...
			condition: () => itds.model.can_print(this.doctype, this) && !this.meta.issingle,
		});
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new itds.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new itds.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new itds.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new itds.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = itds.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", itds.model.table_fields],
		});
	...
	setup_notify_on_rename() {
		...
		itds.realtime.on("doc_rename", (data) => {
			...
			if (data.doctype == this.doctype && data.old == this.docname) {
				...
				itds.set_route("Form", this.doctype, data.new);
			}
		});
	}
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				itds.msgprint(__("Please save before attaching."));
				throw "attach error";
			}
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => itds.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = itds.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				itds.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = itds.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only();
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				itds.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				itds.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return itds.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
			...
			return itds.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			itds.after_ajax(function () {
				me.trigger_link_fields();
			});

			itds.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (itds.boot.desk_settings.form_sidebar) {
				this.sidebar = new itds.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (itds.route_hooks.after_load) {
			let route_callback = itds.route_hooks.after_load;
			delete itds.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			itds.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
	...
	validate_and_save(save_action, callback, btn, on_error, resolve, reject) {
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				...
				if (itds.route_hooks.after_save) {
					let route_callback = itds.route_hooks.after_save;
					delete itds.route_hooks.after_save;
			...
			itds
				.run_serially([
					...
					() => {
						...
						itds.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
	...
	discard(btn, callback, on_error) {
		...
		return new Promise((resolve) => {
			itds.confirm(__("Discard {0}", [this.docname]), function () {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									itds.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (itds.route_hooks.after_submit) {
												let route_callback =
													itds.route_hooks.after_submit;
												delete itds.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		itds
			.call({
				method: "itds.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => itds.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => itds.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new itds.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="itds-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				itds.call({
					method: "itds.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
						ignore_doctypes_on_cancel_all: me.ignore_doctypes_on_cancel_all || [],
					},
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						itds.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				itds.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	_discard(btn, on_error, skip_confirm) {
		...
		const discard_doc = () => {
			...
			me.script_manager.trigger("before_discard").then(() => {
				...
				//itds.ui.form.discard(me, after_discard, btn);
				itds.call({
					freeze: true,
					method: "itds.desk.form.save.discard",
					args: {
						doctype: me.doc.doctype,
						name: me.doc.name,
					},
	...
	savetrash() {
		...
		itds.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			itds.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		itds
			.xcall("itds.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = itds.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				itds.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = itds.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(itds.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!itds.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = itds.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			itds.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (itds.views.formview[docname] || itds.pages["List/" + docname]) {
				window.location.reload();
			}
		} else {
			if (
				itds.views.formview.DocType &&
				itds.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
			}
		}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && itds.meta.docfield_copy[dt]) {
			// delete docfield copy
			itds.meta.docfield_copy[dt][name] = itds.meta.docfield_copy[dt][old];
			delete itds.meta.docfield_copy[dt][old];
		}
		...
		itds.re_route[itds.router.get_sub_path()] = `${encodeURIComponent(
			itds.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!itds._from_link && itds.set_route("Form", this.doctype, name);
	}
	...
	navigate_records(prev) {
		...
		itds.call("itds.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
				itds.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		itds.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new itds.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = itds.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			itds.model.remove_from_locals(this.doctype, this.docname);
			return itds.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return itds.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return itds.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = itds.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (itds.route_options && itds.route_options.scroll_to) {
			var scroll_to = itds.route_options.scroll_to;
			delete itds.route_options.scroll_to;
			...
			if (selector.length) {
				itds.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				itds.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new itds.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = itds.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = itds.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return itds.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return itds.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = itds.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = itds.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? itds.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		itds.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = itds.model.add_child(
			this.doc,
			itds.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !itds.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						itds.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						itds.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...itds.model.std_fields_list,
							...itds.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = itds.model.add_child(
								me.doc,
								fieldobj.df.options,
				...
				itds.msgprint(__("Field {0} not found.", [f]));
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = itds.model.get_server_module_name(me.doctype) + "." + opts.method;
					...
					if (opts.child) {
						...
						if (opts.child) {
							var std_field_list = ["doctype"]
								.concat(itds.model.std_fields_list)
								.concat(itds.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = itds.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return itds.format(
			this.doc[fieldname],
			itds.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return itds.ui.form.get_open_grid_form();
	}

	get_title() {
		return itds.model.get_doc_title(this.doc);
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		itds.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (itds.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			itds.meta.get_table_fields(this.doctype).every(function (df) {
				if (itds.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		itds.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (itds.form.link_formatters[df.options]) {
					label = itds.form.link_formatters[df.options](value, doc, df);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${itds.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${itds.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!itds.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			itds.model.with_doctype(doctype, function () {
				let new_doc = itds.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				itds.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// itds.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		itds.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		itds.model
			...
			.filter((child) => !itds.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				itds.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		itds.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".itds-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			itds.realtime.doc_subscribe(doctype, docname);
		}
		itds.realtime.off("docinfo_update");
		itds.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = itds.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				itds.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					itds.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					itds.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			itds.model.with_doctype(reference_doctype, () => {
				itds.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", itds.session.user].includes(user))
			.filter(Boolean);
	}
	...
	show_submission_queue_banner() {
		...
		itds
			.call({
				method: "itds.core.doctype.submission_queue.submission_queue.get_latest_submissions",
	...
	show_workflow_read_only_banner() {
		...
		if (this.dashboard) {
			...
			itds.after_ajax(_show_read_only_banner);
		}
	}
};
