itds.ui.form.ControlButton = class ControlButton extends itds.ui.form.ControlData {
	can_write() {
	...
	make_input() {
		...
		this.$input = $(
			`<button
				class="btn ${itds.utils.escape_html(btn_size)} ${itds.utils.escape_html(btn_type)}"
				title="${itds.utils.escape_html(this.df.label)}"
			>`
		)
	...
	set_label(label) {
		...
		label =
			(this.df.icon ? itds.utils.icon(this.df.icon) : "") +
