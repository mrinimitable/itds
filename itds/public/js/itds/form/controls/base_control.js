itds.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='itds-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = itds.perm.get_field_display_status(
			this.df,
			itds.model.get_doc(this.doctype, this.docname),
		...
		let value = itds.model.get_value(this.doctype, this.docname, this.df.fieldname);
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(itds.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!itds.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!itds.views || !itds.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new itds.views.TranslationManager({
						df: this.df,
