itds.ui.form.ControlPassword = class ControlPassword extends itds.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(itds.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(itds.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		itds.call({
			type: "POST",
			method: "itds.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
