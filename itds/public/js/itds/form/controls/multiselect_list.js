itds.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	itds.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${itds.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
