...
itds.ui.form.ControlTable = class ControlTable extends itds.ui.form.Control {
	make() {
		super.make();
		...
		this.$wrapper.on("paste", ":text", (e) => {
			...
			let data = itds.utils.csv_to_array(pasted_data, "\t");
			...
			if (this.get_field(data[0][0])) {
				data[0].forEach((column) => {
					...
					const df = itds.meta.get_docfield(doctype, this.get_field(column));
				...
				visible_columns.forEach((column) => {
					...
					if (
						...
						const df = itds.meta.get_docfield(doctype, column.fieldname);
						fieldtypes.push(df ? df.fieldtype : "");
