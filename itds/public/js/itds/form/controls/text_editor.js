...
import ImageResize from "itds-quill-image-resize";
...
itds.ui.form.ControlTextEditor = class ControlTextEditor extends itds.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			itds.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = itds.ui.keys && itds.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: itds.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "itds.desk.search.get_names_for_mentions";
				let values = await itds.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? itds.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return itds.dom.remove_script_and_style(value);
	}
