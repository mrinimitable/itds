itds.ui.form.ControlDynamicLink = class ControlDynamicLink extends itds.ui.form.ControlLink {
	get_options() {
		let options = "";
		if (this.df.get_options) {
			...
			options = itds.model.get_value(this.df.parent, this.docname, this.df.options);
		}

		if (itds.model.is_single(options)) {
			itds.throw(__("{0} is not a valid DocType for Dynamic Link", [options.bold()]));
		}
