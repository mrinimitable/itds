...
itds.ui.form.ControlGeolocation = class ControlGeolocation extends itds.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (!this.map_id) {
			this.map_id = itds.dom.get_unique_id();
			this.map_area = $(
		...
		if (this.frm) {
			...
			$(document).on("itds.ui.Dialog:shown", () => {
				this.make_map();
				if (value) {
					this.bind_leaflet_data(value);
				}
	...
	customize_draw_controls() {
		...
		L.Icon.Default.imagePath = itds.utils.map_defaults.image_path;
	}

	bind_leaflet_map() {
		this.map = L.map(this.map_id);
		this.map.setView(itds.utils.map_defaults.center, itds.utils.map_defaults.zoom);

		L.tileLayer(itds.utils.map_defaults.tiles, itds.utils.map_defaults.options).addTo(
			this.map
		);
		...
		return new L.Control.Draw({
			...
			draw: {
				...
				polygon: {
					...
					drawError: {
						color: itds.ui.color.get("orange"), // Color the shape will turn when intersects
