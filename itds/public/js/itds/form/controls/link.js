...
itds.ui.form.recent_link_validations = {};

itds.ui.form.ControlLink = class ControlLink extends itds.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open" style="display: inline-block;" title="${__("Open Link")}">
					${itds.utils.icon("arrow-right", "xs")}
				</a>
			</span>
	...
	show_link_and_clear_buttons() {
		if (this.$input.val() && this.get_options()) {
			...
			this.$link_open.attr("href", itds.utils.get_form_link(doctype, name));
		}
	}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return itds.get_route && itds.get_route()[0] === "List"
				? itds.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (itds.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (itds.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			itds.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			itds.route_options = df.get_route_options_for_new_doc(this);
		} else {
			itds.route_options = {};
		}

		// partially entered name field
		itds.route_options.name_field = this.get_label_value();

		// reference to calling link
		itds._from_link = itds.utils.deep_clone(this);
		itds._from_link_scrollY = $(document).scrollTop();

		itds.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $(`<div role="option">`)
					...
					.html(`<p title="${itds.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			itds.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(itds.boot.sysdefaults?.link_field_results_limit) || 10,
				};
				...
				itds.call({
					type: "POST",
					method: "itds.desk.search.search_link",
					no_spinner: true,
					args: args,
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								itds.ui.form.ControlLink.link_options &&
								itds.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							itds.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				itds.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (itds.model.can_select(doctype) && !itds.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = itds.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : itds.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = itds.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = itds.utils.eval(value, context);
			}
