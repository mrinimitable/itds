...
itds.provide("itds.form.formatters");

itds.form.link_formatters = {};

itds.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			itds.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				itds.meta.docfield_map[df.parent] &&
				itds.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return itds.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(itds.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(itds.boot.sysdefaults && itds.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return itds.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return itds.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		...
		if (cstr(docfield.options).trim() === "File Size") {
			return itds.form.formatters.FileSize(value);
		}
		return itds.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		...
		const precision =
			...
			cint(itds.boot.sysdefaults && itds.boot.sysdefaults.float_precision) ||
			2;
		return itds.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		...
		var currency = itds.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || itds.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					itds.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return itds.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = itds.utils.get_link_title(doctype, value);
		...
		if (docfield && docfield.link_onclick) {
			...
			if (itds.model.can_read(doctype)) {
				const a = document.createElement("a");
				a.href = `/app/${encodeURIComponent(
					itds.router.slug(doctype)
	...
	Date: function (value) {
		if (!itds.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = itds.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				itds.datetime.str_to_user(value[0]),
				itds.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(itds.datetime.convert_to_user_tz(value)).format(
				itds.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(itds.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = itds.utils.replace_newlines(value);
			}
		}

		return itds.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = itds.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = itds.utils.get_duration_options(docfield);
			value = itds.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return itds.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = itds.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = itds.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return `<span class="text-nowrap">
				${itds.format(value, link_field, options, row)}
			</span>`;
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${itds.utils.icon(value, "md")}</div>
...
itds.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return itds.form.formatters[fieldtype.replace(/ /g, "")] || itds.form.formatters.Data;
};

itds.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || itds.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = itds.dom.remove_script_and_style(formatted);

	return formatted;
};

itds.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = itds.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return itds.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
itds.form.link_formatters["User"] = function (value, doc, docfield) {
