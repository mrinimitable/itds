...
itds.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${itds.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !itds.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(itds.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(itds.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		itds.provide("itds.route_options");
		itds.route_options[fieldname] = this.frm.doc.name;
		itds.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				itds.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			itds.route_options = this.get_document_filter(doctype);
			if (show_open && itds.ui.notifications) {
				itds.ui.notifications.show_open_count_list(doctype);
			}
		}

		itds.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "itds.desk.notifications.get_open_count";
		itds.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new itds.Chart("#heatmap-" + itds.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new itds.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
