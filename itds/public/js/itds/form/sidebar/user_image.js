itds.ui.form.set_user_image = function (frm) {
	...
	if (image) {
		image = window.cordova && image.indexOf("http") === -1 ? itds.base_url + image : image;
		...
		image_section
			...
			.removeClass("hide")
			.find(".standard-image")
			.html(itds.get_abbr(title));

		title_image.css("background-image", "").html(itds.get_abbr(title));
...
itds.ui.form.setup_user_image_event = function (frm) {
	// re-draw image on change of user image
	if (frm.meta.image_field) {
		itds.ui.form.on(frm.doctype, frm.meta.image_field, function (frm) {
			itds.ui.form.set_user_image(frm);
		});
	}
