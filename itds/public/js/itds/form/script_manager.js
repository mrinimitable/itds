...
itds.provide("itds.ui.form.handlers");
...
itds.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!itds.ui.form.handlers[doctype]) {
		itds.ui.form.handlers[doctype] = {};
	}
	if (!itds.ui.form.handlers[doctype][fieldname]) {
		itds.ui.form.handlers[doctype][fieldname] = [];
	}
	return itds.ui.form.handlers[doctype][fieldname];
};

itds.ui.form.on = itds.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = itds.ui.form.get_event_handler_list(doctype, fieldname);
...
itds.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = itds.ui.form.get_event_handler_list(doctype, fieldname);
...
itds.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

itds.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = itds.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			itds.ui.form.handlers[this.frm.doctype] &&
			itds.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (itds.ui.form.handlers[doctype] && itds.ui.form.handlers[doctype][event_name]) {
			$.each(itds.ui.form.handlers[doctype][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
		if (itds.ui.form.handlers["*"] && itds.ui.form.handlers["*"][event_name]) {
			$.each(itds.ui.form.handlers["*"][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (itds.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					itds.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && itds.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		itds.show_alert({ message: __("Error in Client Script."), indicator: "error" });
