...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			itds.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = itds.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
		...
		itds.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					itds.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				itds.render(this.grid.template, {
					doc: this.doc ? itds.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			itds.utils.only_allow_num_decimal(this.row_index.find("input"));
		}
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col pointer">
					<a>${itds.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new itds.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = itds.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = itds.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = itds.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-1' style='padding-top: 4px;'>
								<a style='cursor: grabbing;'>${itds.utils.icon("drag", "xs")}</a>
							</div>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					itds.throw(__("Column width cannot be zero."));
				}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					itds.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		itds.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			itds.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		itds.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			itds.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? itds.format(this.doc[df.fieldname], df, null, this.doc)
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = itds.utils.eval(expression.substr(5), { doc, parent });
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			itds.utils.only_allow_num_decimal($search_input);
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $(
			...
			.on("click", function (event) {
				...
				if (itds.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (itds.ui.form.editable_row && itds.ui.form.editable_row !== this) {
			...
			itds.ui.form.editable_row = this;
			return false;
		} else {
			...
			itds.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var me = this,
			...
			df = column.df;
		var field = itds.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { ESCAPE, TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = itds.ui.keyCode;
	...
	show_form() {
		...
		itds.dom.freeze("", "dark grid-form");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!itds.dom.is_element_in_viewport(this.wrapper) &&
			!itds.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			itds.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!itds.dom.is_element_in_modal(this.row)) {
			itds.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = itds.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = itds.format(this.doc[fieldname], df, null, this.frm.doc);
		}
