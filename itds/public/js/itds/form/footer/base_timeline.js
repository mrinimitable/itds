...
class BaseTimeline {
	...
	add_action_button(label, action, icon = null, btn_class = null) {
		let icon_element = icon ? itds.utils.icon(icon, "xs") : null;
	...
	get_timeline_item(item) {
		...
		if (item.icon) {
			timeline_item.append(`
				<div class="timeline-badge" title='${item.title || itds.utils.to_title_case(item.icon)}'>
					${itds.utils.icon(item.icon, item.icon_size || "md", item.icon_class || "")}
				</div>
			`);
		...
		timeline_item.append(
			`<div class="timeline-content ${item.is_card ? "itds-card" : ""}">`
		);
			...
			timeline_content.append(
				`<span> · ${
					...
					cint(itds.boot.sysdefaults.show_absolute_datetime_in_timeline)
						? itds.datetime.str_to_user(item.creation)
...
itds.ui.BaseTimeline = BaseTimeline;
