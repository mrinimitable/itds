...
itds.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || itds.router.doctype_layout || __(this.doctype);
		this.meta = itds.get_meta(this.doctype);
		this.settings = itds.listview_settings[this.doctype] || {};
		this.user_settings = itds.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = itds.is_large_screen() ? 100 : 20;
		this.selected_page_count = this.page_length;
		this.data = [];
		this.method = "itds.desk.reportview.get";

		this.can_create = itds.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return itds
			.call("itds.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(itds.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			itds.model.std_fields_list.includes(fieldname) ||
			itds.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = itds.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!itds.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return itds.model.with_doctype(this.doctype);
	}
	...
	setup_view_menu() {
		if (itds.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new itds.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		itds.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_side_bar() {
		...
		this.list_sidebar = new itds.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$itds_list = $('<div class="itds-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$itds_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$itds_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$itds_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$itds_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => itds.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return itds.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	prepare_data(r) {
		...
		Object.assign(itds.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? itds.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = itds.utils.debounce(
			this.refresh_list_view.bind(this),
			300
		);
		this.setup();
		if (itds.is_mobile()) this.setup_mobile(list_view);
	}
	...
	clear(refresh = true) {
		...
		return itds.run_serially(promises).then(() => {
			this.trigger_refresh = true;
				...
				.filter(
					(df) =>
						(df.fieldname === title_field ||
							(df.in_standard_filter && itds.model.is_value_type(df.fieldtype))) &&
						itds.perm.has_perm(this.list_view.doctype, df.permlevel)
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						itds.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
		$(`<div class="filter-selector">
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${itds.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon button-icon">
						${itds.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new itds.ui.FilterGroup({
...
itds.views.is_valid = (view_mode) => itds.views.view_modes.includes(view_mode);
