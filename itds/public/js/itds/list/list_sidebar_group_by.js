...
itds.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = itds.meta.get_label(this.doctype, fieldname);
				let docfield = itds.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<div class="group-by-field list-link">
						...
						href="#" onclick="return false;">
							...
							<span>${itds.utils.icon("select", "xs")}</span>
						</a>
	...
	setup_search($dropdown) {
		itds.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return itds.call("itds.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === itds.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", itds.session.user].includes(f.name)
			);
			// Set itds.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === itds.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${itds.utils.icon("tick", "xs")} </span>`
			: "";
