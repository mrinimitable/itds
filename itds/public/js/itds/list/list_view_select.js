...
itds.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = itds.utils.to_title_case(route[2] || "");
		if (route.length > 2 && itds.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			itds.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: itds.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && itds.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(itds.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => itds.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					itds.treeview_settings[this.doctype] ||
					itds.get_meta(this.doctype).is_tree,
		...
		itds.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => itds.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return itds.utils.to_title_case(itds.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(itds.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			itds.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						itds.set_route("list", doctype, "kanban", board.name);
					} else {
						itds.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			itds.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			itds.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					itds.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return itds.db
			...
			.then((result) => {
				...
				if (itds.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = itds.boot.email_accounts;
	...
	slug() {
		return itds.router.slug(itds.router.doctype_layout || this.doctype);
	}
};
