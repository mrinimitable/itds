...
itds.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + itds.dom.id_count;
		itds.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + itds.dom.id_count;
		$ele.attr("id", id);
		itds.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = itds.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		itds.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!itds.dom.freeze_count) return; // anything open?
		itds.dom.freeze_count--;
		if (!itds.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				itds.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
itds.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
itds.run_serially = function (tasks) {
...
itds.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
itds.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

itds.scrub = itds.slug = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

itds.unscrub = function (txt) {
	return itds.model.unscrub(txt);
};
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${itds.utils.icon("close", "sm", "es-icon")}
			</span>
		`);
...
itds.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${itds.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${itds.utils.icon("close", "sm")}
						</button>
					</div>
				</div>
...
itds.is_online = function () {
	if (itds.boot.developer_mode == 1) {
...
itds.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + itds.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = itds.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = itds.assets.bundled_asset("desk.bundle.css");
