...
*/
itds.model.DocTypeController = class DocTypeController extends itds.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		itds.meta.docfield_map[
	...
	show_db_utilization() {
		...
		itds
			.xcall("itds.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = itds.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			itds.model.with_doctype(link_doctype, () => {
				let fields = itds.meta
