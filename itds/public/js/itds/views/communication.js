...
itds.last_edited_communication = {};
const separator_element = "<div>---</div>";

itds.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	make() {
		...
		this.dialog = new itds.ui.Dialog({
	...
	get_fields() {
		...
		const fields = [
			...
			},
			{
				...
				fieldname: "content",
				onchange: itds.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: itds.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = itds.boot.email_accounts.filter((account) => {
			return (
		...
		if (email_accounts.length) {
			...
			} else if (this.user_email_accounts.includes(itds.session.user_email)) {
				this["sender"] = itds.session.user_email;
			}
		}
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = itds.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || itds.boot.lang;
	...
	toggle_more_options(show_options) {
		...
		const label = itds.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				itds.call({
					method: "itds.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			itds.call({
				method: "itds.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		itds.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!itds.last_edited_communication[this.doctype]) {
			itds.last_edited_communication[this.doctype] = {};
		}

		if (clear || !itds.last_edited_communication[this.doctype][this.key]) {
			itds.last_edited_communication[this.doctype][this.key] = {};
		}

		return itds.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? itds.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = itds.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		attach
			...
			.on("click", () => new itds.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = itds.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			itds.db.set_value("User", itds.session.user, "send_me_a_copy", val);
			itds.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			itds.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !itds.model.can_email(this.doc.doctype, this.frm)) {
			itds.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return itds.call({
			method: "itds.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? itds.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					itds.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						itds.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								itds.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				itds.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = itds.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await itds.db.get_list("Email Account", {
				filters: filters,
		...
		if (!itds.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = itds.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = itds.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
	...
	sanitize_emails(control) {
		...
		let sanitized = emails
			.split(",")
			.map((email) => itds.utils.xss_sanitise(email.trim()))
			.join(",");
		if (sanitized != emails) {
