...
itds.standard_pages["Workspaces"] = function () {
	var wrapper = itds.container.add_page("Workspaces");
	...
	itds.workspace = new itds.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		itds.workspace.show();
	});
};

itds.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = itds.workspace_block.blocks;
		...
		this.sidebar = itds.app.sidebar;
		this.app_switcher_menu = itds.app.app_switcher_menu;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, itds.boot.sidebar_pages);
		this.has_access = itds.boot.sidebar_pages.has_access;
		this.has_create_access = itds.boot.sidebar_pages.has_create_access;
		this.show();
	...
	`).appendTo(this.body);
		...
		this.body.find(".btn-edit-workspace").on("click", async () => {
			...
			this.editor.isReady.then(() => {
				...
				itds.app.sidebar.toggle_sorting();
			});
		});
	}

	get_pages() {
		return itds.xcall("itds.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!itds.router.current_route[0]) {
			itds.route_flags.replace_route = true;
			itds.set_route(itds.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return itds
			.call({
				method: "itds.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return itds.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (itds.boot.user.default_workspace) {
			default_page = {
				name: itds.boot.user.default_workspace.name,
				public: itds.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = itds.boot.module_app[itds.router.slug(this._page.module)];
				}
				if (!app) app = "itds";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await itds.after_ajax(() => this.get_data(current_page));
			}
			...
			itds.app.sidebar.apps_switcher.set_current_app(app);
			this.wrapper.find(".workspace-title").html(__(this._page.title));
			this.wrapper
				.find(".workspace-icon")
				.html(itds.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			itds.boot.sidebar_pages = this.cached_pages;
			this.reload();
			itds.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				itds.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new itds.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + itds.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : itds.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: itds.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + itds.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			itds.call({
				method: "itds.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						itds.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!itds.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: itds.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				itds.call({
					method: "itds.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	create_page_skeleton() {
		...
		this.body.prepend(itds.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			itds.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			itds.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
