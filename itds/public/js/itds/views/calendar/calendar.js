...
itds.provide("itds.views.calendar");
itds.provide("itds.views.calendars");

itds.views.CalendarView = class CalendarView extends itds.views.ListView {
	static load_last_view() {
		const route = itds.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = itds.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = itds.views.Calendar[this.doctype] || {};
			this.calendar_name = itds.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new itds.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, itds.views.calendar[this.doctype]);
				resolve(options);
			} else {
				itds.model.with_doc("Calendar View", calendar_name, () => {
					const doc = itds.get_doc("Calendar View", calendar_name);
					if (!doc) {
						itds.show_alert(
						...
						itds.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
itds.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(itds.boot.calendars, function (i, doctype) {
			if (itds.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					itds.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = itds.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new itds.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(itds.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(itds.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(itds.utils.icon("today"));
	...
	get_system_datetime(date) {
		return itds.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: itds.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: itds.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return itds.call({
					method: me.get_events_method || "itds.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (itds.model.can_read(doctype)) {
					itds.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = itds.model.get_new_doc(me.doctype);
				...
				itds.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = itds.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = itds.datetime.convert_to_user_tz(d.start);
				d.end = itds.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!itds.datetime.validate(d.start) && d.end) {
				d.start = itds.datetime.add_days(d.end, -1);
			}

			if (d.start && !itds.datetime.validate(d.end)) {
				d.end = itds.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = itds.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = itds.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = itds.ui.color.get(color_name, "extra-light");
			d.textColor = itds.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!itds.ui.color.validate_hex(color) || !color) {
				color = itds.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = itds.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		itds.model.remove_from_locals(me.doctype, event.id);
		return itds.call({
			method: me.update_event_method || "itds.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					itds.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
