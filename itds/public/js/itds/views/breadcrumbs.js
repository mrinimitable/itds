...
itds.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[itds.breadcrumbs.current_page()] = obj;
		this.update();
		itds.app.sidebar.set_active_workspace_item();
	},

	current_page() {
		return itds.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[itds.breadcrumbs.current_page()];
		...
		if (
			...
			itds.workspace_map[breadcrumbs.workspace]?.app &&
			itds.workspace_map[breadcrumbs.workspace]?.app != itds.current_app
		) {
			let app = itds.workspace_map[breadcrumbs.workspace].app;
	...
	get last_route() {
		return itds.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!itds.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${itds.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				itds.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
				breadcrumbs.workspace = last_workspace;
			}
		} else {
			// choose from __workspaces
			const doctype_meta = itds.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				breadcrumbs.module_info = itds.get_module(breadcrumbs.module);

				// set workspace
				if (
					breadcrumbs.module_info &&
					itds.boot.module_wise_workspaces[breadcrumbs.module]
				) {
					breadcrumbs.workspace =
						itds.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = itds.get_meta(doctype);
		if (
			(doctype === "User" && !itds.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = itds.router.slug(itds.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = itds.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = itds.get_route().slice(2).join("/");
		let doc = itds.get_doc(doctype, docname);

		if (doc.__islocal) return; // new doc, no breadcrumb required

		let title = itds.model.get_doc_title(doc);

		if (title == doc.name) return; // title and name are same, don't add breadcrumb

		let form_route = `/app/${itds.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				itds.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = itds.get_route()[1];
		let dashboard_route = `/app/${itds.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!itds.visible_modules) {
			itds.visible_modules = $.map(itds.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete itds.breadcrumbs.all[old_route_str];
		this.update();
	},
