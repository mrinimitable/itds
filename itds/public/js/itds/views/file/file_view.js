...
itds.views.FileView = class FileView extends itds.views.ListView {
	static load_last_view() {
		const route = itds.get_route();
		if (route.length === 2) {
			const view_user_settings = itds.get_user_settings("File", "File");
			itds.set_route(
				"List",
				"File",
				view_user_settings.last_folder || itds.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			itds.views.FileView.grid_view = !itds.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (itds.views.FileView.grid_view) {
			Object.assign(args, {
					...
					itds.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								itds.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							itds.call({
								method: "itds.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new itds.ui.FileUploader({
						...
						on_success: (file) => {
							itds.show_alert(__("Unzipping files..."));
							itds
								.call("itds.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										itds.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				itds.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => itds.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/itds.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => itds.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (itds.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		super.before_render();
		itds.model.user_settings.save("File", "grid_view", itds.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !itds.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !itds.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === itds.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = itds.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? itds.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${itds.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${itds.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					itds.file_manager.paste(folder_name);
					itds.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!itds.file_manager.can_paste ||
			itds.file_manager.old_folder === this.current_folder;
...
itds.views.FileView.grid_view = itds.get_user_settings("File").grid_view || false;
