/**
 * itds.views.ImageView
 */
itds.provide("itds.views");

itds.views.ImageView = class ImageView extends itds.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = itds.utils.escape_html(title);
	...
	get_attached_images() {
		return itds
			.call({
				method: "itds.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new itds.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/itds/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

itds.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = itds.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new itds.PhotoSwipe(options);
		this.pswp.init();
	}
};
