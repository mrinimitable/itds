...
itds.provide("itds.interaction_settings");

itds.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		itds.model.with_doctype("Event", () => {
			let categories = itds.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new itds.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = itds.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = itds.session.user;
		}
		return itds.call({
	...
	add_attachments(doc, attachments) {
		itds.call({
			method: "itds.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
