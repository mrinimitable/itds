...
itds.views.MapView = class MapView extends itds.views.ListView {
	get view_name() {
		return "Map";
	}
	...
	setup_view() {
		...
		this.map = L.map(this.map_id).setView(
			itds.utils.map_defaults.center,
			itds.utils.map_defaults.zoom
		);
	...
	get_popup_content(feature) {
		return itds.utils.get_form_link(this.doctype, feature.properties.name, true);
	}
