...
itds.views.ReportView = class ReportView extends itds.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		itds.realtime.on("list_update", (data) => this.on_update(data));
	}
	...
	setup_sort_selector() {
		this.sort_selector = new itds.ui.SortSelector({
		...
		this.group_by_control = new itds.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse itds.route_options if this is a Custom Report
	...
	async get_link_title_field_value(doctype, value) {
		return (
			itds.utils.get_link_title(doctype, value) ||
			(await itds.utils.fetch_link_title(doctype, value))
		);
	}
	...
	set_dirty_state_for_custom_report() {
		...
		if (!itds.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = itds.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== itds.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && itds.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? itds.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new itds.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return itds.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = itds.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	evaluate_read_only_depends_on(expression, data) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = itds.utils.eval(expression.substr(5), { doc: data });
				if (parent && parent.istable && expression.includes("is_submittable")) {
					out = true;
				}
			} catch (e) {
				itds.throw(__('Invalid "depends_on" expression'));
			}
	...
	is_editable(df, data) {
		if (
			df &&
			itds.model.can_write(this.doctype) &&
			...
			itds.model.is_non_std_field(df.fieldname)
		) {
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				itds.perm.has_perm(this.doctype, df.permlevel, "read") &&
				itds.model.is_value_type(df.fieldtype) &&
				!df.report_hide
			);
		});
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = itds.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => itds.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = itds.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			itds.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = itds.meta.get_label(doctype, field[0]);
			itds.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			itds.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !itds.model.no_value_type.includes(df.fieldtype);

		let doctype_fields = itds.meta
			.get_docfields(this.doctype)
			.filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = itds.model.std_fields.filter(
		...
		if (this.data) {
			has_status_values = itds.get_indicator(this.data[0], this.doctype);
		}

		if (!itds.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = itds.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = itds.meta
				.get_docfields(cdt)
				.filter(standard_fields_filter);
	...
	get_dialog_fields() {
		...
		const table_fields = itds.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!itds.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = itds.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = itds.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !itds.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			itds.model.is_non_std_field(fieldname) &&
			!docfield.read_only &&
			!docfield.is_virtual;

		const align = (() => {
			const is_numeric = itds.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				if (Array.isArray(row)) {
					doc = row.reduce((acc, curr) => {
						...
						if (
							...
							itds.boot.link_title_doctypes.includes(
				...
				return itds.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = itds.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (itds.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !itds.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = itds.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return itds.call({
				method: "itds.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
	...
	delete_report() {
		return itds.call({
			method: "itds.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			itds.model.with_doc("Report", this.report_name, () => {
				resolve(itds.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = itds.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(itds.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								itds.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								itds.format(value[0], docfield),
								itds.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								itds.utils.comma_or(
									value.map((v) => itds.format(v, docfield))
								),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && itds.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					itds.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					itds.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (itds.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = itds.report_utils.get_export_dialog(
						__(this.doctype),
						extra_fields,
						(data) => {
							args.cmd = "itds.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					itds.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(itds.model[method]("Report") || this.report_doc.owner === itds.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					itds.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && itds.user.has_role("System Manager")) {
			items.push({
				...
				action: () => {
					...
					itds.set_route("List", "User Permission", args);
				},
			});
		}
	...
	parse_filters_from_route_options() {
		if (itds.route_options?._group_by) {
			try {
				let config = JSON.parse(itds.route_options._group_by);
				this.group_by_control.apply_settings({
					group_by: config[0],
					aggregate_on: config[1],
					aggregate_function: config[2],
				});
				delete itds.route_options["_group_by"];
			} catch (e) {
