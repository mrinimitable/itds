itds.provide("itds.report_utils");
...
itds.report_utils = {
	make_chart_options: function (
		...
		let datasets = y_fields.map((y_field) => ({
			name: itds.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = itds.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (itds.query_reports[report_name]) {
			let filters = itds.query_reports[report_name].filters;
		...
		return itds
			...
			.then((r) => {
				itds.dom.eval(r.script || "");
				return itds.after_ajax(() => {
					if (
						itds.query_reports[report_name] &&
						!itds.query_reports[report_name].filters &&
						r.filters
					) {
						return (itds.query_reports[report_name].filters = r.filters);
					}
					return (
						itds.query_reports[report_name] &&
						itds.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = itds.get_route()[0] === "query-report";
			const report = is_query_report ? itds.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				itds.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter, decimal_sep) {
		...
		if (delimiter.length > 1) {
			itds.throw(__("Delimiter must be a single character"));
		}

		if (decimal_sep.length > 1) {
			itds.throw(__("Decimal Separator must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			itds.throw(__("Quoting must be between 0 and 3"));
		}

		if (decimal_sep !== "." && quoting === QUOTING.NonNumeric) {
			itds.throw(__("Decimal Separator must be '.' when Quoting is set to Non-numeric"));
		}
