...
export default class QuickListWidget extends Widget {
	...
	set_actions() {
		...
		if (itds.model.can_create(this.document_type)) {
	...
	setup_refresh_list_button() {
		this.refresh_list = $(
			`<div class="refresh-list btn btn-xs pull-right" title="${__("Refresh List")}">
				${itds.utils.icon("es-line-reload", "sm")}
			</div>`
		);
	...
	setup_filter(doctype) {
		...
		this.filters = itds.utils.process_filter_expression(this.quick_list_filter);
	...
	setup_quick_list_item(doc) {
		const indicator = itds.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${itds.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		let icon_to_append = `<div class="right-arrow">${itds.utils.icon("right", "xs")}</div>`;
		if (itds.utils.is_rtl(itds.boot.lang)) {
			icon_to_append = `<div class="left-arrow">${itds.utils.icon("left", "xs")}</div>`;
		}
		...
		$quick_list_item.click((e) => {
			...
			itds.set_route(`${itds.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		itds.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = itds.get_meta(this.document_type);
			...
			this.has_status_field = itds.meta.has_field(this.document_type, "status");
			...
			let workflow_fieldname = itds.workflow.get_state_fieldname(this.document_type);
			workflow_fieldname && fields.push(workflow_fieldname);
			fields.push("modified");

			let add_fields = itds.listview_settings?.[this.document_type]?.add_fields;
			if (Array.isArray(add_fields)) {
				for (const fieldname of add_fields) {
					// Only keep fields that exist and are permitted
					if (itds.meta.has_field(this.document_type, fieldname)) {
			...
			let quick_list_filter = itds.utils.process_filter_expression(this.quick_list_filter);

			let args = {
				method: "itds.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = itds.utils.get_filter_from_json(this.quick_list_filter);
		let route = itds.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
