...
// depends on itds.versions to manage versioning

itds.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => itds.assets.bundled_asset(item));

	return new Promise((resolve) => {
		itds.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	...
	init_local_storage() {
		...
		if (itds.boot) localStorage.metadata_version = itds.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			itds.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
