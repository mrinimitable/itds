...
itds.avatar = function (
	...
	if (user) {
		// desk
		user_info = itds.user_info(user);
	} else {
		// website
		let full_name = title || itds.get_cookie("full_name");
		user_info = {
			image: image_url === null ? itds.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: itds.get_abbr(full_name),
			color: itds.get_palette(full_name),
		};
	}
...
itds.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = itds.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = itds.get_palette(title);
...
itds.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			itds.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => itds.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${itds.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
itds.ui.scroll = function (element, animate, additional_offset) {
...
itds.get_palette = function (txt) {
	if (!txt) return itds.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return itds.palette[idx % 8];
};

itds.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
itds.gravatars = {};
itds.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!itds.gravatars[email_id]) {
		// TODO: check if gravatar exists
		itds.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return itds.gravatars[email_id];
};
...
itds.get_cookie = function getCookie(name) {
	return itds.get_cookies()[name];
};

itds.get_cookies = function getCookies() {
...
itds.is_mobile = function () {
	return $(document).width() < 768;
};

itds.is_large_screen = function () {
	return $(document).height() > 1180;
};

itds.utils.xss_sanitise = function (string, options) {
...
itds.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = itds.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(itds.utils.xss_sanitise(url, { strategies: ["js"] }));
};

itds.utils.strip_url = (url) => {
...
itds.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
	...
	itds.prompt(
		...
		function (values) {
			itds.call({
				method: "itds.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
itds.utils.get_page_view_count = function (route) {
	return itds.call("itds.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
