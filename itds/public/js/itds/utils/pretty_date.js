function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = itds.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(itds.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = itds.datetime.str_to_user ? itds.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="itds-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
itds.datetime.comment_when = comment_when;
itds.datetime.prettyDate = prettyDate;

itds.datetime.refresh_when = function () {
	if (jQuery) {
		$(".itds-timestamp").each(function () {
