	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="itds.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && itds.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: itds.utils.icon("folder-open", "md"),
		closed: itds.utils.icon("folder-normal", "md"),
		leaf: itds.utils.icon("primitive-dot", "xs"),
		search: itds.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return itds.utils.get_form_link("File", filename);
};
