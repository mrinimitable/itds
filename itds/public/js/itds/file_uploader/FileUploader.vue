				...
				<div class="mt-3 text-center">
					...
					>
						<svg width="30" height="30">
							<image
								href="/assets/itds/icons/social/google_drive.svg"
								width="30"
								height="30"
							/>
						</svg>
...
if (itds.user_id !== "Guest" && props.allow_google_drive) {
	itds.call({
		// method only available after login
		method: "itds.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
		callback: (resp) => {
			if (!resp.exc) {
...
if (props.restrictions.max_file_size == null) {
	itds.call("itds.core.api.file.get_max_file_size").then((res) => {
		props.restrictions.max_file_size = Number(res.message);
	});
}
if (props.restrictions.max_number_of_files == null && props.doctype) {
	props.restrictions.max_number_of_files = itds.get_meta(props.doctype)?.max_attachments;
}
...
function upload_files(dialog) {
	...
	if (!files.value.length) {
		itds.msgprint(__("Please select a file first."));
	...
	return itds.run_serially(files.value.map((file, i) => () => upload_file(file, i)));
}
function upload_via_file_browser() {
	let selected_file = file_browser.value.selected_node;
	if (!selected_file.value) {
		itds.msgprint(__("Click on a file to select it."));
...
function return_as_dataurl() {
	let promises = files.value.map((file) =>
		itds.dom.file_to_base64(file.file_obj).then((dataurl) => {
...
function capture_image() {
	const capture = new itds.ui.Capture({
		animate: false,
		error: true,
	});
	capture.show();
	capture.submit((data_urls) => {
		data_urls.forEach((data_url) => {
			let filename = `capture_${itds.datetime
				.now_datetime()
				.replaceAll(/[: -]/g, "_")}.png`;
