		...
		<div class="file-browser-list">
			<div class="file-filter">
				<input
					...
					v-model="search_text"
					@input="itds.utils.debounce(search_by_name(), 300)"
				/>
			</div>
			<TreeNode
...
function get_files_in_folder(folder, start) {
	return itds
		.call("itds.core.api.file.get_files_in_folder", {
			folder,
			start,
...
function search_by_name() {
	...
	itds
		.call("itds.core.api.file.get_files_by_search_text", {
...
function make_file_node(file) {
	...
	let label = itds.utils.file_name_ellipsis(filename, 40);
	return {
		label: label,
		filename: filename,
