 ...
 *
 * @returns {itds.ui.Page}
 */

/**
 * @typedef {Object} itds.ui.Page
 */

itds.ui.make_app_page = function (opts) {
	opts.parent.page = new itds.ui.Page(opts);
	return opts.parent.page;
};

itds.ui.pages = {};

itds.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		itds.ui.pages[itds.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		itds.require(this.required_libs, callback);
	}

	add_main_section() {
		$(itds.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (itds.utils.is_xs() || itds.utils.is_sm()) {
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		if (!tooltip_label) {
			...
			tooltip_label = itds.unscrub(icon);
		}
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? itds.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		itds.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${itds.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			itds.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		itds.utils.set_title(tab_title || title);
		if (icon) {
			title = `${itds.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? itds.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		itds.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = itds.ui.form.make_control({
			df: df,
