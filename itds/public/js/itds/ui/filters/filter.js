itds.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = itds.ui.filter_utils;
		this.set_conditions();
	...
	set_conditions_from_config() {
		if (itds.boot.additional_filters_config) {
			this.filters_config = itds.boot.additional_filters_config;
	...
	make() {
		this.filter_edit_area = $(
			itds.render_template("edit_filter", {
	...
	make_select() {
		this.fieldselect = new itds.ui.FieldSelect({
	...
	set_field(doctype, fieldname, fieldtype, condition) {
		...
		if (
			...
			if (this.filters_config[condition].data) {
				...
				itds.xcall(this.filters_config[condition].get_field, args).then((field) => {
	...
	make_field(df, old_fieldtype) {
		...
		let f = itds.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition =
			df.fieldtype === "Link" && itds.boot.nested_set_doctypes.includes(df.options);
...
itds.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return itds.format(value, field.df, { only_value: 1 });
	},
	...
	get_selected_value(field, condition) {
		...
		} else if (itds.boot.additional_filters_config[condition]) {
			val = field.value || val;
		}
		if (val === "%") {
			val = "";
	...
	get_default_condition(df) {
		const meta = itds.get_meta(df.parent);
