...
itds.tags.utils = {
	get_tags: function (txt) {
		...
		if (!itds.tags.tags) {
			itds.tags.utils.fetch_tags();
			return [];
		}

		itds.tags.tags.forEach((tag) => {
			const search_result = itds.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						itds.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		itds.call({
			method: "itds.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					itds.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			itds.call({
				method: "itds.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
