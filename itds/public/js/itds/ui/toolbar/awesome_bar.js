...
itds.provide("itds.tags");

itds.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = itds.router.make_url(
						itds.router.convert_from_standard_route(
							itds.router.get_route_from_arguments(
			...
			itds.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(itds.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(itds.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				itds.route_options = item.route_options;
			}
		...
		itds.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				itds.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = itds.search.utils
			.get_creatables(txt)
			.concat(
				itds.search.utils.get_search_in_list(txt),
				itds.search.utils.get_doctypes(txt),
				itds.search.utils.get_reports(txt),
				itds.search.utils.get_pages(txt),
				itds.search.utils.get_workspaces(txt),
				itds.search.utils.get_dashboards(txt),
				itds.search.utils.get_recent_pages(txt || ""),
				itds.search.utils.get_executables(txt),
				itds.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = itds.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${itds.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	itds.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [itds.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = itds.container.page?.list_view?.doctype;
			if (!doctype) return;
			...
			this.options.push({
				label: __("Find {0} in {1}", [
					itds.utils.xss_sanitise(txt).bold(),
					__(route[1]).bold(),
				]),
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						itds.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: itds.utils.get_random(16),
				onclick: function () {
					itds.msgprint(itds.utils.get_random(16), __("Result"));
				},
			});
		}
	}
