...
itds.search.utils = {
	setup_recent: function () {
		this.recent = JSON.parse(itds.boot.user.recent || "[]") || [];
	},
	...
	get_recent_pages: function (keywords) {
		...
		itds.route_history.forEach(function (route, i) {
			if (route[0] === "Form") {
				...
				values.push([itds.route_titles[route.join("/")] || route[0], route]);
			}
		});
		...
		find(values, keywords, function (match) {
			...
			if (route[0] === "Form") {
				...
				out.label = itds.utils.escape_html(match[0]).bold();
				out.value = match[0];
			} else {
	...
	get_frequent_links() {
		let options = [];
		itds.boot.frequently_visited_links.forEach((link) => {
			const label = itds.utils.get_route_label(link.route);
			options.push({
				route: link.route,
				label: label,
				value: label,
	...
	get_search_in_list: function (keywords) {
		...
		if (keywords.split(" ").includes("in") && keywords.slice(-2) !== "in") {
			...
			itds.boot.user.can_read.forEach(function (item) {
				if (itds.boot.user.can_search.includes(item)) {
	...
	get_creatables: function (keywords) {
		...
		if (firstKeyword.toLowerCase() === __("new")) {
			itds.boot.user.can_create.forEach(function (item) {
	...
	get_doctypes: function (keywords) {
		...
		itds.boot.user.can_read.forEach(function (item) {
			...
			if (score) {
				target = item;
				if (itds.boot.single_types.includes(item)) {
					out.push(option("", ["Form", item, item], 0.05));
				} else if (itds.boot.user.can_search.includes(item)) {
					// include 'making new' option
					if (itds.boot.user.can_create.includes(item)) {
						var match = item;
						out.push({
							type: "New",
							label: __("New {0}", [search_result.marked_string || __(item)]),
							...
							match: item,
							onclick: function () {
								itds.new_doc(match, true);
							},
						});
					}

					out.push(option("List", ["List", item], 0.05));
					if (itds.model.can_get_report(item)) {
	...
	get_reports: function (keywords) {
		...
		var out = [];
		var route;
		Object.keys(itds.boot.user.all_reports).forEach(function (item) {
			const search_result = me.fuzzy_search(keywords, item, true);
			var level = search_result.score;
			if (level > 0) {
				var report = itds.boot.user.all_reports[item];
	...
	get_workspaces: function (keywords) {
		var me = this;
		var out = [];
		itds.boot.allowed_workspaces.forEach(function (item) {
			...
			if (level > 0) {
				var ret = {
					...
					index: level,
					route: [itds.router.slug(item.name)],
				};
	...
	get_dashboards: function (keywords) {
		var me = this;
		var out = [];
		itds.boot.dashboards.forEach(function (item) {
			const search_result = me.fuzzy_search(keywords, item.name, true);
			var level = search_result.score;
			if (level > 0) {
				var ret = {
	...
	get_global_results: function (keywords, start, limit, doctype = "") {
		...
		return new Promise(function (resolve, reject) {
			itds.call({
				method: "itds.utils.global_search.search",
				args: {
					text: keywords,
					start: start,
					limit: limit,
	...
	/**
	 * @deprecated Use itds.search.utils.fuzzy_search(subseq, str, true).marked_string instead.
	 */
	...
	get_marketplace_apps: function (keywords) {
		var me = this;
		var out = [];
		itds.boot.marketplace_apps.forEach(function (item) {
			...
			if (search_result.score > 0) {
				var ret = {
					...
					route: [
						`https://itdscloud.com/${item.route}?utm_source=awesomebar`,
						item.name,
					],
				};
