itds.ui.AppsSwitcher = class AppsSwitcher {
	constructor(sidebar) {
	...
	make() {
		this.wrapper = $(
			itds.render_template("apps_switcher", {
				app_logo_url: itds.boot.app_data[0].app_logo_url,
				app_title: __(itds.boot.app_data[0].app_title),
			})
	...
	create_app_data_map() {
		itds.boot.app_data_map = {};
		for (var app of itds.boot.app_data) {
			itds.boot.app_data_map[app.app_name] = app;
	...
	add_private_app() {
		...
		itds.boot.app_data_map["private"] = app;
	...
	setup_select_app() {
		this.app_switcher_menu.find(".app-item").on("click", (e) => {
			...
			if (item.attr("data-app-name") == "settings") {
				itds.quick_edit("Workspace Settings");
				return;
			}
			if (route.startsWith("/app/private")) {
				this.set_current_app("private");
				let ws = Object.values(itds.workspace_map).find((ws) => ws.public === 0);
				route += "/" + itds.router.slug(ws.title);
		...
		this.add_app_item(
			{
				...
				app_route: "/",
				app_logo_url: "/assets/itds/images/web.svg",
			},
	...
	add_settings_select() {
		...
		this.add_app_item({
			...
			app_logo_url: "/assets/itds/images/settings-gear.svg",
		});
	...
	set_current_app(app) {
		...
		let app_data = itds.boot.app_data_map[app] || itds.boot.app_data_map["itds"];
		...
		itds.itds_toolbar.set_app_logo(app_data.app_logo_url);

		if (itds.current_app === app) return;
		itds.current_app = app;

		// re-render the sidebar
		itds.app.sidebar.make_sidebar();
	}
