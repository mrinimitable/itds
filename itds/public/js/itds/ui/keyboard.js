...
itds.provide("itds.ui.keys.handlers");

itds.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = itds.ui.keys.get_key(e);
		if (itds.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = itds.ui.keys.handlers[key].length; i < l; i++) {
				var handler = itds.ui.keys.handlers[key][i];
...
itds.ui.keys.standard_shortcuts = standard_shortcuts;
itds.ui.keys.add_shortcut = ({
	...
	itds.ui.keys.off(shortcut, page);
	// attach new handler
	itds.ui.keys.on(shortcut, handler);
...
itds.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (itds.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(itds.utils.to_title_case)
					.join("+");
				if (itds.utils.is_mac()) {
	...
	let dialog = new itds.ui.Dialog({
		...
		on_hide() {
			itds.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	itds.ui.keys.is_dialog_shown = true;
};

itds.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = itds.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
itds.ui.keys.on = function (key, handler) {
	if (!itds.ui.keys.handlers[key]) {
		itds.ui.keys.handlers[key] = [];
	}
	itds.ui.keys.handlers[key].push(handler);
};

itds.ui.keys.off = function (key, page) {
	let handlers = itds.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	itds.ui.keys.handlers[key] = handlers.filter((h) => {
...
itds.ui.keys.add_shortcut({
	...
	action: function (e) {
		...
		itds.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
itds.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

itds.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

itds.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

itds.ui.keys.on("ctrl+down", function (e) {
	const grid_row = itds.ui.form.get_open_grid_form();
...
itds.ui.keys.on("ctrl+up", function (e) {
	const grid_row = itds.ui.form.get_open_grid_form();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	itds.ui.keys.key_map[65 + i] = letter;
});
...
itds.ui.keys.add_shortcut({
	shortcut: "shift+t",
	action: function (e) {
		if (!itds.model.can_write("System Console")) {
			return;
		}
