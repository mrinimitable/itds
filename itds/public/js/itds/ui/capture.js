// itds.ui.Capture
// Author - Achilles Rasquinha <achilles@itds.io>
 ...
 *
 * @example
 * itds._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
itds._.get_data_uri = (element) => {
...
/**
 * @description Itds's Capture object.
 *
 * @example
 * const capture = itds.ui.Capture()
 * capture.show()
 *
 ...
 */
itds.ui.Capture = class {
	constructor(options = {}) {
		this.options = itds.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...itds.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(itds.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (itds.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(itds.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					itds.show_alert(itds.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = itds._.get_data_uri(me.video);
...
itds.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
itds.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
itds.ui.Capture.TEMPLATE = `
<div class="itds-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${itds.ui.Capture.ERR_MESSAGE}</video>
	</div>
