export class DropdownConsole {
	constructor() {
		this.dialog = new itds.ui.Dialog({
			...
			fields: [
				{
					description: `
					${itds.utils.icon("solid-warning", "xs")}
		...
		if (cur_frm && !cur_frm.is_new()) {
			...
			if (!current_code) {
				this.dialog
					.get_field("console")
					.editor?.insert(
						`doc = itds.get_doc("${cur_frm.doc.doctype}", "${cur_frm.doc.name}")\n`
					);
			}
		}
	}
	...
	async execute_code() {
		...
		const start = itds.datetime.now_datetime(true);
		let { output } = await itds.xcall(
			"itds.desk.doctype.system_console.system_console.execute_code",
			{
				doc: {
		...
		const end = itds.datetime.now_datetime(true);
		...
		output_field.set_description(`Executed in ${time_taken} milliseconds.
			<a target="_blank" href="/app/console-log?owner=${itds.session.user}" >View Logs</a>`);
	}

	async load_completions() {
		let me = this;
		let items = await itds.xcall(
			"itds.core.doctype.server_script.server_script.get_autocompletion_items",
			null,
			"GET",
			{ cache: true }
		);
