...
itds.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = itds.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		itds.show_alert(__("Theme Changed"), 3);

		itds.xcall("itds.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
itds.ui.add_system_theme_switch_listener = () => {
	itds.ui.dark_theme_media_query.addEventListener("change", () => {
		itds.ui.set_theme();
	});
};

itds.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

itds.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = itds.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
