...
itds.ui.is_liked = function (doc) {
	return itds.ui.get_liked_by(doc).includes(itds.session.user);
};

itds.ui.get_liked_by = function (doc) {
	return doc._liked_by ? JSON.parse(doc._liked_by) : [];
};

itds.ui.toggle_like = function ($btn, doctype, name, callback) {
	const add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	itds.call({
		method: "itds.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = itds.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(itds.session.user)) {
					liked_by.push(itds.session.user);
				}

				if (add === "No" && liked_by.includes(itds.session.user)) {
					liked_by = liked_by.filter((user) => user !== itds.session.user);
				}
...
itds.ui.click_toggle_like = function () {
	console.warn("`itds.ui.click_toggle_like` is deprecated and has no effect.");
};

itds.ui.setup_like_popover = ($parent, selector) => {
	if (itds.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${itds.avatar(user, "avatar-xs")}
							<span>${itds.user.full_name(user)}</span>
						</li>
					`);
				});
