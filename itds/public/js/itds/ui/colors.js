...
itds.ui.color = {
	get: function (color_name, shade) {
		...
		return itds.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(itds.ui.color_map);
		if (color_names.includes(color_name)) {
			return itds.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(itds.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(itds.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in itds.ui.color_map) {
			const colors = itds.ui.color_map[key];
...
itds.ui.color_map = itds.ui.color.get_color_map();
