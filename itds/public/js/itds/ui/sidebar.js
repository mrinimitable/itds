itds.ui.Sidebar = class Sidebar {
	...
	make_dom() {
		...
		this.wrapper = $(itds.render_template("sidebar")).prependTo("body");
		...
		this.apps_switcher = new itds.ui.AppsSwitcher(this);
	...
	set_all_pages() {
		this.sidebar_pages = itds.boot.sidebar_pages;
	...
	set_default_app() {
		...
		itds.boot.app_data.sort((a, b) => (a.workspaces.length < b.workspaces.length ? 1 : -1));
		itds.current_app = itds.boot.app_data[0].app_name;
		itds.itds_toolbar.set_app_logo(itds.boot.app_data[0].app_logo_url);
	}

	set_active_workspace_item() {
		if (!itds.get_route()) return;
		let current_route = itds.get_route();
		let current_route_str = itds.get_route_str();
		let current_item;
		if (current_route[0] == "Workspaces") {
			current_item = current_route[1];
		} else if (itds.breadcrumbs) {
			if (Object.keys(itds.breadcrumbs.all).length == 0) return;
			if (itds.breadcrumbs.all[current_route_str]) {
				current_item =
					itds.breadcrumbs.all[current_route_str].workspace ||
					itds.breadcrumbs.all[current_route_str].module;
			}
		}
	...
	setup_pages() {
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				itds.workspaces[itds.router.slug(page.name)] = {
					name: page.name,
					public: page.public,
				};
				if (!page.app && page.module) {
					page.app = itds.boot.module_app[itds.slug(page.module)];
				}
				itds.workspace_map[page.name] = page;
				itds.workspace_list.push(page);
			}
			this.make_sidebar();
		}
		this.set_hover();
	...
	make_sidebar() {
		...
		let app_workspaces = itds.boot.app_data_map[itds.current_app || "itds"].workspaces;

		let parent_pages = this.all_pages.filter((p) => !p.parent_page).uniqBy((p) => p.name);
		if (itds.current_app === "private") {
		...
		this.wrapper.find(".selected").length &&
			!itds.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	sidebar_item_container(item) {
		...
		if (item.type === "Link") {
			if (item.link_type === "Report") {
				path = itds.utils.generate_route({
					type: item.link_type,
					name: item.link_to,
				...
				path = itds.utils.generate_route({ type: item.link_type, name: item.link_to });
			}
		} else if (item.type === "URL") {
			path = item.external_link;
		} else {
			if (item.public) {
				path = "/app/" + itds.router.slug(item.name);
			} else {
				path = "/app/private/" + itds.router.slug(item.name.split("-")[0]);
			}
		}
						...
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public || item.icon
									? itds.utils.icon(item.icon || "folder-normal", "md")
	...
	add_toggle_children(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(itds.utils.icon(drop_icon, "sm"))
		...
		for (let container of this.$sidebar.find(".nested-container")) {
			this.sorting_items[this.sorting_items.length] = Sortable.create(container, {
				...
				onEnd: () => {
					...
					itds.xcall(
						"itds.desk.doctype.workspace_settings.workspace_settings.set_sequence",
						{
	...
	close_sidebar() {
		...
		if (itds.is_mobile()) itds.app.sidebar.prevent_scroll();
	}
	open_sidebar() {
		this.sidebar_expanded = true;
		this.expand_sidebar();
	...
	reload() {
		return itds.workspace.get_pages().then((r) => {
