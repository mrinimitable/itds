...
itds.ui.open_dialogs = [];

itds.ui.Dialog = class Dialog extends itds.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				itds.ui.form.get_open_grid_form?.()?.hide_form();

				if (itds.ui.open_dialogs[itds.ui.open_dialogs.length - 1] === me) {
					itds.ui.open_dialogs.pop();
					if (itds.ui.open_dialogs.length) {
						window.cur_dialog =
							itds.ui.open_dialogs[itds.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				itds.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("itds.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(itds.utils.icon(icon));
