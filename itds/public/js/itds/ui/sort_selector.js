itds.ui.SortSelector = class SortSelector {
	// parent:
	// change:
	// args:
	...
	make() {
		...
		this.wrapper = $(itds.render_template("sort_selector", this.args)).appendTo(this.parent);
		this.bind_events();
	}
	bind_events() {
		var me = this;
	...
	set_value(sort_by, sort_order) {
		...
		if (this.sort_order !== sort_order) {
			...
			$icon.html(itds.utils.icon(icon_name, "sm"));
		}
	}
	prepare_args() {
		var me = this;
		...
		if (!this.args.options) {
			...
			meta.fields.forEach(function (df) {
				if (
					...
					itds.perm.has_perm(me.doctype, df.permlevel, "read")
				) {
	...
	get_label(fieldname) {
		if (fieldname === "idx") {
			...
			return this.labels[fieldname] || itds.meta.get_label(this.doctype, fieldname);
		}
	}
	get_sql_string() {
