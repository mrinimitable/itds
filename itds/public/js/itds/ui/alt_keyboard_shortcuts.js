itds.provide("itds.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
itds.ui.keys.shortcut_groups = shortcut_groups;

itds.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new itds.ui.keys.AltShortcutGroup());
	}
...
itds.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (itds.ui.keys.key_map[e.which] || "").toLowerCase();
...
itds.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = itds.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
