...
itds.provide("itds.web_form");
import EventEmitterMixin from "../../itds/event_emitter";

export default class WebForm extends itds.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		itds.web_form = this;
		itds.web_form.events = {};
		Object.assign(itds.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		itds.init_client_script && itds.init_client_script();
		this.setup_listeners();
		itds.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = itds.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${itds.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".itds-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		itds.form_dirty = false;

		itds.call({
			type: "POST",
			method: "itds.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					itds.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						itds.call({
							type: "POST",
							method: "itds.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			itds.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
