...
itds.ready(function () {
	let web_form_doc = itds.web_form_doc;
	let reference_doc = itds.reference_doc;
	...
	function show_login_prompt() {
		if (itds.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new itds.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(itds.form.formatters.FileSize($(this).text()));
		});
	}
