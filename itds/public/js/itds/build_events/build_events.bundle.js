...
itds.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						itds.assets._executed = itds.assets._executed.filter(
		...
		// update assets json
		itds.call("itds.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				itds.boot.assets_json = r.message;

				if (itds.hot_update) {
					itds.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
