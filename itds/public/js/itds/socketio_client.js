...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = itds.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${itds.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		...
		itds.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			itds.flags.doc_subscribe = false;
		}, 1000);
...
itds.realtime = new RealTimeClient();

// backward compatibility
itds.socketio = itds.realtime;
