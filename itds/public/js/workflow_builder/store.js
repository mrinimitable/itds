...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await itds.model.clear_doc("Workflow", workflow_name.value);
		await itds.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = itds.get_doc("Workflow", workflow_name.value);
		await itds.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await itds.model.with_doctype("Workflow");
			workflowfields.value = itds.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await itds.model.with_doctype("Workflow Document State");
			statefields.value = itds.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await itds.model.with_doctype("Workflow Transition");
			transitionfields.value = itds.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await itds.model.with_doctype(doc_type);
			workflow_doc_fields.value = itds.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", itds.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		...
		try {
			...
			await itds.call("itds.client.save", { doc });
			itds.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		itds.breadcrumbs.clear();
		itds.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
