let itdsCloudBaseEndpoint = "https://itdscloud.com";
let isFCUser = false;

$(document).ready(function () {
	if (
		itds.boot.is_fc_site &&
		itds.boot.setup_complete === 1 &&
		!itds.is_mobile() &&
		itds.user.has_role("System Manager")
	) {
		itds.call({
			method: "itds.integrations.itds_providers.itdscloud_billing.current_site_info",
			callback: (r) => {
				if (!r?.message) return;

				const response = r.message;
				const trial_end_date = new Date(response.trial_end_date);
				itdsCloudBaseEndpoint = response.base_url;
...
function openItdsCloudDashboard() {
	window.open(`${itdsCloudBaseEndpoint}/dashboard/sites/${itds.boot.sitename}`, "_blank");
}
