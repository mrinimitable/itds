import itds
from itds.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			itds.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			itds.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({itds.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if itds.db.db_type == "mariadb":
		...
		values = (itds.db.cur_db_name,)

	col_type = itds.db.sql(query, values)
