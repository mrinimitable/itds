...
from itds.patches.v11_0.replicate_old_user_permissions import get_doctypes_to_skip
...
def execute():
	itds.reload_doctype("User Permission")

	# to check if we need to migrate from skip_for_doctype
	has_skip_for_doctype = itds.db.has_column("User Permission", "skip_for_doctype")
	...
	for user_permission in itds.get_all("User Permission", fields=["*"]):
		skip_for_doctype = []
		...
		if skip_for_doctype:
			...
			itds.db.set_value("User Permission", user_permission.name, "apply_to_all_doctypes", 1)

	if new_user_permissions_list:
		itds.qb.into("User Permission").columns(
			"name",
			"user",
			"allow",
			"for_value",
	...
	if user_permissions_to_delete:
		itds.db.delete("User Permission", filters=(Field("name").isin(tuple(user_permissions_to_delete))))
