...
def execute():
	...
	reports = itds.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = itds.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = itds._dict(json.loads(settings))
		...
		itds.db.set_value("Report", report_name, "json", json.dumps(settings))
