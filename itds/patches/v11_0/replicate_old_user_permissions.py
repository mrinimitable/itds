...
import itds
from itds.permissions import get_valid_perms
from itds.utils import cint


def execute():
	itds.reload_doctype("User Permission")
	user_permissions = itds.get_all("User Permission", fields=["allow", "name", "user"])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		if not doctype_to_skip:
			continue
		if not itds.db.has_column("User Permission", "applicable_for") and itds.db.has_column(
...
def get_linked_doctypes(doctype):
	from itds.permissions import get_linked_doctypes

	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in itds.get_meta(doctype).get_table_fields()]
