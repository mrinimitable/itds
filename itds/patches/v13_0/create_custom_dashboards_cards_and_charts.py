...
from itds.utils.dashboard import get_dashboards_with_link


def execute():
	if (
		not itds.db.table_exists("Dashboard Chart")
		or not itds.db.table_exists("Number Card")
		or not itds.db.table_exists("Dashboard")
	):
		return

	itds.reload_doc("desk", "doctype", "dashboard_chart")
	itds.reload_doc("desk", "doctype", "number_card")
	itds.reload_doc("desk", "doctype", "dashboard")
...
def get_modified_docs(doctype):
	return itds.get_all(doctype, filters={"owner": "Administrator", "modified_by": ["!=", "Administrator"]})


def rename_modified_doc(docname, doctype):
	new_name = docname + " Custom"
	try:
		itds.rename_doc(doctype, docname, new_name)
