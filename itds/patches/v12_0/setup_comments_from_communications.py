...
def execute():
	...
	if itds.db.count("Communication", filters=dict(communication_type="Comment")) > 20000:
		itds.db.auto_commit_on_many_writes = True

	for comment in itds.get_all("Communication", fields=["*"], filters=dict(communication_type="Comment")):
	...
	# clean up
	itds.db.delete("Communication", {"communication_type": "Comment"})
