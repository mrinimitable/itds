...
def execute():
	...
	all_tables = itds.db.get_tables()
	final_deletion_map = itds._dict()

	for table in all_tables:
		indexes_to_keep_map = itds._dict()
		indexes_to_delete = []
		index_info = itds.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				itds.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
