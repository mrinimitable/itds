...
def execute():
	...
	for name, content in itds.get_all(
		...
		itds.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in itds.get_all(
		...
		if content and content.startswith("Removed "):
			itds.db.set_value("Comment", name, "content", content[8:], update_modified=False)
