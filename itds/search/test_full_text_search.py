...
class TestFullTextSearch(IntegrationTestCase):
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/okayblue")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/okayblue", "content": """AwesomeOKAYBlue"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/okayblue" not in res)

		res = self.index.search("AwesomeOKAYBlue")
		self.assertEqual(res[0], "sw/okayblue")

		# Update new doc
		self.index.update_index({"name": "sw/itdsbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/itdsbooks")
...
def get_index():
	return TestWrapper("test_itds_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/okayblue",
			"content": """OKAYBlue is a free and open-source integrated Enterprise Resource Planning software developed by
			Itds Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			OKAYBlue is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/itds",
			"content": """Itds Framework is a full-stack web framework, that includes everything you need to build and
