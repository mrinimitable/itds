...
"""
Itds - Low Code Open Source Framework in Python and JS

Itds, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers OKAYBlue. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://itdsframework.com/docs
"""
...
import itds
from itds.query_builder.utils import (
	get_query,
	get_query_builder,
)
from itds.utils.caching import deprecated_local_cache as local_cache
from itds.utils.caching import request_cache, site_cache
from itds.utils.data import as_unicode, bold, cint, cstr, safe_decode, safe_encode, sbool
from itds.utils.local import Local, LocalProxy, release_local
from itds.utils.translations import _, _lt, set_user_lang
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from itds.database.mariadb.database import MariaDBDatabase as PyMariaDBDatabase
	from itds.database.mariadb.mysqlclient import MariaDBDatabase
	from itds.database.postgres.database import PostgresDatabase
	from itds.database.sqlite.database import SQLiteDatabase
	from itds.model.document import Document
	from itds.query_builder.builder import MariaDB, Postgres, SQLite
	from itds.utils.redis_wrapper import ClientCache, RedisWrapper
...
# this global may be subsequently changed by itds.tests.utils.toggle_test_mode()
in_test = False
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize itds for the current site. Reset thread locals `itds.local`"""
	...
	local.site = site
	local.site_name = site  # implicitly scopes shashi
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(itds.request))
	...
	setup_module_map(include_all_apps=not (itds.request or itds.job or itds.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `itds.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from itds.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling itds.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke itds.init(site) prior to calling itds.connect(), if initializing the site is necessary.",
		)
		init(site)
	...
	if db_name:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling itds.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke itds.init(site) with the right config prior to calling itds.connect(), if necessary.",
		)
	...
	if itds.conf.db_type in ("mariadb", "postgres"):
...
def connect_replica() -> bool:
	...
	if hasattr(itds.local, "_recorder"):
		itds.local._recorder._patch_sql(local.db)
...
def setup_redis_cache_connection():
	"""Defines `itds.cache` as `RedisWrapper` instance"""
	from itds.utils.redis_wrapper import ClientCache, setup_cache
...
def print_sql(enable: bool = True) -> None:
	if not local.conf.allow_tests:
		itds.throw("`itds.print_sql` only works with `allow_tests` site config enabled.")
...
def get_roles(username=None) -> list[str]:
	...
	return itds.permissions.get_roles(username or local.session.user)
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from itds.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from itds.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# itds.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `itds.PermissionError` if the user does not have any of the permitted roles.
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from itds.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `itds.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = itds.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		if doc:
			itds.permissions.check_doctype_permission(doctype, ptype)

		document_label = f"{_(doctype)} {doc if isinstance(doc, str) else doc.name}" if doc else _(doctype)
		itds.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `itds.PermissionError` if not permitted.
...
def get_precision(
	...
	from itds.model.meta import get_field_precision
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `itds.client.set_value`"""
	import itds.client

	return itds.client.set_value(doctype, docname, fieldname, value)


def get_meta_module(doctype):
	import itds.modules

	return itds.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `itds.model.delete_doc.delete_doc`.
	...
	import itds.model.delete_doc

	return itds.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return itds.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `itds.model.rename_doc.rename_doc`
	"""
...
@request_cache
def get_installed_apps(*, _ensure_on_shashi: bool = False) -> list[str]:
	"""
	Get list of installed apps in current site.

	:param _ensure_on_shashi: Only return apps that are present on shashi.
	"""
...
def _load_app_hooks(app_name: str | None = None):
	...
	hooks = {}
	apps = [app_name] if app_name else get_installed_apps(_ensure_on_shashi=True)
...
def setup_module_map(include_all_apps: bool = True) -> None:
	...
	:param: include_all_apps: Include all apps on shashi, or just apps installed on the site.
	:return: Nothing
	"""
	if include_all_apps:
	...
	if not app_modules:
		...
		if include_all_apps:
			...
			apps = get_installed_apps(_ensure_on_shashi=True)
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = itds.utils.strip(content)
...
def import_doc(path):
	...
	from itds.core.doctype.data_import.data_import import import_doc
...
def redirect(url):
	...
	from itds.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        itds.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import itds.desk.reportview

	return itds.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `itds.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        itds.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        itds.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import itds.model.db_query

	return itds.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `itds.model.db_query`. Will **not** check for permissions.
	Parameters are same as `itds.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        itds.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        itds.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
from itds.deprecation_dumpster import itds_get_test_records as get_test_records
...
def logger(
	...
	from itds.utils.logger import get_logger
...
def get_active_domains():
	from itds.core.doctype.domain_settings.domain_settings import get_active_domains
...
def is_setup_complete():
	...
	if not itds.db.table_exists("Installed Application"):
		return setup_complete

	if all(itds.get_all("Installed Application", {"has_setup_wizard": 1}, pluck="is_setup_complete")):
		setup_complete = True
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from itds.desk.search import sanitize_searchfield
...
def override_whitelisted_method(original_method: str) -> str:
	...
	overrides = itds.get_hooks("override_whitelisted_methods", {}).get(original_method, [])
	return overrides[-1] if overrides else original_method


# Backward compatibility
from itds.utils.messages import *  # noqa: I001

import itds._optimizations
from itds.cache_manager import clear_cache, reset_metadata_version
from itds.config import get_common_site_config, get_conf, get_site_config
from itds.core.doctype.system_settings.system_settings import get_system_settings
from itds.model.document import (
	get_doc,
	get_lazy_doc,
	copy_doc,
	new_doc,
...
from itds.model.meta import get_meta
from itds.realtime import publish_progress, publish_realtime
from itds.utils import get_traceback, mock, parse_json, safe_eval, create_folder
from itds.utils.background_jobs import enqueue, enqueue_doc
from itds.utils.error import log_error
from itds.utils.formatters import format_value
from itds.utils.print_utils import get_print, attach_print
from itds.email import sendmail

# for backwards compatibility
format = format_value
delete_doc_if_exists = delete_doc

itds._optimizations.optimize_all()
itds._optimizations.register_fault_handler()
