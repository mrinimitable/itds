...
import itds
from itds import _, cstr, get_module_path
from itds.core.doctype.access_log.access_log import make_access_log
from itds.core.doctype.document_share_key.document_share_key import is_expired
from itds.utils import cint, escape_html, strip_html
...
if TYPE_CHECKING:
	...
	from itds.printing.doctype.print_format.print_format import PrintFormat
	from itds.printing.doctype.print_settings.print_settings import PrintSettings
...
def get_context(context) -> PrintContext:
	...
	if not ((itds.form_dict.doctype and itds.form_dict.name) or itds.form_dict.doc):
		return PrintContext(
			print_style="",
			comment="",
			title="Error",
...
<pre>{escape_html(itds.as_json(itds.form_dict, indent=2))}</pre>
""",
		)

	if itds.form_dict.doc:
		doc = itds.form_dict.doc
	else:
		doc = itds.get_lazy_doc(itds.form_dict.doctype, itds.form_dict.name)

	set_link_titles(doc)

	settings = itds.parse_json(itds.form_dict.settings)

	letterhead = itds.form_dict.letterhead or None
	...
	if print_format and print_format.get("print_format_builder_beta"):
		from itds.utils.weasyprint import get_html

		body = get_html(
			doctype=itds.form_dict.doctype, name=itds.form_dict.name, print_format=print_format.name
		)
		...
		body = get_rendered_template(
			...
			meta=meta,
			trigger_print=itds.form_dict.trigger_print,
			no_letterhead=itds.form_dict.no_letterhead,
	...
	return {
		"body": body,
		"print_style": get_print_style(itds.form_dict.style, print_format),
		"comment": itds.session.user,
		"title": itds.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": itds.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": itds.form_dict.doctype,
		"name": itds.form_dict.name,
		"key": itds.form_dict.get("key"),
		"print_format": print_format_name,
		"letterhead": letterhead,
		"no_letterhead": itds.form_dict.no_letterhead,
		"pdf_generator": itds.form_dict.get("pdf_generator", "wkhtmltopdf"),
	}
...
def get_print_format_doc(print_format_name: str, meta: "Meta") -> Optional["PrintFormat"]:
	...
	if not print_format_name:
		print_format_name = itds.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return itds.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	if not itds.flags.ignore_print_permissions:
		validate_print_permission(doc)

	print_settings = itds.get_single("Print Settings").as_dict()
	...
	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			itds.throw(_("Not allowed to print draft documents"), itds.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			itds.throw(_("Not allowed to print cancelled documents"), itds.PermissionError)
	...
	if print_format:
		...
		template = None
		if hook_func := itds.get_hooks("get_print_format_template"):
			template = itds.call(hook_func[-1], jenv=jenv, print_format=print_format)
	...
	letter_head = itds._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = itds.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})
	...
	if letter_head.footer:
		letter_head.footer = itds.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	hook_func = itds.get_hooks("pdf_body_html")
	html = itds.get_attr(hook_func[-1])(jenv=jenv, template=template, print_format=print_format, args=args)
...
def set_title_values_for_link_and_dynamic_link_fields(
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = itds.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta: "Meta", doc: "Document") -> None:
	for field in meta.get_table_fields():
		...
		_meta = itds.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, itds.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = itds.get_lazy_doc(doc, name)
	else:
		document = itds.get_doc(json.loads(doc))
	...
	try:
		html = get_rendered_template(
			...
			settings=itds.parse_json(settings),
		)
	except itds.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None) -> dict:
	...
	if not print_format or (print_format and not print_format.raw_printing):
		itds.throw(
			_("{0} is not a raw printing format.").format(print_format), itds.TemplateNotFoundError
		)
...
def validate_print_permission(doc: "Document") -> None:
	for ptype in ("read", "print"):
		if itds.has_permission(doc.doctype, ptype, doc):
			return

	if itds.has_website_permission(doc):
		return

	if (key := itds.form_dict.key) and isinstance(key, str) and validate_key(key, doc) is not False:
		return
...
def validate_key(key: str, doc: "Document") -> None:
	document_key_expiry = itds.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise itds.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if itds.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return
...
def get_print_format(doctype: str, print_format: "PrintFormat") -> str:
	if print_format.disabled:
		itds.throw(_("Print Format {0} is disabled").format(print_format.name), itds.DoesNotExistError)

	# server, find template
	module = print_format.module or itds.db.get_value("DocType", doctype, "module")

	is_custom_module = itds.get_cached_value("Module Def", module, "custom")

	if not is_custom_module:
		path = os.path.join(
			get_module_path(module, "Print Format", print_format.name),
			itds.scrub(print_format.name) + ".html",
		)
	...
	itds.throw(_("No template found at path: {0}").format(path), itds.TemplateNotFoundError)
...
def make_layout(doc: "Document", meta: "Meta", format_data=None) -> list:
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/itds/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = itds.get_template("templates/styles/standard.css").render(context)

	if style and itds.db.exists("Print Style", style):
		css = css + "\n" + itds.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data: list, table_meta: "Meta", df: "DocField") -> list["DocField"]:
	...
	columns = []
	doc = data[0] or itds.new_doc(df.options)
