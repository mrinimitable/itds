...
{% if enable_view_tracking %}
	if (navigator.doNotTrack != 1 && !window.is_404) {
		itds.ready(() => {
			let browser = itds.utils.get_browser();
			let query_params = itds.utils.get_query_params();

			// Get visitor ID based on browser uniqueness
			import('/assets/itds/js/lib/fingerprintjs.js')
				.then(fingerprint_js => fingerprint_js.load())
				.then(fp => fp.get())
				.then(result => {
					itds.call("itds.website.doctype.web_page_view.web_page_view.make_view_log", {
