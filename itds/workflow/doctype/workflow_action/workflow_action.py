...
import itds
from itds import _
from itds.desk.form.utils import get_pdf_link
from itds.desk.notifications import clear_doctype_notifications
from itds.email.doctype.email_template.email_template import get_email_template
from itds.model.document import Document
from itds.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from itds.utils import get_datetime, get_url
from itds.utils.background_jobs import enqueue
from itds.utils.data import get_link_to_form
from itds.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from itds.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	itds.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(itds.get_roles(user))
...
def process_workflow_actions(doc, state):
	...
	if send_email_alert(workflow) and itds.db.get_value(
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = itds.session.user
...
def return_success_page(doc):
	itds.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), itds.bold(doc.get("name")), itds.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	itds.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			itds.bold(doc.get("name")),
			itds.bold(doc_workflow_state),
			itds.bold(itds.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else itds.session.user

	allowed_roles = itds.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(itds.get_roles(user))
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = itds._dict(
					{
						"possible_actions": [],
						"email": itds.db.get_value("User", user, "email"),
					}
				)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/itds.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/itds.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		itds.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = itds.db.get_value(
		"Workflow Document State",
