...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("itds.attach_print", return_value={})
		self.patcher.start()
		itds.db.delete("Workflow Action")
		self.workflow = create_todo_workflow()

	def tearDown(self):
		itds.set_user("Administrator")
		self.patcher.stop()
		itds.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = itds.get_all("Workflow Action", fields=["*"])
...
def create_todo_workflow():
	from itds.tests.ui_test_helpers import UI_TEST_USER

	if itds.db.exists("Workflow", "Test ToDo"):
		itds.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not itds.db.exists("Role", TEST_ROLE):
		itds.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if itds.db.exists("User", UI_TEST_USER):
			itds.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = itds.new_doc("Workflow")
...
def create_new_todo():
	return itds.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
