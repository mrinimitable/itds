...
def add_docshare(
	...
	if share_name:
		doc = itds.get_doc("DocShare", share_name)
	else:
		doc = itds.new_doc("DocShare")
	...
	if itds.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = itds.db.get_value("DocShare", {"user": user, "share_name": name, "share_doctype": doctype})

	if share_name:
		itds.delete_doc("DocShare", share_name, flags=flags)
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = itds.get_doc("DocShare", share_name)
		if flags:
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = itds.session.user
	table = itds.qb.DocType("DocShare")
	query = (
		itds.qb.from_(table)
...
def check_share_permission(doctype, name):
	...
	if not itds.has_permission(doctype, ptype="share", doc=name):
		itds.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), itds.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(itds.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		itds.bold(reference_user), itds.bold(_(doctype)), get_title_html(title)
	)
