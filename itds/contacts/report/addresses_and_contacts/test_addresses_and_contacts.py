...
from itds.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from itds.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(itds.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if itds.flags.test_address_created:
		return

	address = itds.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	itds.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if itds.flags.test_contact_created:
		return
	...
	itds.flags.test_contact_created = True
