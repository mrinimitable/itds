...
import itds
from itds import _
from itds.contacts.address_and_contact import set_link_title
from itds.core.doctype.access_log.access_log import make_access_log
from itds.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from itds.model.document import Document
from itds.model.naming import append_number_if_name_exists
from itds.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from itds.contacts.doctype.contact_email.contact_email import ContactEmail
		from itds.contacts.doctype.contact_phone.contact_phone import ContactPhone
		from itds.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def autoname(self):
		...
		if itds.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			itds.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = itds.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not itds.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not itds.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			itds.throw(_("Only one {0} can be set as primary.").format(itds.bold(_("Email ID"))))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			itds.throw(
				_("Only one {0} can be set as primary.").format(itds.bold(itds.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	contact = itds.get_doc("Contact", contact)
	contact.check_permission()

	vcard = contact.get_vcard()
	make_access_log(doctype="Contact", document=contact.name, file_type="vcf")

	itds.response["filename"] = f"{contact.name}.vcf"
	itds.response["filecontent"] = vcard.serialize().encode("utf-8")
	itds.response["type"] = "binary"
...
def download_vcards(contacts: str):
	...
	from itds.utils.data import now

	contact_ids = itds.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = itds.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	itds.response["filename"] = f"{timestamp} Contacts.vcf"
	itds.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	itds.response["type"] = "binary"
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		itds.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = itds.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	from itds.desk.reportview import get_match_cond

	doctype = "Contact"
	if not itds.get_meta(doctype).get_field(searchfield) and searchfield not in itds.db.DEFAULT_COLUMNS:
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = itds.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = itds.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return itds.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from itds.contacts.doctype.address.address import get_condensed_address

	if not itds.has_permission("Contact", "read"):
		return []

	contact_list = itds.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = itds.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = itds.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and itds.has_permission("Address", "read"):
			address = itds.get_doc("Address", contact.address)
