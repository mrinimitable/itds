...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False, lightmode=False):
		self.app = app
		self.site = site
		self.build_number = itds.utils.cint(build_number) or 1
		self.total_builds = itds.utils.cint(total_builds)
	...
	def before_test_setup(self):
		...
		for fn in itds.get_hooks("before_tests", app_name=self.app):
			itds.get_attr(fn)()

		test_module = itds.get_module(f"{self.app}.tests")
	...
	def run_tests_for_file(self, file_info):
		...
		if not self.lightmode:
			from itds.deprecation_dumpster import compat_preload_test_records_upfront
	...
	def get_module(self, path, filename):
		app_path = itds.get_app_path(self.app)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or itds.generate_hash(length=10)
