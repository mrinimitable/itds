	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("itds")
				.then((itds) => {
					itds.boot.sysdefaults.currency = test_case.currency;
					itds.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					itds.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
