context("Kanban Board", () => {
	...
	it("Create ToDo from kanban", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/itds.client.save",
		}).as("save-todo");
	...
	it("Add and Remove fields", () => {
		...
		cy.intercept(
			"POST",
			"/api/method/itds.desk.doctype.kanban_board.kanban_board.save_settings"
		).as("save-kanban");
		cy.intercept(
			"POST",
			"/api/method/itds.desk.doctype.kanban_board.kanban_board.update_order"
		).as("update-order");
		...
		cy.get(".itds-control .label-area").contains("Show Labels").click();
		cy.click_modal_primary_button("Save");
		...
		cy.get_open_dialog()
			.find(
				'.itds-control[data-fieldname="fields_html"] div[data-label="ID"] .remove-field'
			)
			.click();

		cy.wait("@update-order");
		cy.get_open_dialog().find(".itds-control .label-area").contains("Show Labels").click();
		cy.get(".modal-footer .btn-primary").last().click();
	...
	it.skip("Checks if Kanban Board edits are blocked for non-System Manager and non-owner of the Board", () => {
		...
		cy.call("itds.tests.ui_test_helpers.create_test_user", {
			username: not_system_manager,
		});
		cy.remove_role(not_system_manager, "System Manager");
		cy.call("itds.tests.ui_test_helpers.create_todo", { description: "Itds User ToDo" });
		cy.call("itds.tests.ui_test_helpers.create_admin_kanban");
		...
		cy.switch_to_user("Administrator");
		cy.call("itds.client.delete", { doctype: "User", name: not_system_manager });
	});
