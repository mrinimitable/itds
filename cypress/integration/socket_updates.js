context("Realtime updates", () => {
	...
	it("Recieves custom messages from server", { scrollBehavior: false }, () => {
		...
		cy.window()
			.its("itds")
			.then((itds) => {
				itds.realtime.on(event, () => handler.handle());
			});
	...
	it("Progress bar", { scrollBehavior: false }, () => {
		...
		cy.call("itds.tests.ui_test_helpers.publish_progress", { title }).then(() => {
...
function publish_realtime(args) {
	return cy.call("itds.tests.ui_test_helpers.publish_realtime", args);
}
