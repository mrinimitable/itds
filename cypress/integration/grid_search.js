...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((itds) => {
				return itds.xcall(
					"itds.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("itds")
			.then((itds) => {
				itds.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.itds-control[data-fieldname="child_table_1"]').as("table");
